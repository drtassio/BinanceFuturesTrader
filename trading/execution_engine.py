...
class ExecutionOrder:
    ...
    def __init__(self, signal: Signal, strategy: str, order_id: str):
        ...
        self.strategy: str = (
            strategy  # Estratégia de execução utilizada (MARKET, LIMIT, TWAP)
        )
        self.status: OrderStatus = OrderStatus.PENDING  # Status atual da meta-ordem
        self.created_at: datetime = datetime.utcnow().replace(
            tzinfo=timezone.utc
        )  # Timestamp de criação

        self.total_quantity: float = (
            0.0  # Quantidade total que a ordem pretende negociar (em base asset)
        )
        self.filled_quantity: float = 0.0  # Quantidade já preenchida
        self.avg_fill_price: float = 0.0  # Preço médio de preenchimento
        self.notional_value_at_creation: float = (
            0.0  # Valor nocional da ordem no momento da criação
        )
        ...
        logger.info(
            f"🆕 [EXEC] Nova meta-ordem criada: {self.id} (Símbolo: {self.signal.symbol}, Ação: {self.signal.action.value}, Estratégia: {self.strategy}, Alavancagem: {self.signal.leverage:.2f}x)."
        )
    ...
    def is_complete(self) -> bool:
        ...
        return self.status in [
            ...
            self.filled_quantity, self.total_quantity, rel_tol=1e-5
    ...
    def update_with_fill(self, fill_data: Dict[str, Any]) -> Optional[Trade]:
        ...
        if not isinstance(fill_data, dict):
            logger.error(
                f"❌ [ERRO EXEC] 'fill_data' inválido para update_with_fill. Deve ser um dicionário. Recebido: {type(fill_data)}."
            )
            return None
        ...
        try:
            ...
            if (
                newly_filled_qty <= 1e-9
                and OrderStatus(order_status_str) == self.status
            ):
            ...
            if current_order_executed_qty > 0:
                current_trade_price = (
                    current_order_cum_quote_qty / current_order_executed_qty
                )
            ...
            logger.info(
                f"✅ [EXEC] Preenchimento para meta-ordem {self.id}: {newly_filled_qty:.4f} {self.signal.symbol} @ {current_trade_price:.2f}."
            )
            ...
            if newly_filled_qty > 1e-9:
                trade_timestamp_ms = fill_data.get(
                    "updateTime", int(time.time() * 1000)
                )
                trade_timestamp = datetime.fromtimestamp(
                    trade_timestamp_ms / 1000.0, tz=timezone.utc
                )
                ...
                new_trade = Trade(
                    trade_id=str(
                        fill_data.get(
                            "tradeId",
                            f"fill_{self.id}_{int(trade_timestamp.timestamp() * 1000)}",
                        )
                    ),
                    ...
                    fee=float(
                        fill_data.get(
                            "commission",
                            newly_filled_qty * current_trade_price * 0.0004,
                        )
                    ),
            ...
            logger.error(
                f"❌ [ERRO EXEC] Erro ao atualizar meta-ordem {self.id} com fill data: {e}. Data: {fill_data}",
                exc_info=True,
            )
...
class ExecutionEngine:
    ...
    def __init__(
        ...
        system_state: Dict,
        trade_log_callback: Optional[Callable] = None,
    ):
        if not isinstance(config, TradingConfig):
            raise TypeError(
                "🚨 [ERRO EXEC] 'config' deve ser uma instância de TradingConfig."
            )
        if not isinstance(connector, BinanceConnector):
            raise TypeError(
                "🚨 [ERRO EXEC] 'connector' deve ser uma instância de BinanceConnector."
            )
        if not isinstance(portfolio, PortfolioOptimizer):
            raise TypeError(
                "🚨 [ERRO EXEC] 'portfolio' deve ser uma instância de PortfolioOptimizer."
            )
        ...
        self._exec_task: Optional[asyncio.Task] = (
            None  # Task para o loop principal de execução
        )
        self._monitor_task: Optional[asyncio.Task] = (
            None  # Task para o loop de monitoramento de ordens reais
        )
        ...
        self.STOP_BACKOFF_CYCLES = (
            6  # tenta a cada N ciclos quando em backoff (6 × 10s = 60s)
        )
        ...
        self._software_stops: Dict[
            str, float
        ] = {}  # symbol → preço de disparo calculado

        # [PROFIT SHIELD v2.0] Controle de proteção de lucro e realização parcial
        self._partial_realizations: Dict[
            str, bool
        ] = {}  # symbol → True se já realizou parcial
        self._breakeven_activated: Dict[
            str, bool
        ] = {}  # symbol → True se BE está ativo
    ...
    async def resume_pending_twap(self):
        ...
        try:
            ...
            if remaining_qty_base <= 1e-6:
                logger.info(
                    "[TWAP RESUME] Ordem ja totalmente preenchida. Limpando estado."
                )
            ...
            resume_signal = Signal(
                ...
                explanation={
                    "reason": f"TWAP resumido após restart (fatia {state['slice_idx']}/{state['total_slices']})"
                },
            )

            # Recria e requeue a ordem
            resume_order = ExecutionOrder(
                resume_signal, "TWAP", f"resume_{state['order_id']}"
            )
            ...
            logger.info(
                f"[TWAP RESUME] Ordem {resume_order.id} agendada (aguardando live_trading_enabled=True)."
            )
        except Exception as e:
    ...
    async def _enqueue_when_live(self, order: "ExecutionOrder"):
        ...
        if not self.system_state.get("live_trading_enabled", False):
            logger.error(
                f"[TWAP RESUME] Timeout aguardando modo live para {order.id}. Estado TWAP limpo."
            )
            self._clear_twap_state()
            return
        await self.order_queue.put(order)
        logger.info(
            f"[TWAP RESUME] ✅ Ordem {order.id} enfileirada após live_trading_enabled=True."
        )
    ...
    async def _cancel_orphan_orders(self):
        ...
        try:
            ...
            if not open_orders:
                logger.info(
                    f"[STARTUP] Nenhuma ordem aberta encontrada para {primary_symbol}. Nada a limpar."
                )
                return
            ...
            logger.info(
                f"[STARTUP] {len(open_orders)} ordens abertas em {primary_symbol}: {type_summary}"
            )
                ...
                if order_type not in protected_types:
                    ...
                    try:
                        await self.connector._make_request(
                            "DELETE",
                            "/fapi/v1/order",
                            params={"symbol": primary_symbol, "orderId": oid},
                            signed=True,
                        )
                        cancelled += 1
                        logger.warning(
                        ...
                        logger.warning(
                            f"⚠️ [STARTUP] Falha ao cancelar ordem {oid}: {ce}"
                        )

            if cancelled:
                logger.warning(
                    f"[STARTUP] ✅ {cancelled}/{len(open_orders)} ordem(ns) zumbi cancelada(s) para {primary_symbol}."
                )
            else:
                logger.info(
                    f"[STARTUP] Todas as {len(open_orders)} ordens abertas são proteções ativas. Nenhuma cancelada."
                )
        except Exception as e:
    ...
    async def start(self):
        ...
        self._exec_task = asyncio.create_task(
            self._execution_loop(), name="ExecutionEngine_ExecLoop"
        )
        self._monitor_task = asyncio.create_task(
            self._monitor_orders_loop(), name="ExecutionEngine_MonitorLoop"
        )
    ...
    async def stop(self):
        ...
        try:
            ...
            await asyncio.gather(
                self._exec_task, self._monitor_task, return_exceptions=True
            )
            ...
            logger.error(
                f"❌ [ERRO EXEC] Exceção durante o desligamento do ExecutionEngine: {e}",
                exc_info=True,
            )
    ...
    async def _get_cached_symbol_info(self, symbol: str) -> Optional[Dict]:
        ...
        if symbol not in self._symbol_info_cache:
            try:
                ...
                logger.debug(
                    f"[CACHE] symbol_info armazenado para {symbol}: qty_prec={info.get('quantityPrecision') if info else 'N/A'}"
                )
            except Exception as e:
                logger.warning(
                    f"⚠️ [CACHE] Falha ao buscar symbol_info para {symbol}: {e}. Usando defaults."
                )
    ...
    async def _cancel_limit_if_not_filled(
        ...
        try:
            ...
            cancel_result = await self.connector.cancel_order(
                order.signal.symbol, exchange_order_id
            )
            if cancel_result:
                logger.info(
                    f"✅ [LIMIT TIMEOUT] Ordem {exchange_order_id} cancelada com sucesso."
                )
            else:
                logger.warning(
                    f"⚠️ [LIMIT TIMEOUT] Cancelamento retornou vazio — pode ja ter preenchido."
                )

            # Consulta status final antes de reenviar
            final_status = await self.connector.get_order(
                order.signal.symbol, order_id=exchange_order_id
            )
            if final_status and final_status.get("status") == "FILLED":
                logger.info(
                    f"ℹ️ [LIMIT TIMEOUT] Ordem {exchange_order_id} ja estava FILLED. Nao re-executa."
                )
            ...
            logger.error(
                f"❌ [LIMIT TIMEOUT] Erro ao cancelar/re-executar {order.id}: {e}",
                exc_info=True,
            )
    ...
    async def submit_order(self, signal: Signal):
        ...
        if not isinstance(signal, Signal):
            logger.error(
                f"❌ [ERRO EXEC] Tentativa de submeter objeto não-Signal. Ignorando: {signal}"
            )
            return
        ...
        if has_position:
            if signal.action == Action.CLOSE:
                is_reducing = True
                logger.info(
                    f"ℹ️ [EXEC] Sinal CLOSE recebido. Ignorando limite de posição para fechar {signal.symbol}."
                )
                ...
                if is_reducing:
                    logger.info(
                        f"ℹ️ [EXEC] Sinal {signal.action} oposto à posição atual ({current_pos.quantity}). Permitindo redução/fechamento."
                    )

            if not is_reducing:
                logger.warning(
                    f"⚠️ [EXEC] LIMITE DE POSIÇÃO: Já existe uma posição aberta para {signal.symbol} e sinal não é de redução. Ordem ignorada."
                )
        ...
        if current_price <= 0:
            logger.error(
                f"❌ [ERRO EXEC] Não foi possível obter o preço atual para {signal.symbol} ou preço é zero. Ordem {order_id} REJEITADA."
            )
            exec_order.status = OrderStatus.REJECTED
            self.active_orders[order_id] = exec_order
            self.system_state["command_feedback"] = (
                f"Erro: Ordem {order_id} REJEITADA (Preço {signal.symbol} inválido)."
            )
            return
        ...
        if is_reducing and current_pos:
            ...
            if signal.action == Action.CLOSE or signal.position_size_pct >= 0.9:
                ...
                logger.info(
                    f"🛡️ [EXEC] Ordem de FECHAMENTO TOTAL para {signal.symbol}. Qtd exata: {quantity:.6f}"
                )
            else:
                # Redução parcial: ainda usa a quantidade calculada, mas marca como reduceOnly
                exec_order.reduce_only = True
                logger.info(
                    f"🛡️ [EXEC] Ordem de REDUÇÃO PARCIAL para {signal.symbol}. reduceOnly ativado."
                )
        ...
        if exec_order.total_quantity <= 0:
            logger.warning(
                f"⚠️ [EXEC] Quantidade calculada para ordem {order_id} é zero ou negativa. Ordem REJEITADA. Margem alocar: ${margin_to_allocate_usd:,.2f}, Valor Nocional: ${notional_trade_value_usd:,.2f}, Preço: {current_price}."
            )
            exec_order.status = OrderStatus.REJECTED
            self.active_orders[order_id] = exec_order
            self.system_state["command_feedback"] = (
                f"Aviso: Ordem {order_id} REJEITADA (Quantidade zero)."
            )
            return

        self.active_orders[order_id] = (
            exec_order  # Adiciona ao dicionário de ordens ativas
        )
        await self.order_queue.put(
            exec_order
        )  # Coloca a ordem na fila para processamento

        logger.info(
            f"📦 [EXEC] Meta-ordem {order_id} ({signal.action.value} {exec_order.total_quantity:.4f} {signal.symbol}) enfileirada com estratégia: {strategy}, Alavancagem: {signal.leverage:.2f}x. Valor Nocional: ${notional_trade_value_usd:,.2f}, Margem: ${margin_to_allocate_usd:,.2f}."
        )
        self.system_state["command_feedback"] = (
            f"Ordem {order_id} ({signal.action.value} {exec_order.total_quantity:.4f} {signal.symbol}) enfileirada. Nocional: ${notional_trade_value_usd:,.2f}."
        )
        ...
        while True:
            try:
                ...
                if order.status == OrderStatus.PENDING:
                    ...
                    )  # Cria uma task para executar a ordem, não bloqueia a fila
                else:
                    logger.debug(
                        f"ℹ️ [EXEC LOOP] Ordem {order.id} não está PENDING ({order.status.value}). Ignorando."
                    )
                ...
                logger.exception(
                    f"❌ [ERRO EXEC LOOP] Erro inesperado no loop de execução de ordens: {e}"
                )
                await asyncio.sleep(1)
    ...
    async def _monitor_orders_loop(self):
        ...
        logger.info(
            f"⚙️ [MONITOR] Loop de monitoramento de ordens e posições iniciado (checa a cada {monitor_interval}s)."
        )
        while True:
            try:
                ...
                logger.error(
                    f"❌ [ERRO MONITOR] Erro inesperado no loop de monitoramento: {e}",
                    exc_info=True,
                )
                await asyncio.sleep(monitor_interval)
            ...
            for symbol, position in positions_copy:
                ...
                if last_veto:
                    ...
                    if time_diff < 300:  # 5 minutos de cooldown
                        logger.debug(
                            f"[MONITOR] {symbol}: Ignorando monitoramento de stop devido a veto de margem recente ({time_diff:.0f}s atrás)."
                        )
                        continue
                    else:
                        # Limpa veto antigo
                ...
                if software_stop_price is not None:
                    ...
                    if current_price > 0:
                        stop_breached = (
                            position.quantity > 0
                            and current_price <= software_stop_price
                        ) or (
                            position.quantity < 0
                            and current_price >= software_stop_price
                        )
                        if stop_breached:
                            ...
                            qty_prec = (
                                int(sym_info["quantityPrecision"]) if sym_info else 3
                            )
                            ...
                            if close_result and close_result.get("orderId"):
                                logger.warning(
                                    f"[MONITOR] ✅ Stop software executado: {symbol} fechado @ market. OrderId={close_result.get('orderId')}"
                                )
                ...
                if not has_stop:
                    ...
                    if self._stop_fail_count.get(symbol, 0) > 0:
                        logger.info(
                            f"[MONITOR] {symbol}: stop ativo detectado. Resetando backoff."
                        )
                    self._stop_fail_count[symbol] = 0

        except Exception as e:
            logger.error(
                f"[MONITOR] Erro em _manage_existing_positions: {e}", exc_info=True
            )
    ...
    async def _apply_profit_protection_logic(self, symbol: str, position: Position):
        ...
        try:
            ...
            if roi_display >= 2.0 and not self._breakeven_activated.get(symbol):
                ...
                logger.info(
                    f"🛡️ [PROFIT SHIELD] {symbol} atingiu {roi_display:.2f}% ROI. BREAK-EVEN ATIVADO @ {be_price:.2f}"
                )

            # --- FASE 2: REALIZAÇÃO PARCIAL 50% (ROI > 4%) ---
            if roi_display >= 4.0 and not self._partial_realizations.get(symbol):
                logger.warning(
                    f"💰 [PROFIT SHIELD] {symbol} atingiu {roi_display:.2f}% ROI. Executando REALIZAÇÃO PARCIAL (50%)."
                )
                ...
                if partial_qty > 0:
                    ...
                    if result and result.get("orderId"):
                        ...
                        logger.warning(
                            f"✅ [PROFIT SHIELD] Realização parcial executada para {symbol}. Qtd: {partial_qty}"
                        )
                    else:
                        logger.error(
                            f"❌ [PROFIT SHIELD] Falha ao realizar parcial para {symbol}: {result}"
                        )
            ...
            if roi_display >= 5.0:
                ...
                if is_better:
                    ...
                    logger.info(
                        f"🦾 [PROFIT SHIELD] {symbol} em lucro forte ({roi_display:.2f}%). Trailing Software Stop apertado para {target_stop:.2f}"
                    )

        except Exception as e:
            logger.error(
                f"❌ [ERRO PROFIT SHIELD] Falha ao aplicar lógica para {symbol}: {e}"
            )
    ...
    async def _execute_order(self, order: ExecutionOrder):
        ...
        if is_live and self.config.MAX_LEVERAGE > 1:
            try:
                ...
                await self.connector.set_leverage_for_symbol(
                    order.signal.symbol, int(order.signal.leverage)
                )
                logger.debug(
                    f"🌐 [LIVE] Alavancagem ajustada para {order.signal.symbol} para {int(order.signal.leverage)}x."
                )
            except Exception as e:
                logger.error(
                    f"❌ [ERRO LIVE] Falha ao ajustar alavancagem para {order.signal.symbol} para {order.signal.leverage:.2f}x: {e}. Ordem pode falhar.",
                    exc_info=True,
                )
                order.status = OrderStatus.REJECTED
                self.system_state["command_feedback"] = (
                    f"Erro: Ordem {order.id} REJEITADA (Falha ao ajustar alavancagem)."
                )
                return

        if is_live:
            logger.info(
                f"🟢 [LIVE EXEC] Iniciando execução REAL da meta-ordem {order.id} com a estratégia {order.strategy}."
            )
            ...
            execution_function = strategy_map.get(
                order.strategy, self._execute_market_real
            )
            await execution_function(order)
        else:
            logger.info(
                f"📝 [PAPER EXEC] Simulando execução de meta-ordem {order.id} com estratégia {order.strategy}."
            )
        ...
        if order.status == OrderStatus.FILLED:
            self.system_state["command_feedback"] = (
                f"Ordem {order.id} ({order.signal.action.value} {order.total_quantity:.4f} {order.signal.symbol}) PREENCHIDA. Nocional: ${order.notional_value_at_creation:,.2f}."
            )
        elif order.status == OrderStatus.REJECTED:
            self.system_state["command_feedback"] = (
                f"Ordem {order.id} ({order.signal.symbol}) REJEITADA."
            )
    ...
    def _choose_smart_strategy(self, signal: Signal) -> str:
        ...
        notional_value_usd = (
            portfolio_value * signal.position_size_pct * signal.leverage
        )
        ...
        ) > self.config.TWAP_THRESHOLD_PCT:
            return "TWAP"
    ...
    async def _execute_ladder_entry(self, order: ExecutionOrder):
        ...
        try:
            ...
            )  # No SELL, queremos vender mais ALTO. No BUY, comprar mais BAIXO.
            ...
            for i, weight in enumerate(weights):
                ...
                if slice_qty <= 0 or slice_notional < 55:
                    ...
                    if i > 0 and len(batch_orders) > 0:
                        ...
                        batch_orders[0]["quantity"] = (
                            f"{first_qty + slice_qty:.{qty_precision}f}"
                        )
                        continue
                    else:
                        # Se for a primeira e for pequena, aborta ladder e faz market único
                        logger.info(
                            f"⚠️ [LADDER] Ordem muito pequena para fatiamento ponderado. Executando como MARKET único."
                        )
                        await self._execute_market_real(order)
                        return
            ...
            logger.info(
                f"🧱 [LADDER PONDERADO] Enviando BATCH (40/20/15/15/10) para {symbol}..."
            )
            ...
            if results and isinstance(results, list):
                ...
                if main_res.get("orderId"):
                    ...
                    logger.info(
                        f"✅ [LADDER] Batch enviado com sucesso. ID Principal: {main_res.get('orderId')}"
                    )
            else:
                logger.error(f"❌ [LADDER] Falha ao enviar Batch: {results}")
                await self._execute_market_real(
                    order
                )  # Fallback para market se batch falhar
    ...
    async def _execute_paper_trade(self, order: ExecutionOrder):
        ...
        if order.total_quantity <= 0:
            logger.warning(
                f"⚠️ [PAPER] Ordem de papel {order.id} tem quantidade zero. Marcando como REJEITADA."
            )
            order.status = OrderStatus.REJECTED
            return
        ...
        if simulated_price <= 0:
            logger.error(
                f"❌ [PAPER] Não foi possível obter preço para {order.signal.symbol}. Ordem {order.id} REJEITADA."
            )
            order.status = OrderStatus.REJECTED
            return

        slippage_factor = (
            1.0 + self.config.SLIPPAGE_TOLERANCE_PCT
            if order.signal.action == OrderSide.BUY
            else 1.0 - self.config.SLIPPAGE_TOLERANCE_PCT
        )
        ...
        logger.info(
            f"📝 [PAPER] Ordem {order.id} ({order.signal.action.value} {order.total_quantity:.4f}) SIMULADA @ ${executed_price:.2f}."
        )
    ...
    async def _execute_market_real(self, order: ExecutionOrder):
        ...
        if order.total_quantity <= 0:
            logger.warning(
                f"⚠️ [LIVE] Ordem a mercado {order.id} tem quantidade zero. Marcando como REJEITADA."
            )
            order.status = OrderStatus.REJECTED
            return
        ...
        logger.info(
            f"🌐 [LIVE] Enviando ordem a mercado real {client_order_id} para {order.signal.symbol} (Qtd: {params['quantity']}, Alavancagem: {order.signal.leverage:.2f}x)..."
        )
        ...
        if result:
            ...
            if order.status != OrderStatus.FILLED:
                ...
                logger.info(
                    f"⏳ [LIVE] Ordem {result.get('orderId')} retornou status {original_status}. Iniciando polling de preenchimento..."
                )

                for i in range(5):  # Tenta por 5 segundos
                    await asyncio.sleep(1)
                    updated_order_info = await self.connector.get_order(
                        order.signal.symbol, order_id=result.get("orderId")
                    )
                    ...
                    if updated_order_info:
                        ...
                        if current_status == "FILLED":
                            logger.info(
                                f"✅ [LIVE] Polling: Ordem {result.get('orderId')} agora está FILLED!"
                            )
                            ...
                            current_status == "CANCELED" or current_status == "REJECTED"
                        ):
                            logger.warning(
                                f"⚠️ [LIVE] Polling: Ordem {result.get('orderId')} foi {current_status}."
                            )
            ...
            if order.status == OrderStatus.FILLED:
                logger.info(
                    f"✅ [LIVE] Ordem a mercado {result.get('orderId')} para {order.id} PREENCHIDA."
                )
                ...
                if new_trade:
                    ...
                    logger.debug(
                        f"✅ [LIVE] Portfolio atualizado com trade real: {new_trade.side.value} {new_trade.quantity:.4f} @ {new_trade.executed_price:.2f}"
                    )
                    ...
                    sl_pct = (
                        ...
                        if (
                            order.signal.stop_loss is not None
                            and order.signal.stop_loss > 0
                        )
                    ...
                    tp_pct = (
                        ...
                        if (
                            order.signal.take_profit is not None
                            and order.signal.take_profit > 0
                        )
                    ...
                    logger.warning(
                        f"⚠️ [SAFETY] Nao foi possivel obter objeto Trade para colocar protecoes em {order.id}."
                    )
            else:
                logger.warning(
                    f"⚠️ [LIVE] Ordem a mercado {result.get('orderId')} para {order.id} não foi FILLED após polling (Status: {order.status.value})."
                )
        else:
            logger.error(
                f"❌ [ERRO LIVE] Falha crítica ao enviar ordem a mercado para {order.id}. Resultado nulo do conector."
            )
            order.status = OrderStatus.REJECTED
            order.signal.explanation["rejection_reason"] = (
                "Falha de comunicação com a exchange ou erro na ordem."
            )
    ...
    async def _execute_limit_real(self, order: ExecutionOrder):
        ...
        current_market_price = (
            float(ticker["price"]) if ticker and "price" in ticker else 0.0
        )

        if current_market_price <= 0:
            logger.error(
                f"❌ [LIVE] Não foi possível obter preço de mercado para {order.signal.symbol} para ordem limite. Ordem {order.id} REJEITADA."
            )
            order.status = OrderStatus.REJECTED
            self.system_state["command_feedback"] = (
                f"Erro: Ordem {order.id} REJEITADA (Preço de mercado inválido para ordem limite)."
            )
            return
        ...
        if order.signal.action == OrderSide.BUY:
            ...
            limit_price = current_market_price * (
                1.0 - self.config.SLIPPAGE_TOLERANCE_PCT * 0.1
            )
        ...
        if limit_price <= 0:
            logger.error(
                f"❌ [LIVE] Preço limite calculado para {order.id} é zero ou negativo. Executando como ordem de mercado como fallback."
            )
            await self._execute_market_real(order)
            return
        ...
        params = {
            ...
            ),  # Arredonda o preço para a precisão da exchange
        ...
        logger.info(
            f"🌐 [LIVE] Enviando ordem limite real {client_order_id} para {order.signal.symbol} (Qtd: {params['quantity']}, Preço: {params['price']})..."
        )
        result = await self.connector.place_order(params)
        ...
        if result:
            ...
            if order.status == OrderStatus.FILLED:
                logger.info(
                    f"✅ [LIVE] Ordem limite {result.get('orderId')} para {order.id} PREENCHIDA IMEDIATAMENTE."
                )
                if new_trade:
                ...
                logger.info(
                    f"⏳ [LIVE] Ordem limite {result.get('orderId')} para {order.id} PARCIALMENTE PREENCHIDA."
                )
            ...
            logger.error(
                f"❌ [ERRO LIVE] Falha crítica ao enviar ordem limite para {order.id}. Resultado nulo do conector."
            )
            order.status = OrderStatus.REJECTED
            order.signal.explanation["rejection_reason"] = (
                "Falha de comunicação com a exchange ou erro na ordem limite."
            )
    ...
    async def _execute_twap_real(self, order: ExecutionOrder):
        ...
        if order.total_quantity <= 0:
            logger.warning(
                f"⚠️ [LIVE] Ordem TWAP {order.id} tem quantidade zero. Marcando como REJEITADA."
            )
            order.status = OrderStatus.REJECTED
            return
        ...
        sl_pct_order = (
            order.signal.stop_loss
            if (order.signal.stop_loss is not None and order.signal.stop_loss > 0)
            else self.config.DEFAULT_STOP_LOSS_PCT
        )
        tp_pct_order = (
            order.signal.take_profit
            if (order.signal.take_profit is not None and order.signal.take_profit > 0)
            else self.config.DEFAULT_TAKE_PROFIT_PCT
        )
        ...
        self._save_twap_state(
            order, 0, num_slices, 0.0, order.total_quantity, sl_pct_order, tp_pct_order
        )
        ...
        for i in range(num_slices):
            ...
            if i > 0 and order.filled_quantity > 0:
                try:
                    pos_risk = await self.connector._make_request(
                        ...
                        params={"symbol": order.signal.symbol, "recvWindow": 5000},
                        signed=True,
                    )
                    exchange_qty = 0.0
                    if pos_risk:
                        exchange_qty = sum(
                            ...
                            for p in pos_risk
                            if p.get("symbol") == order.signal.symbol
                        )
                    ...
                    logger.warning(
                        f"⚠️ [TWAP POSITION CHECK] Falha ao verificar posição: {pos_check_err}. Continuando TWAP."
                    )

            if (
                order.is_complete
                or order.status == OrderStatus.CANCELED
                or self.system_state.get("shutdown_imminent", False)
            ):
                logger.info(
                    f"🎉 [LIVE] Execução TWAP da meta-ordem {order.id} concluída ou cancelada prematuramente."
                )
                break
            ...
            try:
                ...
                if math.isclose(remaining_qty, 0.0, abs_tol=1e-9):
                    order.status = OrderStatus.FILLED
                    logger.info(
                        f"✅ [LIVE] TWAP para {order.id}: Quantidade restante insignificante. Ordem marcada como FILLED."
                    )
                    break
                ...
                symbol_info = await self._get_cached_symbol_info(order.signal.symbol)
                qty_precision = (
                    int(symbol_info["quantityPrecision"]) if symbol_info else 3
                )
                ...
                if current_slice_qty <= 0:
                    logger.info(
                        f"ℹ️ [LIVE] TWAP para {order.id}: Quantidade da fatia {i + 1} é zero ou negativa. Finalizando TWAP."
                    )
                    break
                ...
                logger.info(
                    f"🌐 [LIVE] Enviando fatia TWAP {i + 1}/{num_slices} para {order.id} (Qtd: {current_slice_qty:.4f})..."
                )
                    ...
                    if new_trade:
                        if self.trade_log_callback:
                            trade_log_data = {
                                ...
                                "notional_value": new_trade.quantity
                                * new_trade.executed_price,
                                "leverage": order.signal.leverage,
                                "profit_probability": order.signal.profit_probability,
                                "order_id": result.get("orderId", order.id),
                        ...
                        confidence_factor = (
                            0.5 + confidence * 1.0
                        )  # range [0.5, 1.5] para conf [0.0, 1.0]
                        trailing_sl_pct = float(
                            np.clip(
                                sl_pct * confidence_factor, sl_pct * 0.5, sl_pct * 2.0
                            )
                        )
                        ...
                        try:
                            ...
                            await self._place_trailing_stop(
                                ...
                                side=new_trade.side,
                                quantity=abs(
                                    self.portfolio.get_position_quantity(
                                        new_trade.symbol
                                    )
                                ),
                            ...
                            logger.warning(
                                f"⚠️ [TWAP SAFETY] Erro ao colocar proteções para fatia: {protect_err}"
                            )
                    # <<-- FIM DA CORREÇÃO -->>
                ...
                logger.info(
                    f"[LIVE] TWAP {order.id} cancelado na fatia {i + 1}. Estado salvo para retomada."
                )
                order.status = OrderStatus.CANCELED
                break
            except Exception as e:
                logger.error(
                    f"[LIVE] Erro na fatia TWAP {i + 1} para {order.id}: {e}",
                    exc_info=True,
                )
        ...
        if not order.is_complete:
            logger.warning(
                f"⚠️ [LIVE] Loop TWAP para meta-ordem {order.id} finalizado, mas a ordem nao esta completa. Status final: {order.status.value}."
            )

            if order.filled_quantity > 0 and math.isclose(
                order.filled_quantity, order.total_quantity, rel_tol=1e-5
            ):
                order.status = OrderStatus.FILLED
                logger.info(
                    f"✅ [LIVE] Forcando status FILLED para TWAP {order.id} devido a preenchimento quase completo."
                )
            else:
                logger.warning(
                    f"⚠️ [LIVE] TWAP {order.id} nao preenchida completamente. Quantidade faltante: {order.total_quantity - order.filled_quantity:.4f}."
                )
        ...
        if order.status == OrderStatus.FILLED and order.trades:
            ...
            logger.info(
                f"[TWAP] Protecoes colocadas para TWAP {order.id}: Hard Stop + TP + Trailing Stop."
            )
    ...
    async def _check_position_limit(self, symbol: str) -> bool:
        ...
        try:
            ...
            if (
                symbol in self.portfolio.positions
                and self.portfolio.positions[symbol].quantity != 0
            ):
                logger.info(
                    f"🔒 [EXEC] Posição existente encontrada no portfolio para {symbol}."
                )
                return True
            ...
            if is_live:
                ...
                if symbol in positions and positions[symbol].get("quantity", 0) != 0:
                    logger.info(
                        f"🔒 [EXEC] Posição existente encontrada na exchange para {symbol}."
                    )
                    return True
            ...
            logger.error(
                f"❌ [ERRO EXEC] Erro ao verificar limite de posição: {e}",
                exc_info=True,
            )
    ...
    async def _place_trailing_stop(
        ...
        try:
            ...
            logger.info(
                f"   ├─ Posição: {side.value} {quantity:.6f} -> Ajustado: {final_qty}"
            )
            logger.info(
                f"   └─ Trailing Stop: {trailing_side} @ {callback_rate:.1f}% callback"
            )
            ...
            if result and result.get("software_stop"):
                ...
                logger.warning(
                    f"[EXEC] 🛡️ Stop SOFTWARE ativado para {symbol} @ {sw_price:.2f} (API rejeitou todas as tentativas)"
                )
                self.system_state["command_feedback"] = (
                    f"Stop SOFTWARE {symbol} @ {sw_price:.2f}"
                )
                return result
                ...
                logger.info(
                    f"[EXEC] Stop {order_type} {order_id} colocado para {symbol} @ {callback_rate:.1f}%"
                )
                self.system_state["command_feedback"] = (
                    f"Stop ativo {symbol} @ {callback_rate:.1f}%"
                )
                return result
            else:
            ...
            logger.error(
                f"[EXEC] Erro ao colocar Trailing Stop para {symbol}: {e}",
                exc_info=True,
            )
            return None
    ...
    async def _place_initial_hard_stop(self, trade: Trade, stop_loss_pct: float):
        ...
        try:
            ...
            result = await self.connector.place_stop_loss_order(
                symbol=symbol, side=sl_side, quantity=quantity, stop_price=stop_price
            )
            ...
            if result:
                ...
                logger.critical(
                    f"🚨 [PERIGO] Falha ao colocar Hard Stop para {symbol}! Posição desprotegida!"
                )
                self.system_state["command_feedback"] = (
                    f"PERIGO: Falha ao colocar Stop Loss para {symbol}!"
                )

        except Exception as e:
            logger.error(
                f"❌ [ERRO SAFETY] Erro crítico ao colocar Hard Stop: {e}",
                exc_info=True,
            )
    ...
    async def _place_initial_take_profit(self, trade: Trade, take_profit_pct: float):
        ...
        try:
            ...
            result = await self.connector.place_take_profit_order(
                symbol=symbol, side=tp_side, quantity=quantity, stop_price=stop_price
            )
            ...
            if result:
                ...
                logger.warning(
                    f"⚠️ [SAFETY] Falha ao colocar Take Profit para {symbol}."
                )

        except Exception as e:
            logger.error(
                f"❌ [ERRO SAFETY] Erro ao colocar Take Profit: {e}", exc_info=True
            )
    ...
    async def reconcile_with_exchange(self):
        ...
        try:
            logger.info(
                "🔄 [RECONCILIATION] Iniciando sincronização total com a Binance..."
            )
            summary = await self.connector.get_account_summary()

            if not summary or "total_value" not in summary:
                logger.warning(
                    "⚠️ [RECONCILIATION] Falha ao obter resumo da conta. Abortando sync."
                )
                return
            ...
            self.portfolio.margin_used = summary.get(
            ...
            logger.error(
                f"❌ [RECONCILIATION] Erro durante sincronização: {e}", exc_info=True
            )
    ...
    def register_margin_veto(self, symbol: str):
        ...
        logger.warning(
            f"🛡️ [EXEC] Registro de VETO POR MARGEM para {symbol}. Monitoramento de stop será suprimido por 5 minutos."
        )
