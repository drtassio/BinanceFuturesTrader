...
class PortfolioOptimizer:
    def __init__(self, config: TradingConfig):
        ...
        self.positions: Dict[str, Position] = {} # Posições abertas: {'symbol': Position}
        self.portfolio_value_history = pd.Series(dtype=float) # Histórico do valor total do portfólio
        self.last_prices: Dict[str, float] = {} # Armazena os últimos preços conhecidos para cálculo de PnL não realizado
        ...
        logger.info(f"💰 [PORTFOLIO] PortfolioOptimizer inicializado. Capital Inicial: ${self.initial_capital:,.2f}.")
    ...
    def update_from_trade(self, trade: Trade):
        ...
        logger.debug(f"💰 [PORTFOLIO] Processando trade para atualização de portfólio: {trade.side.value} {executed_quantity_abs:.4f} {symbol} @ ${executed_price:.2f} (Fee: ${fee:.4f}).")
        ...
        if current_position is None or (np.sign(current_position.quantity) != (1 if trade_is_long else -1) and abs(current_position.quantity) > 1e-9):
            ...
            if current_position is not None:
                ...
                qty_to_close_by_trade = min(abs(current_position.quantity), executed_quantity_abs)
                
                if current_position.quantity > 0 and trade_is_short: # Fechando LONG com SELL
                    pnl_realized_trade = (executed_price - current_position.entry_price) * qty_to_close_by_trade
                elif current_position.quantity < 0 and trade_is_long: # Fechando SHORT com BUY
                    pnl_realized_trade = (current_position.entry_price - executed_price) * qty_to_close_by_trade

                self.cash += pnl_realized_trade # Adiciona PnL realizado ao caixa
                logger.debug(f"💰 [PORTFOLIO] PnL Realizado na reversão de {qty_to_close_by_trade:.4f} {symbol}: ${pnl_realized_trade:,.2f}. Cash atualizado: ${self.cash:,.2f}.")
            ...
            new_quantity = current_position.quantity + (executed_quantity_abs if trade_is_long else -executed_quantity_abs) if current_position else (executed_quantity_abs if trade_is_long else -executed_quantity_abs)
        ...
        # Cenário 2: Aumentando uma posição existente na mesma direção
            ...
            old_notional_value = current_position.quantity * current_position.entry_price
            new_notional_value = old_notional_value + (executed_quantity_abs * executed_price if trade_is_long else -executed_quantity_abs * executed_price)
            
            new_quantity = current_position.quantity + (executed_quantity_abs if trade_is_long else -executed_quantity_abs)
            new_entry_price = new_notional_value / (new_quantity + np.finfo(float).eps) # Evita divisão por zero
            
            self.positions[symbol] = replace(current_position, quantity=new_quantity, entry_price=new_entry_price, timestamp=trade.timestamp)
            logger.debug(f"📊 [PORTFOLIO] Posição em {symbol} AUMENTADA. Nova Qtd: {new_quantity:.4f}, Novo Entry: ${new_entry_price:.2f}.")
        ...
        logger.info(f"💰 [PORTFOLIO] Estado do Portfólio atualizado após trade de {trade.symbol}. Cash: ${self.cash:,.2f}, Posições Ativas: {len(self.positions)}.")
        # Atualiza o valor total do portfólio no histórico imediatamente
        self.update_portfolio_value({symbol: executed_price})


    def update_portfolio_value(self, current_prices: Dict[str, float], timestamp: Optional[datetime] = None):
        """
        ...
        for symbol, position in self.positions.items():
            if symbol in self.last_prices:
                ...
                logger.warning(f"⚠️ [PORTFOLIO] Preço atual para {symbol} não disponível para cálculo de PnL não realizado.")
        ...
        self.portfolio_value_history = pd.concat([self.portfolio_value_history, new_entry]).sort_index()

        logger.debug(f"💰 [PORTFOLIO] Valor do Portfólio atualizado: ${total_value:,.2f}. PnL Não Realizado: ${total_unrealized_pnl:,.2f}.")
    ...
    def get_detailed_status(self) -> Dict[str, Any]:
        ...
        capital_for_leverage_calc = self.get_total_value() # Ou self.initial_capital se a alavancagem for sobre isso
        total_leverage_ratio = self.total_notional_value / (capital_for_leverage_calc + np.finfo(float).eps)
    ...
    def load_state(self, state: Dict[str, Any]):
        ...
        for pos_data in state.get('positions', []):
            self.positions[pos_data['symbol']] = Position(
                ...
                unrealized_pnl=pos_data.get('unrealized_pnl', 0.0), # Assume 0.0 se não estiver presente
                leverage=pos_data.get('leverage', 1)
            )
        # Recarrega o histórico de valor do portfólio se salvo
        if 'portfolio_value_history' in state and state['portfolio_value_history'] is not None:
            ...
            if isinstance(state['portfolio_value_history'], dict):
                ...
                history_dict = {datetime.fromisoformat(k): v for k, v in state['portfolio_value_history'].items()}
                self.portfolio_value_history = pd.Series(history_dict).sort_index()
            elif isinstance(state['portfolio_value_history'], list): # Se for lista de {'timestamp', 'equity'}
                timestamps = [datetime.fromisoformat(item['timestamp']) for item in state['portfolio_value_history']]
                equities = [item['equity'] for item in state['portfolio_value_history']]
                self.portfolio_value_history = pd.Series(equities, index=timestamps).sort_index()
            logger.info(f"✅ [PORTFOLIO] Histórico do portfólio restaurado com {len(self.portfolio_value_history)} entradas.")
        else:
            logger.info("ℹ️ [PORTFOLIO] Histórico do portfólio não encontrado no estado. Iniciando novo histórico.")
            self.portfolio_value_history = pd.Series(dtype=float)
            self.portfolio_value_history.loc[datetime.utcnow()] = self.get_total_value() # Inicia com o valor total atual

        logger.info(f"✅ [PORTFOLIO] Estado do portfólio carregado: {len(self.positions)} posições, Cash: ${self.cash:,.2f}.")