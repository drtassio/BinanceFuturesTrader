            ...
            try:
                with open(ae_features_path, 'r') as f:
                    ...
                    if isinstance(cols, list) and cols:
                        ...
                        logger.debug(f" Contrato de features inferido: {len(cols)} colunas")
            except Exception as e:
    ...
    def load_all_models(self):
        ...
        try:
            ...
            if not self.specialists or not self.hrl_master:
                # Determina input_dim a partir dos metadados ou padro
                input_dim = int(self.metadata.get('final_feature_count') or 54)
                logger.info(f" [FAST_STARTUP] Inicializando agentes de deciso com input_dim={input_dim}...")
