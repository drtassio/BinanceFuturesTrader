...
# [REACTIVATED] ProfitabilityPredictor — Meta-Labeling (Lopez de Prado 2018)
...
class AIController:
    ...
    def __init__(
        ...
        self.metadata_path: str = os.path.join(
            self.config_ai.MODEL_DIR, "training_metadata.json"
        )
        self.metadata: Dict[str, Any] = {
            ...
            "models": {
                ...
                "bull_specialist_sac": {
                    "trained": False,
                    "path": "bull_specialist_sac.zip",
                    "trained_on": None,
                },
                "bear_specialist_sac": {
                    "trained": False,
                    "path": "bear_specialist_sac.zip",
                    "trained_on": None,
                },
                "ranger_specialist_sac": {
                    "trained": False,
                    "path": "ranger_specialist_sac.zip",
                    "trained_on": None,
                },
            },
    ...
    def set_feature_pipeline(self, pipeline: FeatureEngineeringPipeline):
        self.feature_pipeline = pipeline

    def set_portfolio(self, portfolio: PortfolioOptimizer):
        self.portfolio = portfolio

    def set_risk_manager(self, manager: RiskManager):
        self.risk_manager = manager

    def set_ai_monitor(self, monitor: AIMonitor):
        self.ai_monitor = monitor

    def set_drift_detector(self, detector: Any):
        self.drift_detector = detector

    def set_curriculum_engine(self, engine: CurriculumEngine):
        self.curriculum_engine = engine

    def set_tape_engine(self, engine: TapeEngine):
        self.tape_engine = engine

    def set_onchain_engine(self, engine: OnChainEngine):
        self.onchain_engine = engine
    ...
    def _initialize_feature_models(self, input_dim: int):
        ...
        """
        logger.info(
            f" [AI] Inicializando modelos de features com input_dim: {input_dim}."
        )
        # [REACTIVATED] Meta-Labeling filter (Lopez de Prado 2018, Ch.3)
        try:
            self.profitability_predictor = ProfitabilityPredictor(
                input_dim=input_dim,
                profit_threshold=getattr(
                    self.config_ai, "MIN_PROFIT_PROBABILITY", 0.55
                ),
                model_dir=self.config_ai.MODEL_DIR,
                config=self.config_ai,
                taker_fee=getattr(self.config_ai, "TAKER_FEE", None),
            )
            loaded = self.profitability_predictor.load_model()
            self.profitability_predictor = loaded
            if loaded.is_trained and loaded.last_metrics:
                m = loaded.last_metrics
                logger.info(
                    f" [MetaLabel] Carregado: auc={m.roc_auc:.3f}, "
                    f"n={m.n_samples}, pass_rate={m.passed_rate:.1%}, "
                    f"profit_passed={m.profit_rate_passed:.1%}"
                )
            else:
                logger.info(
                    f" [MetaLabel] Inicializado: min={loaded.min_samples} trades para treinar."
                )
        except Exception as e:
            logger.warning(
                f" [MetaLabel] Falha: {e}. Operando sem filtro de lucratividade."
            )
            self.profitability_predictor = None

    def _initialize_decision_agents(self, input_dim: int):
        logger.info(
            f" [AI] Inicializando agentes de deciso MoE com input_dim sugerido: {input_dim}."
        )
        ...
        logger.info(
            " [MoE] TrendSpecialist  apenas classe base - no ser treinado separadamente."
        )
        ...
        try:
            self.online_learner = create_online_learner(
                self.gating_network, self.config_ai
            )
        ...
        # [ARCH] hrl_master_ppo removido - roteamento via SoftGatingNetwork
        self.hrl_master = None
    ...
    def get_training_status(self) -> Dict[str, bool]:
        ...
        logger.info(
            f" [STATUS TREINO] {valid_count}/{total_count} modelos vlidos: {status}"
        )
        ...
        for name, details in self.metadata["models"].items():
            ...
            if os.path.exists(file_path) and os.path.getsize(file_path) > 1000:
                ...
                if not details.get("trained_on"):
                    ...
                    logger.debug(
                        f" {name}: data de treinamento atualizada para {file_dt.strftime('%Y-%m-%d')}"
                    )
    ...
    def _is_file_valid(self, file_path: str) -> bool:
        ...
        if file_path.endswith(".pth"):
            return (
                file_size > 50000
            )  # 50KB para modelos PyTorch (autoencoder pode ser pequeno)
    ...
    def _has_valid_checkpoint(self, model_name: str) -> bool:
        ...
        checkpoint_dir = os.path.join(
            os.path.dirname(self.config_ai.MODEL_DIR), "checkpoints_ai"
        )
        ...
        if (
            ...
            and os.path.getsize(best_model_path) > 1000000
        ):
            return True
        ...
        for file_name in os.listdir(specialist_dir):
            if (
                ...
                and os.path.getsize(os.path.join(specialist_dir, file_name)) > 1000000
            ):
                return True
    ...
    def _normalize_model_paths(self) -> None:
        overrides = {
            ...
            "autoencoder_model": "hybrid_autoencoder_v3.pt",
        }
        for key, expected in overrides.items():
            ...
            if current != expected:
                logger.debug(
                    "[META] Ajustando caminho do modelo %s: %s -> %s",
                    key,
                    current,
                    expected,
                )
    ...
    def _bootstrap_metadata_from_disk(self) -> None:
        ...
        try:
            ...
            for name, details in self.metadata["models"].items():
                ...
                if os.path.exists(model_path) and self._is_file_valid(model_path):
                    ...
                    if latest_ts is None or file_dt > latest_ts:
                        latest_ts = file_dt
                    logger.debug(
                        f" {name}: modelo principal vlido encontrado ({file_dt.strftime('%Y-%m-%d')})"
                    )
                else:
                    ...
                    if self._has_valid_checkpoint(name):
                        ...
                        if checkpoint_dir:
                            ...
                            if checkpoint_files:
                                ...
                                file_dt = datetime.fromtimestamp(
                                    os.path.getmtime(checkpoint_path), tz=timezone.utc
                                )
                                self.metadata["models"][name]["trained_on"] = (
                                    file_dt.isoformat()
                                )
                                valid_models_count += 1
                                ...
                                if latest_ts is None or file_dt > latest_ts:
                                    latest_ts = file_dt
                                logger.debug(
                                    f" {name}: checkpoint vlido encontrado ({file_dt.strftime('%Y-%m-%d')})"
                                )
                    else:
            ...
            self.metadata["last_full_training_date"] = (
            ...
            self._save_metadata()
            logger.info(
                f" Metadados reconstrudos: {valid_models_count} modelos vlidos encontrados. ltimo treinamento: {latest_ts.strftime('%Y-%m-%d') if latest_ts else 'N/A'}"
            )
    ...
    def _get_checkpoint_dir(self, model_name: str) -> Optional[str]:
        ...
        checkpoint_dir = os.path.join(
            os.path.dirname(self.config_ai.MODEL_DIR),
            "checkpoints_ai",
            checkpoint_mapping[model_name],
        )
    ...
    def _infer_feature_contract(self):
        ...
        # Tenta inferir contrato de features caso exista
        ae_features_path = os.path.join(
            self.config_ai.MODEL_DIR, "autoencoder_feature_columns.json"
        )
            ...
            try:
                with open(ae_features_path, "r") as f:
                    ...
                    if isinstance(cols, list) and cols:
                        ...
                        logger.debug(
                            f"Contrato de features inferido: {len(cols)} colunas AE (base_feature_columns)"
                        )
            except Exception as e:
        ...
        if os.path.exists(base_df_path) and not self.metadata.get(
            ...
            try:
                ...
                if numeric_cols:
                    ...
                    logger.debug(
                        f" Contrato de features inferido do dataset: {len(numeric_cols)} colunas"
                    )
            except Exception as e:
    ...
    def load_metadata(self):
        ...
        if os.path.exists(self.metadata_path):
            try:
                    ...
                    if self._validate_metadata_structure(loaded_meta):
                        ...
                        for model_key in self.metadata["models"]:
                            if model_key in loaded_meta.get("models", {}):
                                self.metadata["models"][model_key].update(
                                    loaded_meta["models"][model_key]
                                )

                        self.metadata["last_full_training_date"] = loaded_meta.get(
                            "last_full_training_date"
                        )
                        self.metadata["final_feature_count"] = loaded_meta.get(
                            "final_feature_count"
                        )
                        self.metadata["base_feature_columns"] = loaded_meta.get(
                            "base_feature_columns"
                        )
                        ...
                        if self.metadata["last_full_training_date"]:
                            try:
                                self.last_trained_date = datetime.fromisoformat(
                                    self.metadata["last_full_training_date"]
                                )
                            except (ValueError, TypeError):
                                logger.warning(
                                    " Data de treinamento invlida nos metadados. Ser recalculada."
                                )
                                self.last_trained_date = None
                        else:
                            self.last_trained_date = None

                        self.final_feature_count = self.metadata.get(
                            "final_feature_count"
                        )
                        self.base_feature_columns = self.metadata.get(
                            "base_feature_columns"
                        )
                        ...
                        logger.warning(
                            " Estrutura de metadados invlida. Reconstruindo..."
                        )
                ...
                logger.error(
                    f" Erro ao ler o arquivo de metadados: {e}. Reconstruindo metadados..."
                )
                self._bootstrap_metadata_from_disk()
                return
            except Exception as e:
                logger.error(
                    f" Erro inesperado ao carregar metadados: {e}. Reconstruindo..."
                )
                self._bootstrap_metadata_from_disk()
                return
        else:
            logger.warning(
                " Arquivo de metadados no encontrado. Inicializando estrutura padro."
            )
    ...
    def load_all_models(self):
        ...
        try:
            ...
            if (
                ...
                and self.feature_pipeline.temporal_autoencoder_pipeline
            ):
                if not self.feature_pipeline.temporal_autoencoder_pipeline.is_trained:
                    logger.info(
                        " [FAST_STARTUP] Carregando pesos do Autoencoder Temporal..."
                    )
                    try:
                        self.feature_pipeline.temporal_autoencoder_pipeline.load_state()
                    except Exception as ae_e:
                        logger.warning(
                            f" [FAST_STARTUP] Falha ao carregar Autoencoder: {ae_e}"
                        )

            autoencoder_trained = (
                self.feature_pipeline
                and self.feature_pipeline.temporal_autoencoder_pipeline
                and self.feature_pipeline.temporal_autoencoder_pipeline.is_trained
            )
            ...
            if not self.specialists or not self.hrl_master:
                ...
                logger.info(
                    f"⚡ [FAST_STARTUP] Inicializando agentes de decisão com input_dim={input_dim}..."
                )
            ...
            for spec_key, spec_name in zip(specialist_keys, specialist_names):
                ...
                if spec_name in self.specialists and os.path.exists(spec_path):
                    try:
                        ...
                        logger.info(
                            f" [FAST_STARTUP] Specialist {spec_name} carregado de {spec_path}"
                        )
                    except Exception as e:
                        logger.warning(
                            f" [FAST_STARTUP] Falha ao carregar {spec_name}: {e}",
                            exc_info=True,
                        )

            # --- ETAPA 3: Verificar status final ---
            hrl_master_trained = (
                self.hrl_master is not None and self.hrl_master.is_trained
            )
            ...
            for spec_key in specialist_keys:
                ...
                model_path = os.path.join(
                    self.config_ai.MODEL_DIR, model_meta.get("path", default_filename)
                )
                ...
                if os.path.exists(model_path):
                    ...
                    if not model_meta.get("trained", False):
                        logger.info(
                            f" [FAST_STARTUP] Modelo encontrado em disco: {default_filename}. Atualizando metadados..."
                        )
                        if "models" not in self.metadata:
                            self.metadata["models"] = {}
                        self.metadata["models"][spec_key] = {
                            "trained": True,
                    ...
                    logger.warning(
                        f" [FAST_STARTUP] Modelo no encontrado: {model_path}"
                    )
            ...
            if os.path.exists(hrl_path):
                if (
                    not self.metadata.get("models", {})
                    .get("hrl_master_ppo", {})
                    .get("trained", False)
                ):
                    logger.info(
                        f" [FAST_STARTUP] HRL Master encontrado em disco. Atualizando metadados..."
                    )
                    if "models" not in self.metadata:
                        self.metadata["models"] = {}
                    self.metadata["models"]["hrl_master_ppo"] = {
                        "trained": True,
                        "path": "hrl_master_ppo.zip",
                        "trained_on": datetime.utcnow().isoformat(),
                    }
                    self._save_metadata()
            ...
            if autoencoder_trained and specialists_trained:
                ...
                logger.info(
                    " [FAST_STARTUP] Sistema IA validado como TREINADO (baseado em arquivos e metadados)."
                )
            else:
                self.is_trained = False
                logger.warning(
                    f" [FAST_STARTUP] Sistema IA incompleto. AE={autoencoder_trained}, Specs={specialists_trained}, HRL(opcional)={hrl_master_trained}"
                )

            if self.is_trained:
                logger.info(
                    " Todos os modelos esto carregados. AIController est treinado e pronto para operar."
                )

                # [REGIMES] Carrega Detector de Regimes
                try:
                    regime_detector_path = os.path.join(
                        self.config_ai.MODEL_DIR, "crypto_regime_detector.pkl"
                    )
                    if os.path.exists(regime_detector_path):
                        ...
                        logger.info(
                            f" [REGIMES] CryptoRegimeDetector carregado de '{regime_detector_path}'."
                        )
                    else:
                        logger.warning(
                            f" [REGIMES] Arquivo do detector de regimes no encontrado: {regime_detector_path}"
                        )
                except Exception as e:
                    logger.error(
                        f" [REGIMES] Falha ao carregar CryptoRegimeDetector: {e}"
                    )
                ...
                if self.drift_detector and not self.drift_detector.reference_data:
                    try:
                        logger.info(
                            " [DRIFT] Carregando dados de referncia a partir do dataset base..."
                        )
                        base_df_path = os.path.join(
                            self.config_ai.MODEL_DIR, "base_featured_df.pkl"
                        )
                        if os.path.exists(base_df_path):
                            ...
                            if not features_to_monitor:
                                features_to_monitor = base_df.select_dtypes(
                                    include=["number"]
                                ).columns.tolist()
                                # [FIX] Persiste colunas inferidas para o contrato de features
                                self.base_feature_columns = features_to_monitor
                                logger.info(
                                    f" [FAST_STARTUP] 'base_feature_columns' inferido de base_featured_df ({len(self.base_feature_columns)} features)."
                                )

                            self.drift_detector.set_reference_data(
                                base_df, features_to_monitor
                            )
                        else:
                            logger.warning(
                                " [DRIFT] Dataset base no encontrado. Drift status permanecer DESCONHECIDO."
                            )
                    except Exception as e:
                        logger.error(
                            f" [DRIFT] Erro ao carregar dados de referncia: {e}"
                        )
            else:
                status = {
                ...
                logger.warning(
                    f" Falha no carregamento. Status dos componentes: {status}. AIController no est totalmente treinado."
                )
        except Exception as e:
            logger.critical(
                f" Erro crtico durante o carregamento dos modelos: {e}", exc_info=True
            )
            self.is_trained = False
    ...
    def is_retraining_due(self, retrain_days: int) -> bool:
        ...
        if latest_ts is None:
            logger.warning(
                " No foi possvel determinar a data do ltimo treinamento. Verificando arquivos..."
            )
            return self._check_files_for_retraining(retrain_days)

        days_since_training = (now - latest_ts).days
        logger.info(
            f" ltimo treinamento: {latest_ts.strftime('%Y-%m-%d')} ({days_since_training} dias atrs)"
        )
        ...
        if days_since_training > retrain_days and valid_models < 3:
            logger.warning(
                f" Modelos muito antigos ({days_since_training} dias) e poucos modelos vlidos ({valid_models}). Retreinamento recomendado."
            )
            return True
        elif days_since_training > retrain_days:
            logger.info(
                f" Modelos antigos ({days_since_training} dias), mas h {valid_models} modelos vlidos. Treinamento granular recomendado."
            )
            return False
    ...
    def _check_files_for_retraining(self, retrain_days: int) -> bool:
        ...
        logger.info(
            f" Verificao de arquivos: ltimo modelo modificado h {days_since} dias"
        )
    ...
    async def train_missing_components(self, status: Dict[str, bool]) -> bool:
        ...
        try:
            logger.info(
                "--- Iniciando Treinamento Granular dos Componentes Ausentes ---"
            )
            ...
            if base_df is None:
                logger.error(
                    " Falha ao obter o dataset base para o treinamento granular."
                )
                return False
            ...
            if not status.get("autoencoder_model"):
                # [CD1 FIX] Gate-aware retraining: até 3 tentativas para passar no gate
                # antes de prosseguir treinando SAC. Treinar SAC em latentes colapsados
                # (eRank<30%, AU<40%, KL<1e-4) ensina o agente em representacao lixo.
                _MAX_AE_ATTEMPTS = 3
                for attempt in range(1, _MAX_AE_ATTEMPTS + 1):
                    logger.info(
                        f" -> [Granular] Treinando Autoencoder (tentativa {attempt}/{_MAX_AE_ATTEMPTS})..."
                    )
                    self.feature_pipeline.temporal_autoencoder_pipeline.train_autoencoder_temporal(
                        numeric_df
                    )
                    if not self.feature_pipeline.temporal_autoencoder_pipeline.is_trained:
                        logger.warning(
                            f" [Granular] Tentativa {attempt}: treino do AE nao convergiu."
                        )
                        continue
                    _hp = self.feature_pipeline.temporal_autoencoder_pipeline.hparams
                    gate_status = _hp.get("gate_approved", True)
                    if gate_status:
                        logger.info(
                            f" [Granular] Autoencoder APROVADO no gate (tentativa {attempt})."
                        )
                        self._update_model_metadata("autoencoder_model", True)
                        autoencoder_ready = True
                        break
                    else:
                        _failures = _hp.get("gate_failures", [])
                        logger.warning(
                            f" [Granular] Tentativa {attempt}: GATE REPROVADO. Falhas: {_failures}. "
                            "Limpando estudo Optuna para nova exploracao..."
                        )
                        # Limpa o estudo Optuna para forcar exploracao de novos hiperparametros
                        try:
                            _ae_pipe = self.feature_pipeline.temporal_autoencoder_pipeline
                            _db_path = getattr(_ae_pipe, "optuna_db_path", None)
                            if _db_path and os.path.exists(_db_path):
                                os.remove(_db_path)
                                logger.info(f" [Granular] Optuna DB removido: {_db_path}")
                            # Reseta hparams para forcar novo HPO
                            _ae_pipe.hparams = {}
                            _ae_pipe.model = None
                        except Exception as _reset_err:
                            logger.warning(f" [Granular] Falha ao resetar AE state: {_reset_err}")

                if not autoencoder_ready:
                    logger.error(
                        f" [Granular] Autoencoder REPROVADO em {_MAX_AE_ATTEMPTS} tentativas. "
                        "Abortando treinamento para evitar SAC em latentes colapsados."
                    )
                    return False
            else:
                ...
                try:
                    ...
                    if not autoencoder_ready:
                        logger.warning(
                            " [Granular] Autoencoder marcado como treinado mas load_state() falhou."
                        )
                    ...
                    logger.error(
                        f" [Granular] Exceo ao carregar Autoencoder: {exc}",
                        exc_info=True,
                    )
                    logger.info(
                        " [Granular] Tentando retreinar autoencoder aps erro..."
                    )
                    try:
                        self.feature_pipeline.temporal_autoencoder_pipeline.train_autoencoder_temporal(
                            numeric_df
                        ...
                        logger.error(
                            f" [Granular] Falha no retreinamento do autoencoder: {retry_exc}",
                            exc_info=True,
                        )
                        autoencoder_ready = False
            ...
            if not autoencoder_ready:
                logger.error(
                    " Erro ao preparar Autoencoder para o treinamento granular."
                )
                return False

            # [CD2 FIX] Validacao cross-arquivo: latent_dim do AE vs SAC scalers.
            # Se HPO escolheu latent_dim diferente do que os SAC scalers esperam,
            # limpa scalers + checkpoints SAC desatualizados antes do treino RL.
            _mismatch_handled = self._enforce_feature_contract()
            if _mismatch_handled and status is not None:
                # Refaz o status apos invalidar especialistas
                for _spec_key in ("bull_specialist_sac", "bear_specialist_sac", "ranger_specialist_sac"):
                    _meta = self.metadata.get("models", {}).get(_spec_key, {})
                    status[_spec_key] = bool(_meta.get("trained", False))

            try:
                df_with_hidden_features = self.feature_pipeline.apply_hidden_features(
                    base_df
                )
            except Exception as exc:
                logger.error(
                    f"Falha ao aplicar hidden features no fluxo granular: {exc}"
                )
                return False
            ...
            try:
                if self.base_feature_columns:
                    available_cols = [
                        c
                        for c in self.base_feature_columns
                        if c in df_with_hidden_features.columns
                    ]
                    if len(available_cols) == len(self.base_feature_columns):
                        shap_bg = df_with_hidden_features[
                            self.base_feature_columns
                        ].tail(100)
                        shap_bg_path = os.path.join(
                            self.config_ai.MODEL_DIR, "shap_background.pkl"
                        )
                        shap_bg.to_pickle(shap_bg_path)
                        logger.info(
                            f" [SHAP] Background atualizado com dados reais para '{shap_bg_path}' ({len(shap_bg)} linhas, {len(self.base_feature_columns)} cols)"
                        )
            ...
            logger.info(
                " -> [Granular] TrendPredictor removido - usando ScientificRegimeDetector nos especialistas"
            )
            ...
            if needs_rl_training:
                logger.info(
                    "--- Iniciando Treinamento Granular dos Agentes de Deciso (RL) ---"
                )
                ...
                if enriched_training_df is None or enriched_training_df.empty:
                    logger.error(
                        " Falha ao construir o dataset enriquecido no treinamento granular."
                    )
                    return False

                # [FIX] Garantir colunas de regime no dataset granular
                if (
                    "regime" in base_df.columns
                    and "regime" not in enriched_training_df.columns
                ):
                    for col in ["regime", "regime_confidence", "regime_name"]:
                        if col in base_df.columns:
                            enriched_training_df[col] = base_df[col]
                    logger.info(
                        " [GRANULAR] Colunas de regime restauradas no dataset enriquecido."
                    )
                ...
                logger.info(
                    f" Objetivos de treinamento granular para agentes de RL definidos: {training_goals}"
                )
                ...
                for model_key, filename in specialist_file_mapping.items():
                    ...
                    if os.path.exists(model_path):
                        ...
                        logger.debug(
                            f" Modelo '{model_key}' detectado em disco - marcado como treinado."
                        )
                    else:
                        logger.debug(
                            f" Modelo '{model_key}' no encontrado em disco - no marcado como treinado."
                        )
            else:
                logger.info(
                    " [Granular] Todos os agentes de deciso (RL) j esto treinados."
                )
            ...
            if self.is_trained:
                logger.info(
                    " Treinamento granular concludo com sucesso. Todos os componentes esto prontos."
                )
                return True
            else:
                logger.error(
                    " Treinamento granular finalizado, mas a IA ainda no est em estado 'treinado'. Verifique os logs."
                )
                return False

        except Exception as e:
            logger.error(
                f" Erro catastrfico durante o treinamento granular: {e}", exc_info=True
            )
            return False
    ...
    def _update_model_metadata(self, model_key: str, trained_status: bool):
        ...
        if model_key in self.metadata["models"]:
            ...
            self.metadata["models"][model_key]["trained_on"] = (
                datetime.now(timezone.utc).isoformat() if trained_status else None
            )
            self._save_metadata()
        else:
            logger.warning(
                f"Tentativa de atualizar metadados para um modelo desconhecido: {model_key}"
            )

    def _validate_feature_contract(self) -> Dict[str, Any]:
        """
        [CD2 FIX] Verifica consistencia entre latent_dim do HybridAE e
        feature_names_in_ dos scalers dos especialistas SAC.

        Se o HPO escolher latent_dim diferente do que os SAC scalers esperam
        (e.g., AE produz 32 hidden_features mas scaler tem 25), o treino SAC
        falha silenciosamente: features extras sao ignoradas ou KeyError.

        Retorna dict com:
          - 'consistent': bool
          - 'ae_latent_dim': int (latent_dim atual do AE)
          - 'mismatched_scalers': list of paths (scalers a deletar p/ retrain)
        """
        result = {"consistent": True, "ae_latent_dim": None, "mismatched_scalers": []}
        try:
            if self.feature_pipeline is None or self.feature_pipeline.temporal_autoencoder_pipeline is None:
                logger.debug("[CONTRACT] Feature pipeline nao inicializada; skip validation.")
                return result
            _ae_pipe = self.feature_pipeline.temporal_autoencoder_pipeline
            _hp = getattr(_ae_pipe, "hparams", {}) or {}
            ae_latent = int(_hp.get("latent_dim", 0))
            if ae_latent <= 0:
                logger.debug("[CONTRACT] AE latent_dim desconhecido; skip validation.")
                return result
            result["ae_latent_dim"] = ae_latent

            import joblib as _joblib
            for spec_key in ("bull_specialist", "bear_specialist", "ranger_specialist"):
                _scaler_path = os.path.join(
                    self.config_ai.MODEL_DIR, f"{spec_key}_scaler.joblib"
                )
                if not os.path.exists(_scaler_path):
                    continue
                try:
                    _scaler = _joblib.load(_scaler_path)
                    _names = getattr(_scaler, "feature_names_in_", None)
                    if _names is None:
                        continue
                    _hidden_count = sum(1 for n in _names if str(n).startswith("hidden_feature_"))
                    if _hidden_count > 0 and _hidden_count != ae_latent:
                        logger.warning(
                            f"[CONTRACT] Mismatch em {spec_key}: scaler espera "
                            f"{_hidden_count} hidden_features, AE produz {ae_latent}. "
                            "Marcando para retreino."
                        )
                        result["mismatched_scalers"].append(_scaler_path)
                        result["consistent"] = False
                except Exception as _e:
                    logger.warning(f"[CONTRACT] Falha ao validar {_scaler_path}: {_e}")
            return result
        except Exception as _e:
            logger.warning(f"[CONTRACT] Validation falhou ({_e}); assumindo consistente.")
            return result

    def _enforce_feature_contract(self) -> bool:
        """
        [CD2 FIX] Aplica a validacao de contrato. Se mismatch:
          1. Deleta scalers + checkpoints SAC dos especialistas afetados
          2. Marca como `trained=False` no metadata
          3. Retorna True (mismatch detectado e tratado)
          4. Caller deve retreinar SAC do zero
        """
        report = self._validate_feature_contract()
        if report["consistent"]:
            if report["ae_latent_dim"] is not None:
                logger.info(
                    f"[CONTRACT] OK: AE latent_dim={report['ae_latent_dim']} consistente com SAC scalers."
                )
            return False
        # Mismatch: limpar scalers + checkpoints SAC
        _affected_specs = set()
        for _scaler_path in report["mismatched_scalers"]:
            try:
                os.remove(_scaler_path)
                logger.info(f"[CONTRACT] Removido scaler obsoleto: {_scaler_path}")
            except Exception as _e:
                logger.warning(f"[CONTRACT] Falha ao remover {_scaler_path}: {_e}")
            # Identifica especialista pelo nome
            for _name in ("bull_specialist", "bear_specialist", "ranger_specialist"):
                if _name in _scaler_path:
                    _affected_specs.add(_name)
                    break
        # Remove SAC checkpoint e marca como nao-treinado
        for _spec in _affected_specs:
            _ckpt = os.path.join(self.config_ai.MODEL_DIR, f"{_spec}_sac.zip")
            if os.path.exists(_ckpt):
                try:
                    os.remove(_ckpt)
                    logger.info(f"[CONTRACT] Removido SAC checkpoint: {_ckpt}")
                except Exception as _e:
                    logger.warning(f"[CONTRACT] Falha ao remover {_ckpt}: {_e}")
            _meta_key = f"{_spec}_sac"
            if _meta_key in self.metadata.get("models", {}):
                self._update_model_metadata(_meta_key, False)
        logger.warning(
            f"[CONTRACT] {len(_affected_specs)} especialista(s) marcados para retreino "
            f"devido a mismatch de latent_dim={report['ae_latent_dim']}."
        )
        return True

    async def _get_or_create_enriched_dataset(self) -> Optional[pd.DataFrame]:
        enriched_df_path = os.path.join(
            self.config_ai.MODEL_DIR, "enriched_featured_df.pkl"
        )
        if os.path.exists(enriched_df_path):
            logger.info(
                f" [CACHE] Cache do dataset enriquecido final encontrado. Carregando de '{enriched_df_path}'..."
            )
            try:
                return pd.read_pickle(enriched_df_path)
            except Exception as e:
                logger.error(
                    f" [CACHE] Falha ao carregar o cache do dataset enriquecido: {e}. Retornando None."
                )
                return None
        else:
            logger.warning(
                " [CACHE] Dataset enriquecido no encontrado. Um ciclo de treinamento  necessrio para cri-lo."
            )
            return None
    ...
    def needs_foundation_training(self, retrain_days: int) -> bool:
        if not os.path.exists(self.metadata_path):
            logger.warning(
                " Arquivo de metadados no encontrado. Treinamento de fundao necessrio."
            )
            return True
        try:
            with open(self.metadata_path, "r") as f:
            ...
            if not last_trained_str:
                logger.warning(
                    " Data do ltimo treino no encontrada nos metadados. Treinamento de fundao necessrio."
                )
                return True
            ...
            if (datetime.now(timezone.utc) - last_trained_date).days > retrain_days:
                logger.warning(
                    f" Modelos de fundao desatualizados (treinados h mais de {retrain_days} dias). Retreinamento necessrio."
                )
                return True
            status = self.get_training_status()
            if not status["all_trained"]:
                logger.warning(
                    f" Arquivos de modelo essenciais esto faltando. Treinamento de fundao necessrio."
                )
                return True
        ...
        try:
            ...
            if "regime_confidence" in df.columns and "regime_val" in df.columns:
                ...
                if conf_std > 0.01:  # Valores dinmicos j existem
                    logger.debug(
                        "[AI] Colunas de regime j presentes com valores dinmicos."
                    )
                    return df
            ...
            if not hasattr(self, "regime_detector") or self.regime_detector is None:
                ...
                if os.path.exists(model_path):
                    ...
                    logger.info(
                        " [CACHE] CryptoRegimeDetector carregado e cacheado em memria."
                    )
                else:
                    logger.warning(
                        f"  [AI] CryptoRegimeDetector no encontrado em: {model_path}"
                    )
                    self.regime_detector = None
            ...
            if self.regime_detector and self.regime_detector.is_trained:
                ...
                df["regime_confidence"] = (
                    regime_df["confidence"].reindex(df.index).fillna(0.5)
                )
                df["regime_val"] = (
                    regime_df["regime"].reindex(df.index).fillna(2).astype(int)
                )
                ...
                if self.regime_detector:
                    logger.warning(
                        "  [AI] CryptoRegimeDetector cacheado mas no treinado!"
                    )
    ...
    def _enrich_df_with_trend_predictions(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        has_regime_info = (
            "regime_confidence" in df_enriched.columns
            and "regime_val" in df_enriched.columns
        )
        ...
        if has_regime_info:
            ...
            if conf_std < 0.01:
                ...
                logger.info(
                    f" [AI] Mapping CryptoRegimeDetector outputs to Trend priors (mean_conf={conf_mean:.4f}, std={conf_std:.4f})."
                )
            ...
            df_enriched["tp_regime_down"] = (df_enriched["regime_val"] == 1).astype(
                float
            )
            df_enriched["tp_regime_sideways"] = (df_enriched["regime_val"] >= 2).astype(
                float
            )
            ...
            defaults = {"tp_uncertainty": 0.5, "tp_duration_median": 10.0}
    ...
    def report_regime_validation(self, df: pd.DataFrame) -> None:
        ...
        try:
            if df is None or df.empty:
                logger.warning(
                    " [REGIME REPORT] DataFrame vazio. Relatrio por regime ignorado."
                )
                return
            ...
            if (
                "high_volatility" not in df_local.columns
                or "low_volatility" not in df_local.columns
            ):
                # Tolerncia: tenta inferir atr_percentage se no existir
                if not has_atr_pct:
                    tf = str(
                        getattr(self.config_trading, "PRIMARY_TIMEFRAME_TRADING", "1h")
                    ).lower()
                    ...
                    if atr_pct_tf_col in df_local.columns:
                        ...
                        df_local["atr_percentage"] = (
                            df_local[atr_tf_col] / (df_local["close"] + 1e-9)
                        ) * 100.0
                        has_atr_pct = True
                if not has_atr_pct:
                    logger.warning(
                        " [REGIME REPORT] 'atr_percentage' ausente. No  possvel derivar volatilidade."
                    )
                    return
                atr_ma = df_local["atr_percentage"].rolling(20, min_periods=20).mean()
                df_local["high_volatility"] = (
                    df_local["atr_percentage"] > (atr_ma * 1.5)
                ) & (atr_ma > 1e-9)
                df_local["low_volatility"] = (
                    df_local["atr_percentage"] < (atr_ma * 0.7)
                ) & (atr_ma > 1e-9)

            # ADX: usa flags existentes se disponveis; caso contrrio, deriva
            if (
                "adx_strong_trend" not in df_local.columns
                or "adx_weak_trend" not in df_local.columns
            ):
                if not has_adx:
                    ...
                    if adx_tf_col in df_local.columns:
                        ...
                        if all(
                            col in df_local.columns for col in ["high", "low", "close"]
                        ):
                            try:
                                adx_series, _, _ = NativeIndicators.adx(
                                    df_local["high"],
                                    df_local["low"],
                                    df_local["close"],
                                    length=14,
                                ...
                                has_adx = True
                                logger.info(
                                    " [REGIME REPORT] ADX recalculado on-the-fly para o relatrio de regime."
                                )
                            except Exception as _e:
                                logger.warning(
                                    f" [REGIME REPORT] Falha ao recalcular ADX: {_e}"
                                )
                if not has_adx:
                    logger.warning(
                        " [REGIME REPORT] 'adx' ausente. No  possvel derivar fora de tendncia."
                    )
                    return
            ...
            if "close" not in df_local.columns:
                logger.warning(
                    " [REGIME REPORT] Coluna 'close' ausente. No  possvel calcular retornos."
                )
                return
            ...
            def sharpe(series: pd.Series) -> float:
                ...
                if series.empty or series.std() == 0:
                    return float("nan")
                return float(
                    series.mean() / (series.std() + 1e-12) * np.sqrt(bars_per_year)
                )
            ...
            buckets = [
                (
                    "VOL=ALTA  ADX=ALTO",
                    (df_local["high_volatility"] & df_local["adx_strong_trend"]),
                ),
                (
                    "VOL=ALTA  ADX=BAIXO",
                    (df_local["high_volatility"] & df_local["adx_weak_trend"]),
                ),
                (
                    "VOL=BAIXA  ADX=ALTO",
                    (df_local["low_volatility"] & df_local["adx_strong_trend"]),
                ),
                (
                    "VOL=BAIXA  ADX=BAIXO",
                    (df_local["low_volatility"] & df_local["adx_weak_trend"]),
                ),
            ]
    ...
    async def _train_rl_agents(
        ...
        sample_features = featured_df.select_dtypes(
            include=[np.number]
        ).columns.tolist()
        ...
        relevant_features = processor.filter_autoencoder_features(
            sample_features, featured_df
        )
        ...
        if self.curriculum_engine:
            ...
            logger.info(
                f" [CURRCULO] Estgio ativo: '{stage_name}' "
            ...
            logger.info(
                f" [CURRCULO] Dataset filtrado: {len(featured_df)} amostras "
                f"(estgio '{stage_name}')"
            )
        #
        ...
        EMBARGO_BARS = 50
        if len(featured_df) > 2 * EMBARGO_BARS:
            split_idx = int(len(featured_df) * 0.8)
            train_idx_end = split_idx - EMBARGO_BARS
            eval_idx_start = split_idx
            if train_idx_end > EMBARGO_BARS:
                train_df = featured_df.iloc[:train_idx_end]
                eval_df = featured_df.iloc[eval_idx_start:]
                logger.info(
                    f"[EMBARGO] Split treino/eval com gap de {EMBARGO_BARS} barras: "
                    f"treino [0:{train_idx_end}], eval [{eval_idx_start}:{len(featured_df)}]"
                )
            else:
                train_df, eval_df = train_test_split(
                    featured_df, test_size=0.2, shuffle=False
                )
        else:
            train_df, eval_df = train_test_split(
                featured_df, test_size=0.2, shuffle=False
        ...
        logger.info(
            "--- Etapa 4.1: Verificando e treinando especialistas individuais ---"
        )
        ...
        for spec_name in specialist_order:
            ...
            if (
                spec
                and hasattr(spec, "optuna_db_path")
                and os.path.exists(spec.optuna_db_path)
            ):
                try:
                    import optuna

                    summaries = optuna.study.get_all_study_summaries(
                        storage=f"sqlite:///{spec.optuna_db_path}"
                    )
                    specialist_progress[spec_name] = (
                        summaries[0].n_trials if summaries else 0
                    )
                except:
        ...
        logger.info(
            f" [ORDEM] Processando specialists por progresso: {[(s, specialist_progress.get(s, 0)) for s in specialist_order_sorted]}"
        )
        ...
        for spec_class_name in specialist_order_sorted:
            ...
            if hasattr(specialist, "is_optimized") and specialist.is_optimized:
                logger.info(
                    f" [OPTUNA] Especialista '{spec_class_name}' j otimizado (via is_optimized). Pulando."
                )
            ...
            if not skip_optimization and status:
                ...
                if status_key and status.get(status_key):
                    logger.info(
                        f" [OPTUNA] Especialista '{spec_class_name}' j marcado como treinado no status. Pulando otimizao."
                    )
                    skip_optimization = True
                ...
                try:
                    ...
                    if os.path.exists(db_path):
                        ...
                        for summary in study_summaries:
                            ...
                            if (
                                summary.n_trials >= 50
                            ):  # Alvo reduzido para 50 trials
                                logger.info(
                                    f" [OPTUNA] Especialista '{spec_class_name}' tem {summary.n_trials} trials no DB. Pulando otimizao."
                                )
                                skip_optimization = True
                                ...
                                if study.best_trial:
                                    ...
                                    logger.info(
                                        f" [OPTUNA] Melhores hiperparmetros carregados do estudo '{summary.study_name}'."
                                    )
                                break
                            else:
                                # Tem trials mas no completou - FOCA neste specialist
                                logger.info(
                                    f" [OPTUNA] Especialista '{spec_class_name}' tem {summary.n_trials}/100 trials. Continuando otimizao..."
                                )
                except Exception as e:
                    logger.warning(
                        f" [OPTUNA] Erro ao verificar DB Optuna para '{spec_class_name}': {e}"
                    )
            ...
            if (
                ...
                and not os.path.exists(getattr(specialist, "optuna_db_path", ""))
                and hasattr(specialist, "model_path")
            ):
                if os.path.exists(specialist.model_path):
                    ...
                    if file_size > 1000000:  # >1MB = modelo vlido
                        logger.info(
                            f" [OPTUNA] Modelo '{specialist.model_path}' existe ({file_size / 1e6:.1f}MB). Pulando otimizao."
                        )
                        skip_optimization = True

            if not skip_optimization:
                logger.info(
                    f"[OPTUNA] Especialista '{spec_class_name}' no otimizado. Executando otimizao Optuna..."
                )
                try:
                    ...
                    train_opt, eval_opt = train_test_split(
                        train_df, test_size=0.2, shuffle=False
                    )
                    specialist.optimize_hyperparameters(
                        train_opt, eval_opt, n_trials=50, n_timesteps=100000
                    )  #  HPO Cientfico (50 trials, 100k steps)
                    logger.info(f"[OPTUNA] Otimizao do '{spec_class_name}' concluda.")
                except Exception as opt_err:
                    logger.error(
                        f"[OPTUNA] Erro na otimizao do '{spec_class_name}': {opt_err}. Continuando com hiperparmetros padro."
                    )
                ...
                logger.info(
                    f" [OPTUNA] Focando em '{spec_class_name}' at completar otimizao. Outros specialists sero processados na prxima execuo."
                )
            ...
            if spec_class_name == "TrendSpecialist":
                logger.info(
                    f" Pulando treinamento de '{spec_class_name}' ( apenas classe base)."
                )
                continue
            ...
            completed_steps = training_progress.get(spec_class_name, {}).get(
            ...
            if not model_save_path:
                ...
                spec_name_attr = getattr(
                    specialist, "specialist_name", spec_class_name.lower()
                )
                model_save_path = os.path.join(
                    self.config_ai.MODEL_DIR, f"{spec_name_attr}_sac.zip"
                )

            # [FIX] Caminho dinmico para checkpoints usando specialist_name
            spec_name_for_folder = getattr(
                specialist, "specialist_name", spec_class_name.lower()
            )
            spec_folder_name = f"{spec_name_for_folder}_checkpoints"
            checkpoint_dir = os.path.join(
                self.config_ai.CHECKPOINT_DIR, spec_folder_name
            )
            ...
            if os.path.exists(model_save_path):
                ...
                logger.info(
                    f" [MODELO] Modelo FINAL encontrado para '{spec_class_name}'. Status: Completo."
                )
            elif os.path.exists(best_model_path):
                # Checkpoint existe - logar que vai resumir
                logger.info(
                    f" [RESUME] Best Model encontrado em '{best_model_path}'. O treinamento continuar deste ponto."
                )
                ...
                if model_steps > completed_steps:
                    ...
                    logger.info(
                        f" [TIMESTEPS] Modelo '{spec_class_name}' j tem {model_steps:,} steps treinados."
                    )
            ...
            if remaining_steps > 0:
                logger.info(
                    f" Especialista '{spec_class_name}' precisa treinar por mais {remaining_steps:,} timesteps (atual: {completed_steps:,}, meta: {total_target_steps:,})."
                )
                ...
                if hasattr(specialist, "train_model"):
                    logger.info(
                        f" Usando train_model() com filtragem por regime para '{spec_class_name}'"
                    )
                    result = specialist.train_model(
                        train_df, total_timesteps=remaining_steps
                    )
                    new_total_steps = (
                        result.get("timesteps", remaining_steps)
                        if isinstance(result, dict)
                        else remaining_steps
                    )
                else:
                    # Fallback para train() genrico
                    new_total_steps = specialist.train(
                        train_df, timesteps_to_train=remaining_steps
                    )

                if spec_class_name not in training_progress:
                    training_progress[spec_class_name] = {}
                training_progress[spec_class_name]["completed_timesteps"] = (
                    completed_steps + new_total_steps
                )
                ...
                if self.curriculum_engine:
                    try:
                        ...
                        if hasattr(specialist, "evaluate"):
                            ...
                            if _close_col and len(eval_df) > 20:
                                ...
                                if len(_rets) > 10:
                                    ...
                                    _ann = np.sqrt(252 * 96)
                                    _sr = float(
                                        (_rets.mean() / (_rets.std() + 1e-8)) * _ann
                                    )
                        ...
                        logger.debug(
                            f" [CURRCULO] Mtricas no disponveis para update_progress: {_ce}"
                        )
                #

                logger.info(
                    f" [PROGRESSO] '{spec_class_name}' treinado com sucesso. Prosseguindo para o prximo componente..."
                )
            else:
                logger.info(
                    f" Especialista '{spec_class_name}' j completou seu treinamento ({completed_steps:,} >= {total_target_steps:,}). Pulando."
                )
    ...
    async def _train_meta_labeler_from_holdout(
        self,
        ...
        """
        [PASSO 6] Treina o ProfitabilityPredictor no holdout temporal.

        Separa os últimos `holdout_pct` do dataset (nunca vistos pelos especialistas),
        roda o pipeline de decisão sobre eles como um mini-backtest para coletar
        trade_log com outcomes reais, e treina o meta-labeler.

        Lopez de Prado (2018) AFLM Ch.4: o meta-labeler é uma segunda camada
        que filtra os sinais do modelo primário, treinada em dados out-of-sample.
        """
        if self.profitability_predictor is None:
            logger.warning(
                "[MetaLabel] ProfitabilityPredictor não inicializado. Pulando Passo 6."
            )
            return False
        ...
        logger.info("─" * 60)
        logger.info(
            "🏷️  [PASSO 6] Meta-Labeling: Holdout Backtest + ProfitabilityPredictor"
        )
        logger.info("─" * 60)

        n = len(enriched_df)
        split_idx = int(n * (1.0 - holdout_pct))
        holdout_df = enriched_df.iloc[split_idx:].copy()

        if len(holdout_df) < 100:
            logger.warning(
                f"[MetaLabel] Holdout insuficiente ({len(holdout_df)} barras). "
                f"Pulando. Necessário ≥ 100."
            )
            return False

        logger.info(
            f"[MetaLabel] Holdout: {holdout_df.index[0].date()} → {holdout_df.index[-1].date()} "
            f"({len(holdout_df)} barras = {holdout_pct:.0%} do dataset)"
        )

        # Mini-backtest no holdout: gera trade_log com PnL real
        trade_log: List[Dict[str, Any]] = []
        open_position: Optional[Dict] = None

        iterator = range(1, len(holdout_df))
        if _tqdm:
            iterator = _tqdm(
                iterator,
                desc="  ↳ Holdout backtest (meta-label)",
                unit="bar",
                leave=False,
            )
        ...
        for i in iterator:
            ...
            prev_row = holdout_df.iloc[i - 1]
            price = float(row.get("close", 0))
            if price <= 0:
                continue

            # Gera sinal com o pipeline de decisão atual
            try:
                window = holdout_df.iloc[max(0, i - 299) : i + 1]
                signal = await self.generate_trading_decision(window)
            except Exception:
                signal = None

            action_str = getattr(signal, "action", None)
            action_val = (
                getattr(action_str, "value", str(action_str)) if action_str else "HOLD"
            )

            # Fecha posição aberta se sinal contrário ou HOLD
            if open_position is not None:
                should_close = (
                    action_val == "HOLD"
                    or (
                        open_position["side"] == "BUY"
                        and action_val in ("SELL", "SHORT")
                    )
                    or (
                        open_position["side"] == "SELL"
                        and action_val in ("BUY", "LONG")
                    )
                )
                if should_close:
                    entry_price = open_position["entry_price"]
                    qty = open_position["quantity"]
                    notional = open_position["notional_value"]
                    taker_fee = getattr(self.config_ai, "TAKER_FEE", 0.0004)
                    fee_total = notional * taker_fee * 2  # entry + exit

                    if open_position["side"] == "BUY":
                        pnl = (price - entry_price) * qty - fee_total
                    else:
                        pnl = (entry_price - price) * qty - fee_total

                    trade = open_position.copy()
            ...
            # Abre nova posição se sinal claro
            if (
                open_position is None
                and signal is not None
                and action_val not in ("HOLD", "CLOSE", "FLAT")
            ):
                pos_size_pct = getattr(signal, "position_size_pct", 0.02)
                notional = float(self.config_ai.INITIAL_CAPITAL) * pos_size_pct
                ...
                if qty > 0:
                    open_position = {
                        ...
                        "entry_time": row.name,
                        "timestamp": row.name,
                        "side": "BUY" if action_val in ("BUY", "LONG") else "SELL",
                        "entry_price": price,
                        "quantity": qty,
                        "notional_value": notional,
                        "profit_probability": getattr(signal, "confidence", 0.5),
                        "exit_time": None,
                        "exit_price": None,
                        "pnl": 0.0,
                    }

        # Fecha posição ainda aberta no final do holdout
        if open_position is not None and len(holdout_df) > 0:
            last_price = float(
                holdout_df.iloc[-1].get("close", open_position["entry_price"])
            )
            entry_price = open_position["entry_price"]
            qty = open_position["quantity"]
            notional = open_position["notional_value"]
            taker_fee = getattr(self.config_ai, "TAKER_FEE", 0.0004)
            fee_total = notional * taker_fee * 2
            pnl = (
                (last_price - entry_price) * qty - fee_total
                if open_position["side"] == "BUY"
                else (entry_price - last_price) * qty - fee_total
            )
            open_position["exit_time"] = holdout_df.index[-1]
            open_position["exit_price"] = last_price
            open_position["pnl"] = pnl
            trade_log.append(open_position)

        closed = [t for t in trade_log if t.get("pnl") != 0.0]
        logger.info(
            f"[MetaLabel] Holdout completado: {len(closed)} trades fechados "
            f"| WR bruta: {sum(1 for t in closed if t['pnl'] > 0) / max(len(closed), 1):.1%}"
        )

        if len(closed) < min_trades_required:
            logger.warning(
                f"[MetaLabel] {len(closed)} trades < mínimo {min_trades_required}. "
                f"Meta-labeler não treinado. Aumente holdout_pct ou rode mais dados."
            )
            return False

        # Treina o meta-labeler com o trade_log do holdout
        try:
            from tqdm import tqdm as _tqdm_train

            steps_done = [0]
            pbar = _tqdm_train(
                total=6, desc="  ↳ MetaLabel training", unit="step", leave=False
            )

            def _cb(step, total):
                delta = step - steps_done[0]
                if delta > 0:
                    pbar.update(delta)
                    steps_done[0] = step

            metrics = self.profitability_predictor.train_from_trade_log(
        ...
        if metrics is not None:
            logger.info(
                f"✅ [PASSO 6] ProfitabilityPredictor treinado: "
                f"AUC={metrics.roc_auc:.3f} | threshold={metrics.profit_threshold:.3f} "
                f"| improvement={metrics.improvement_vs_baseline:+.1%}"
            )
            self._update_model_metadata("profitability_predictor", True)
            return True
        else:
            logger.warning(
                "[MetaLabel] Treino do meta-labeler retornou None. Verifique os dados."
            )
            return False
    ...
    async def _bootstrap_gating_network(self, enriched_df: pd.DataFrame) -> bool:
        ...
        try:
            ...
            latent_cols = [
                c for c in enriched_df.columns if c.startswith("hidden_feature_")
            ]
            if not latent_cols:
                logger.error(
                    " Erro: Hidden Features (Latent) no encontradas. Verifique o contrato de features."
                )
                return False

            # [MD3 FIX] Anti-false-signal: drop linhas com latent NaN em vez de fillna(0).
            # Latents do HybridAE no warmup (primeiras seq_len barras) podem vir NaN.
            # fillna(0) treinaria o gating em embeddings sinteticos -> regime errado.
            _df_clean = enriched_df[latent_cols + ["regime_val"]].dropna()
            if len(_df_clean) < 100:
                logger.error(
                    f" [GATING] Apos drop de NaN restam apenas {len(_df_clean)} linhas. "
                    "Verifique o warmup do HybridAE."
                )
                return False
            X = _df_clean[latent_cols].values
            y = _df_clean["regime_val"].fillna(2).astype(int).values

            y = np.clip(y, 0, 2)

            # [ANTI-OVERFITTING] Chronological 80/20 split for validation
            _split = int(len(X) * 0.80)
            if _split > 50 and _split < len(X) - 10:
            ...
            try:
                ...
                if "CUDA error" in str(cuda_err):
                    logger.warning(
                        f" [GATING] Falha CUDA ({cuda_err}). Migrando para CPU..."
                    )
            ...
            if self.gating_network is None or self.gating_network.latent_dim != len(
                latent_cols
            ):
                latent_dim = len(latent_cols)
                logger.info(
                    f" [GATING] Ajustando SoftGatingNetwork para nova dimenso latente: {latent_dim}"
                )
                self.gating_network = create_gating_network(
                    self.config_ai, latent_dim=latent_dim
                )
            ...
            # 3. Loop de Treino (10 epochs) com validacao no conjunto out-of-sample
            epochs = 10
            best_val_acc = 0.0
            for epoch in range(epochs):
                ...
                # [MoE FIX] lb_loss calculado UMA vez por epoch (não por batch).
                # Estatísticas acumuladas no loop acima. Reset=True aqui.
                lb_loss = self.gating_network.compute_load_balancing_loss(reset=True)
                if lb_loss.item() > 0:
                    optimizer.zero_grad()
                    (0.01 * lb_loss).backward()
                    optimizer.step()

                val_str = ""
                if X_val is not None and epoch % 2 == 1:
                    self.gating_network.eval()
                    with torch.no_grad():
                        X_val_t = torch.FloatTensor(X_val).to(device)
                        y_val_t = torch.LongTensor(y_val).to(device)
                        val_preds = self.gating_network(X_val_t)["logits"].argmax(
                            dim=-1
                        )
                        val_acc = (val_preds == y_val_t).float().mean().item()
                        best_val_acc = max(best_val_acc, val_acc)
                        val_str = f" | Val Acc: {val_acc:.3f}"
                    self.gating_network.train()

                if epoch % 2 == 0:
                    logger.debug(
                        f" -> [GATING BOOTSTRAP] Epoch {epoch + 1}/{epochs}, Loss: {total_loss / max(1, len(train_loader)):.4f}{val_str}"
                    )

            # 4. Salvamento
            # Use .pt to match _initialize_decision_agents logic
            gating_path = os.path.join(
                self.config_ai.MODEL_DIR, "soft_gating_network.pt"
            )
            self.gating_network.save(gating_path)

            # 5. Inicializao do Router
            self.moe_router = MixtureOfExpertsRouter(
                gating_network=self.gating_network, specialists=self.specialists
            )

            logger.info(
                f" Bootstrap da SoftGatingNetwork concludo. Salvo em '{gating_path}'."
            )
            return True
    ...
    def _evaluate_trend_specialist_holdout(
        self, specialist: TrendSpecialist, full_df: pd.DataFrame
    ) -> None:
        df = ensure_datetime_index(full_df)
        if df.empty:
            logger.warning(
                "[TrendSpecialist] Dataset vazio. Backtest automtico ignorado."
            )
            return

        _, holdout_df = split_train_holdout(
            df, holdout_months=2, min_holdout_rows=DEFAULT_EVAL_MIN_ROWS
        )
        holdout_label = "two_month_holdout"
        if holdout_df.empty:
            holdout_df = build_unseen_eval_frame(
                df, train_cutoff_date=None, eval_months=3
            )
            holdout_label = "unseen_backtest"
        if holdout_df.empty:
            holdout_df = build_recent_eval_frame(
                df, months=DEFAULT_EVAL_MONTHS, min_rows=DEFAULT_EVAL_MIN_ROWS
            )
            holdout_label = "recent_backtest"
        if holdout_df.empty:
            logger.warning(
                "[TrendSpecialist] Nenhum conjunto vlido de dados no vistos para backtest automtico."
            )
            return

        log_dir = Path(os.getcwd()) / "logs" / "trade_analysis" / "SAC"
        summary = run_manual_backtest(
            specialist, holdout_df, log_dir=log_dir, label=holdout_label, logger=logger
        )
        if summary:
            logger.info(
    ...
    def _augment_with_additional_timeframes(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for file in folder.glob(f"{symbol}_*_historical.pkl"):
            ...
            try:
                ...
                logger.warning(
                    "  [DATA] Falha ao reamostrar %s para %s: %s",
                    tf_part,
                    primary_tf,
                    exc,
                )
        ...
        if added_columns:
            logger.info(
                "  [DATA] Colunas multi-timeframe adicionadas: %d (%s)",
                added_columns,
                ", ".join(sorted(added_suffixes)),
            )
    ...
    async def _get_or_create_base_dataset(self) -> Optional[pd.DataFrame]:
        ...
        if not os.path.exists(base_df_path) and os.path.exists(backup_path):
            try:
                ...
                logger.info(
                    f" [CACHE RESTORE] Arquivo restaurado de backup: '{backup_path}'"
                )
            except Exception as restore_err:
                logger.warning(
                    f" [CACHE RESTORE] Falha ao restaurar backup: {restore_err}"
                )
        ...
        if os.path.exists(base_df_path):
            try:
                ...
                if file_size < 1_000_000 and os.path.exists(
                    backup_path
                ):  # < 1MB is suspicious
                    backup_size = os.path.getsize(backup_path)
                    if (
                        backup_size > file_size * 10
                    ):  # Backup much larger = likely correct
                        logger.warning(
                            f" [CACHE] Arquivo muito pequeno ({file_size / 1024:.0f}KB). Restaurando backup ({backup_size / 1024 / 1024:.1f}MB)..."
                        )
                        import shutil
                ...
                logger.info(
                    f"[CACHE] Cache do dataset base encontrado. Carregando de '{base_df_path}'..."
                )
                ...
                if cached_df is not None and not cached_df.empty:
                    ...
                    if "regime" not in cached_df.columns:
                        logger.warning(
                            " [CACHE] Coluna 'regime' ausente. Injetando labels via ScientificDataProcessor..."
                        )
                        try:
                            from feature_engineering.scientific_data_processor import (
                            ...
                            logger.info(
                                f" [CACHE] Cache atualizado com colunas de regime."
                            )
                        except Exception as e:
                            logger.error(f" [CACHE] Falha ao injetar regimes: {e}")

                    enriched_cached = self._augment_with_additional_timeframes(
                        cached_df
                    )
                    if (
                        enriched_cached is not None
                        and not enriched_cached.empty
                        and enriched_cached is not cached_df
                    ):
                        try:
                            enriched_cached.to_pickle(base_df_path)
                        except Exception as save_exc:
                            logger.warning(
                                "[CACHE] Falha ao atualizar cache enriquecido: %s",
                                save_exc,
                            )
                ...
                logger.warning(
                    f" [CACHE] cache corrompido ou erro de leitura: {e}. Gerando novamente."
                )

        logger.info(
            "[CACHE] Cache do dataset base no encontrado. Gerando a partir de dados histricos..."
        )
        if not self.data_provider or not self.feature_pipeline:
            logger.error(
                "[CACHE] DataProvider/FeaturePipeline no inicializados. Impossvel criar dataset."
            )
            return None

        historical_data = await self.data_provider.get_data_for_training(
            symbol=self.config_trading.PRIMARY_PAIR,
            start_date=(datetime.now(timezone.utc) - timedelta(days=3 * 365)).strftime(
                "%Y-%m-%d"
            ),
        ...
        if not historical_data:
            logger.error(
                "[CACHE] Falha ao obter dados histricos para criar o dataset base."
            )
            return None
        ...
        if featured_df is not None and not featured_df.empty:
            ...
            try:
                ...
                logger.info(
                    f"[CACHE] Dataset base com features salvo em '{base_df_path}'."
                )
            except Exception as e:
                logger.error(
                    f"[CACHE] Falha ao salvar o cache do dataset base: {e}",
                    exc_info=True,
                )

        return featured_df

    def _optimize_trend_predictor(
        self,
        ...
        overlapping = [
            ...
            if c in features_df.columns and c != "primary_signal"
        ]
        feature_only = features_df.drop(columns=overlapping, errors="ignore")
        combined_df = feature_only.join(labels_df, how="inner")
        if combined_df is None or combined_df.empty:
            raise ValueError(
                "Combined dataset for TrendPredictor Optuna is empty after join"
            )

        logger.info(
            "%s Otimizando TrendPredictor via Optuna (%d trials)...", context, n_trials
        )
        self.trend_predictor.optimize_from_combined_dataset(
            combined_df, n_trials=n_trials
        )
    ...
    async def train_foundation_models(self) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            if base_df is None or base_df.empty:
                logger.error(
                    " Falha ao obter o dataset base. Abortando treinamento de fundao."
                )
                return None
            ...
            if "regime" in base_df.columns:
                ...
                for col in cols_to_restore:
                    if (
                        ...
                        and col not in df_with_hidden_features.columns
                    ):
                        df_with_hidden_features[col] = base_df[col]
                logger.info(
                    f" [DATA] Colunas de regime restauradas no dataset enriquecido."
                )
            ...
            enriched_training_df = await self.build_enriched_dataset(
                df_with_hidden_features
            )
            if enriched_training_df is None or enriched_training_df.empty:
                logger.error(
                    " Falha ao construir o dataset enriquecido final. Abortando treinamento."
                )
                return None
            ...
            training_goals = {
                "BullSpecialist": _trend_g,
                "BearSpecialist": _trend_g,
                "RangerSpecialist": _base_g,
            }
            logger.info(
                f" Objetivos de treinamento definidos para esta sesso: {training_goals}"
            )
            ...
            # ── PASSO 6: Meta-Labeling — Holdout Backtest + ProfitabilityPredictor ──
            # Lopez de Prado (2018) AFLM Ch.4: o meta-labeler PRECISA dos sinais
            # do modelo primário com outcomes reais. Usa os últimos 20% dos dados
            # como holdout temporal (nunca vistos pelos especialistas).
            await self._train_meta_labeler_from_holdout(enriched_training_df)
            ...
            logger.info(
                f" [CACHE] Dataset enriquecido final salvo em '{enriched_df_path}'."
            )
            ...
            logger.info(
                " --- TREINAMENTO DE FUNDAO (Fases 1 e 2) Concludo com Sucesso ---"
            )
            ...
            logger.critical(
                f" Falha Crtica no Treinamento de Fundao: {e}", exc_info=True
            )
            self.is_trained = False
            return None
    ...
    async def build_enriched_dataset(self, base_df: pd.DataFrame) -> pd.DataFrame:
        ...
        """
        logger.info(
            "Construindo dataset enriquecido com vetor de features otimizado..."
        )
        ...
        if has_hidden_features:
            logger.info(
                " DataFrame j contm hidden features. Pulando aplicao duplicada."
            )
            ...
            if self.feature_pipeline is None:
                logger.error(
                    " [AI] feature_pipeline no inicializado. No  possvel aplicar hidden features."
                )
                return None
        ...
        if "regime" in base_df.columns:
            ...
            for col in cols_to_restore:
                if col in base_df.columns and col not in df_fully_enriched.columns:
                    ...
                    logger.debug(
                        f" [DATA] Coluna '{col}' restaurada em build_enriched_dataset."
                    )
        ...
        prior_cols = [
            c
            for c in ("tp_prior_dir", "tp_prior_conf", "tp_tau", "tp_primary_signal")
            if c in df_fully_enriched.columns
        ]
        ...
        if self.base_feature_columns:
            for hc in hidden_cols:
                if hc not in self.base_feature_columns:
                    ...
                    logger.debug(
                        f" [AI] Feature latente '{hc}' adicionada ao contrato base."
                    )
        ...
        essential_env_cols = [
            ...
            "volume",
            f"atr_{primary_tf}",
            f"bb_lower_{primary_tf}",
            f"bb_upper_{primary_tf}",
            f"rsi_{primary_tf}",
            "bb_expansion",
            "high_volatility",
            "bb_squeeze",
        ...
        if (
            ema_tf_col in df_fully_enriched.columns
            and "ema_trend" not in df_fully_enriched.columns
        ):
        ...
        all_indicators = [
            c
            for c in df_fully_enriched.columns
            if c
            not in (
        ...
        extras_cols = [
            col for col in extra_feature_keys if col in df_fully_enriched.columns
        ]

        ordered_cols = (
            all_indicators + optimized_feature_cols + prior_cols + extras_cols
        )
        ...
        processor = ScientificDataProcessor()
        final_contract_cols = processor.filter_autoencoder_features(
            ordered_cols, df_fully_enriched
        )
        ...
        final_df_cols = list(
            dict.fromkeys(final_contract_cols + hidden_cols + trend_cols)
        )

        # Colunas reais que o DataFrame deve conter: Contraco IA + Metadados + Colunas Base (OHLCV) + TP Signals + Hidden Features
        hidden_sdae_cols = [
            c for c in df_fully_enriched.columns if "hidden" in c or "sdae" in c
        ]
        data_storage_cols = list(
            dict.fromkeys(
                final_df_cols
        ...
        logger.info(
            f" Dataset enriquecido e otimizado criado com sucesso. Shape final: {final_df.shape}"
        )
        logger.info(
            f" Features no contrato (Filtradas): {len(final_df_cols)} | Metadados: {len(data_storage_cols) - len(final_df_cols)}"
        )
        ...
        logger.info(
            f" Contrato Final Definido: {self.final_feature_count} colunas (Escala Z Protegida)."
        )
    ...
    async def handle_drift_and_adapt(self) -> bool:
        """
        [DEPRECATED] Drift handling removed in favor of regime ensemble.
        """
        return True
    ...
    def predict_for_shap(self, X: np.ndarray) -> np.ndarray:
        ...
        if not self.is_trained or not self.base_feature_columns:
            logger.warning(
                " [SHAP] AIController no treinado ou sem contrato de features. Retornando zeros."
            )
            return np.zeros(X.shape[0])
        assert (
            self.base_feature_columns is not None
        )  # narrow Optional[List[str]] -> List[str]
        ...
        try:
            ...
            for idx, row in input_df.iterrows():
                try:
                    ...
                    if available_mc:
                        scores_arr = (
                            row[available_mc].fillna(0).values.astype(np.float32)
                        )
                    ...
                    regime_conf = (
                        ...
                        if "regime_confidence" in row.index
                        else 0.5
                    )
                    regime_sign = {0: 1.0, 1: -1.0, 2: 0.0}.get(regime_val, 0.0)
                    final_score = float(
                        np.clip(
                            raw_score * 0.7 + regime_sign * regime_conf * 0.3, -1.0, 1.0
                        )
                    )
                    ...
                    logger.warning(
                        f"[SHAP] Erro ao processar linha {idx}: {e}. Score neutro."
                    )
                    scores.append(0.0)
    ...
    async def generate_trading_decision(self, recent_market_df: pd.DataFrame) -> Signal:
        ...
        """
        if not self.is_trained or not all(
            [self.specialists, self.risk_manager, self.portfolio]
        ):  # drift_detector opcional
            # [DEBUG GRANULAR] Diagnstico de falha
            missing = []
            if not self.is_trained:
                missing.append("is_trained=False")
            if not self.specialists:
                missing.append("specialists=None/Empty")
            if not self.risk_manager:
                missing.append("risk_manager=None")
            if not self.portfolio:
                missing.append("portfolio=None")
            ...
            if self.config_ai.ALLOW_UNTRAINED_EXECUTION:
                logger.warning(
                    " [AI] ALLOW_UNTRAINED_EXECUTION=True. Operando com modelos potencialmente no treinados/incompletos. Cuidado!"
                )
            else:
                return Signal(
                    symbol=self.config_trading.PRIMARY_PAIR,
                    ...
                    confidence=0.0,
                    explanation={
                        "reason": f"AI not fully trained. Missing: {missing} (use ALLOW_UNTRAINED_EXECUTION to bypass)."
                    },
                )

        try:
            # --- Etapa 1: Preparao Completa de Dados e Features ---
            df_fully_enriched = self.feature_pipeline.apply_hidden_features(
                recent_market_df
            )
            ...
            _hf_cols = [
                c for c in df_fully_enriched.columns if c.startswith("hidden_feature_")
            ]
            has_real_data = False
            ...
            if not has_real_data and hasattr(self, "_last_valid_hidden_dict"):
                logger.info(
                    " [AI] Hidden Features zeradas no loop atual. Injetando dados da memria persistente."
                )
                for k, v in self._last_valid_hidden_dict.items():
                    df_fully_enriched[k] = v
                xai_features_row = df_fully_enriched.iloc[-1].to_dict()
            else:
                xai_features_row = df_fully_enriched.iloc[
                    -1
                ].to_dict()  # Usa o que tiver (real ou fallback do DF)
            ...
            for window in [20, 50]:
                if len(recent_market_df) >= window:
                    ...
                    df_fully_enriched[f"dist_to_max_{window}"] = (
                        latest_close - latest_max
                    ) / (latest_max + 1e-9)
                    df_fully_enriched[f"dist_to_min_{window}"] = (
                        latest_close - latest_min
                    ) / (latest_min + 1e-9)
                else:
            ...
            ae_pipe = (
                self.feature_pipeline.temporal_autoencoder_pipeline
                if self.feature_pipeline
                else None
            )
            ae_required_count = (
                len(ae_pipe.feature_columns)
                if ae_pipe and ae_pipe.feature_columns
                else 0
            )

            is_contract_too_small = (
                self.base_feature_columns is not None
                and len(self.base_feature_columns) < 20
            )
            is_contract_too_large = (
                self.base_feature_columns is not None
                and len(self.base_feature_columns) > 1000
            )
            is_insufficient_for_ae = (
                self.base_feature_columns is not None
                and ae_required_count > 0
                and len(self.base_feature_columns) < ae_required_count
            )

            if (
                self.base_feature_columns is None
                or is_contract_too_small
                or is_contract_too_large
                or is_insufficient_for_ae
            ):
                reason = (
                    "no carregado"
                    if self.base_feature_columns is None
                    else "muito pequeno"
                    if is_contract_too_small
                    else "muito grande"
                    if is_contract_too_large
                    else f"insuficiente para AE ({len(self.base_feature_columns)} < {ae_required_count})"
                )
                logger.warning(f" [AI] Contrato de Features {reason}. Recalculando...")

                # Logic synced from build_enriched_dataset
                hidden_cols = [
                    c for c in df_fully_enriched.columns if "hidden_feature_" in c
                ]
                trend_cols = [
                    c for c in df_fully_enriched.columns if "trend_pred_" in c
                ]
                optimized_feature_cols = hidden_cols + trend_cols

                primary_tf = self.config_trading.PRIMARY_TIMEFRAME_TRADING
                all_tfs = getattr(
                    self.config_trading, "ALL_TRADING_TIMEFRAMES", [primary_tf]
                )
                ...
                existing_essential_cols = [
                    col
                    for col in essential_env_cols
                    if col in df_fully_enriched.columns
                ]
                extras_cols = [
                    col
                    for col in extra_feature_keys
                ...
                if (
                    ...
                    logger.debug(
                        f" [AI] Sincronizando {len(ae_cols)} colunas do AutoEncoder no contrato."
                    )
                ...
                self._save_metadata()
                logger.info(
                    f" [AI] Contrato Auto-Inicializado com {self.final_feature_count} colunas."
                )

                # Inicializa referncia do drift detector tambm
                if self.drift_detector:
                    self.drift_detector.set_reference_data(
                        df_fully_enriched, self.base_feature_columns
                    )
                    logger.info(
                        " [AI] DriftDetector inicializado com dados atuais como referncia."
                    )

            if self.base_feature_columns:
                assert self.base_feature_columns is not None  # type narrowing for Pyre2
                missing_features = set(self.base_feature_columns) - set(
                    df_fully_enriched.columns
                )
                if missing_features:
                    ...
                    return Signal(
                        ...
                        confidence=0.0,
                        explanation={
                            "reason": f"Features ausentes: {missing_features}"
                        },
                    )
                ...
                feature_order_mismatch = [
                    i
                    for i, col in enumerate(self.base_feature_columns)
                    if i < len(df_fully_enriched.columns)
                    and col != df_fully_enriched.columns[i]
                ]
                if feature_order_mismatch:
                    logger.debug(
                        f" [AI] Ordem de features diferente do esperado. Reordenando..."
                    )
                    # Reordena para garantir contrato mas mantm todas as colunas (regime, etc) para etapas posteriores
                    cols_to_keep = self.base_feature_columns + [
                        c
                        for c in df_fully_enriched.columns
                        if c not in self.base_feature_columns
                    ]
            ...
            if self.drift_detector and self.drift_detector.reference_data:
                ...
                if all(col in latest_row for col in features_to_monitor):
                    ...
                    if len(self.drift_check_buffer) >= self.drift_check_min_samples:
                        ...
                        is_drifting = drift_results.get("global", {}).get(
                        ...
                        if is_drifting:
                            logger.warning(
                                " DRIFT DETECTADO! Acionando protocolo de adaptao e pausando trades."
                            )
            ...
            if self.base_feature_columns is None:
                return Signal(
                    ...
                    action=Action.HOLD,
                    confidence=0.0,
                    explanation={"reason": "Contrato de Features no carregado."},
                )

            market_observation = latest_row[self.base_feature_columns].values.astype(
                np.float32
            )
            ...
            pnl = (
                ...
                / (current_pos.entry_price * abs(current_pos.quantity) + 1e-9)
                if current_pos and current_pos.entry_price > 0
                else 0
            )
            ...
            if current_pos:
                ...
                pos_ts_naive = (
                    current_pos.timestamp.replace(tzinfo=None)
                    if hasattr(current_pos.timestamp, "tzinfo")
                    and current_pos.timestamp.tzinfo
                    else current_pos.timestamp
                )
                # Normalizao idntica ao treino: steps_in_candles / 100.0
                _tf_map = {"1m": 60, "5m": 300, "15m": 900, "1h": 3600, "4h": 14400}
                candle_seconds = _tf_map.get(
                    self.config_trading.PRIMARY_TIMEFRAME_TRADING, 900
                )
                steps_in_candles = (
                    ts_naive - pos_ts_naive
                ).total_seconds() / candle_seconds
            else:
                steps_in_candles = 0.0
            agent_state = np.array(
                [pos_side, pnl, steps_in_candles / 100.0], dtype=np.float32
            )

            # Prior: apenas 2 valores como no treino (prior_dir + prior_conf)
            prior_dir_val = float(latest_row.get("tp_prior_dir", 0.0))
            prior_conf_val = float(
                np.clip(float(latest_row.get("tp_prior_conf", 0.0)), 0.0, 1.0)
            )
            ...
            if self.moe_router and self.moe_router.gating and latent_cols:
                try:
                    latent_features_gating = latest_row[latent_cols].values.astype(
                        np.float32
                    )
                    ...
                    if latent_zero_frac > 0.5:
                        ...
                        latent_tensor = (
                            torch.FloatTensor(latent_features_gating)
                            .unsqueeze(0)
                            .to(device)
                        )

                        gating_output = self.moe_router.gating.get_expert_probabilities(
                            latent_tensor
                        )
                        probs = gating_output["probabilities"]

                        # [ANTI-WHIPSAW] EMA temporal smoothing nas probabilidades
                        # Evita oscilação rápida entre specialists em zonas de transição
                        _ema_alpha = 0.3
                        if not hasattr(self, '_gating_probs_ema'):
                            self._gating_probs_ema = dict(probs)
                        else:
                            for k in probs:
                                prev = self._gating_probs_ema.get(k, probs[k])
                                self._gating_probs_ema[k] = _ema_alpha * probs[k] + (1 - _ema_alpha) * prev
                        probs = dict(self._gating_probs_ema)
                        ...
                        if mode == "weighted":
                            ...
                            active_experts = {
                                ...
                                if v >= self.moe_router.threshold
                            }
                        ...
                        if active_experts.get(_regime_specialist_key, 0.0) < 0.20:
                            _rc_blend = float(
                                np.clip(
                                    float(latest_row["regime_confidence"])
                                    if "regime_confidence" in latest_row.index
                                    else 0.5,
                                    0.40,
                                    0.90,
                                )
                            ...
                            for k in ["bull", "bear", "ranger"]:
                                ...
                                blended[k] = (1.0 - _rc_blend) * active_experts.get(
                                    k, 0.0
                                ) + _rc_blend * hard_w
                            active_experts = {
                                k: v for k, v in blended.items() if v > 0.01
                            }
                            total_weight = sum(active_experts.values())
                            active_experts = {
                                k: v / total_weight for k, v in active_experts.items()
                        ...
                        if _n_exp > 0:
                            ...
                            if _bma1_tot > 0:
                                active_experts = {
                                    k: v / _bma1_tot for k, v in active_experts.items()
                                }
                        #
                        logger.info(
                            f" [SOFT GATING] Probabilidades: {probs} -> Especialistas Ativos: {active_experts}"
                        )
                except Exception as e:
                    logger.error(
                        f" [SOFT GATING] Erro ao obter probabilidades: {e}. Usando fallback para Hard Switch."
                    )
            ...
            if not active_experts:
                if "regime" not in latest_row:
                    logger.warning(
                        " [MONITOR] Coluna 'regime' ausente nos dados de inferncia. Usando fallback Ranger (2)."
                    )
                    current_regime = 2
                else:
                ...
                logger.info(
                    f" [HARD SWITCH] Regime={current_regime} ({specialist_key})  peso 100%."
                )

            # Verifica se h especialistas no pool
            if not self.specialists:
                logger.critical(
                    " FALHA CRTICA: Nenhum especialista disponvel no AIController. Abortando sinal."
                )
                return Signal(
                    symbol=self.config_trading.PRIMARY_PAIR,
                    action=Action.HOLD,
                    confidence=0.0,
                    explanation={"reason": "No active specialists available"},
                )
            ...
            for expert_key, expert_weight in active_experts.items():
                ...
                if active_specialist is None:
                    logger.warning(
                        f" Especialista '{expert_key}' no carregado. Ignorando no ensemble."
                    )
                    continue
                ...
                if _spec_cols and all(c in latest_row.index for c in _spec_cols):
                    _mkt = np.array(
                        [float(latest_row.get(c, 0.0)) for c in _spec_cols],
                        dtype=np.float32,
                    )
                    _mem = np.array(
                        [_mem_live.get(k, 0.0) for k in _mem_keys], dtype=np.float32
                    )
                    ...
                    if _spec_cols:
                        ...
                        _prior_fb = np.array(
                            [prior_dir_val, prior_conf_val], dtype=np.float32
                        )
                        expert_observation = np.concatenate(
                            [_mkt_fb, _mem_fb, agent_state, time_features, _prior_fb]
                        )
                    else:
                ...
                target_shape = None
                if (
                    hasattr(active_specialist, "model")
                    and active_specialist.model is not None
                ):
                ...
                logger.debug(
                    f" [DEBUG SHAPE] Specialist: {expert_key} | Current: {current_shape} | Target: {target_shape}"
                )

                if target_shape and current_shape != target_shape:
                    logger.info(
                        f" [AI SHAPE FIX] Adaptando observao ({current_shape[0]} -> {target_shape[0]}) para {active_specialist.__class__.__name__}"
                    )
                    ...
                    if missing_dims > 0 and missing_dims <= 10:
                        ...
                        expert_observation = np.concatenate(
                            [expert_observation, padding]
                        )
                        logger.info(
                            f" [AI SHAPE FIX] Padding simples aplicado. Novo shape: {expert_observation.shape}"
                        )
                    elif (
                        self.feature_pipeline
                        and hasattr(
                        ...
                        and self.feature_pipeline.temporal_autoencoder_pipeline.autoencoder
                    ):
                        try:
                            pipeline_ae = (
                                self.feature_pipeline.temporal_autoencoder_pipeline
                            ...
                            if len(df_fully_enriched) >= seq_len:
                                ...
                                if len(available_ae_cols) < len(ae_input_cols):
                                    seq_df = pd.DataFrame(
                                        0,
                                        index=df_fully_enriched.index[-seq_len:],
                                        columns=ae_input_cols,
                                    )
                                    for c in available_ae_cols:
                                        seq_df[c] = df_fully_enriched[c].iloc[-seq_len:]
                                    seq_data = seq_df.values.astype(np.float32)
                                else:
                                    seq_data = df_fully_enriched.iloc[-seq_len:][
                                        ae_input_cols
                                    ].values.astype(np.float32)

                                if (
                                    hasattr(pipeline_ae, "scaler")
                                    and pipeline_ae.scaler_fitted
                                ):
                                    seq_data_scaled = pipeline_ae.scaler.transform(
                                        seq_data
                                    )
                                ...
                                seq_tensor = torch.FloatTensor(seq_data).to(
                                    pipeline_ae.device
                                )
                                ...
                                with torch.no_grad():
                                    ...
                                    regime_tensor = torch.LongTensor(
                                        [current_regime]
                                    ).to(pipeline_ae.device)
                                    mu, _ = pipeline_ae.autoencoder.encode(
                                        seq_tensor, regime_labels=regime_tensor
                                    )
                                    latent_features = mu.cpu().numpy().flatten()

                                prior_dir_val = float(
                                    latest_row.get("tp_prior_dir", 0.0)
                                )
                                prior_conf_val = float(
                                    latest_row.get("tp_prior_conf", 0.0)
                                )
                                memory_keys = [
                                    "dist_to_max_20",
                                    "dist_to_min_20",
                                ...
                                memory_extras = np.array(
                                    [
                                        float(latest_row.get(k, 0.0))
                                        for k in memory_keys
                                    ],
                                    dtype=np.float32,
                                )

                                def _build_cerebral_obs(
                                    row,
                                    _agent_state,
                                    _time_features,
                                    _prior_dir,
                                    ...
                                    spec_cols = getattr(
                                        active_specialist, "feature_columns", []
                                    )
                                    if not spec_cols:
                                        fs = getattr(
                                            active_specialist, "feature_scaler", None
                                        )
                                        if fs is not None and hasattr(
                                            fs, "feature_names_in_"
                                        ):
                                            spec_cols = list(fs.feature_names_in_)
                                    mkt = (
                                        np.array(
                                            [float(row.get(c, 0.0)) for c in spec_cols],
                                            dtype=np.float32,
                                        )
                                        if spec_cols
                                        else np.zeros(65, dtype=np.float32)
                                    )
                                    mem = np.array(
                                        [
                                            _mem_live.get(k, 0.0)
                                    ...
                                    prior_2 = np.array(
                                        [_prior_dir, _prior_conf], dtype=np.float32
                                    )
                                ...
                                if target_shape[0] >= 75:
                                    expert_observation = _build_cerebral_obs(
                                        latest_row,
                                        agent_state,
                                        time_features,
                                        prior_dir_val,
                                    ...
                                    primary_signal_val = float(
                                        latest_row.get("tp_primary_signal", 0.0)
                                    )
                                    uncertainty_val = float(
                                        latest_row.get("tp_uncertainty", 0.0)
                                    )
                                    p_extras = np.array(
                                        [
                                            prior_dir_val,
                                ...
                                if expert_observation.shape[0] > target_shape[0]:
                                    ...
                                    expert_observation = np.pad(
                                        expert_observation,
                                        (0, target_shape[0] - len(expert_observation)),
                                    )
                            else:
                            ...
                            logger.error(
                                f" [AI SHAPE FIX] Erro durante encoding: {e}. Fallback emergencial."
                            )
                            if current_shape[0] < target_shape[0]:
                                pad = np.zeros(
                                    target_shape[0] - current_shape[0], dtype=np.float32
                                )
                                expert_observation = np.concatenate(
                                    [expert_observation, pad]
                                )
                ...
                try:
                    ...
                    strategic_signal_ind = active_specialist.decide_action(
                        expert_observation, latest_row
                    )
                    if strategic_signal_ind:
                        if strategic_signal_ind.explanation is not None:
                            strategic_signal_ind.explanation["specialist"] = (
                                specialist_display_name
                            )
                            strategic_signal_ind.explanation["regime"] = (
                                expert_key.upper()
                            )
                        expert_signals.append(
                            (
                                strategic_signal_ind,
                                expert_weight,
                                specialist_display_name,
                                expert_key,
                            )
                    ...
                    logger.error(
                        f" [AI ERROR] Erro no especialista {expert_key}: {e}",
                        exc_info=True,
                    )
            ...
            if len(expert_signals) > 1:
                ...
                _cur_price = float(
                    latest_row["close_15m"]
                    if "close_15m" in latest_row.index
                ...
                for _bk, _bhist in self._bma_perf_history.items():
                    if (
                        _bhist
                        and _bhist[-1].get("price")
                        and _bhist[-1].get("outcome") is None
                    ):
                        _bprev = _bhist[-1]
                        ...
                        _bouts = [
                            h["outcome"] for h in _bhist if h.get("outcome") is not None
                        ]
                        if len(_bouts) >= 3:
                            _balpha = 0.9  # decaimento: observaes recentes valem mais
                            _bws = [
                                _balpha ** (len(_bouts) - 1 - i)
                                for i in range(len(_bouts))
                            ]
                            _bewa = sum(w * int(o) for w, o in zip(_bws, _bouts)) / sum(
                                _bws
                            )
            ...
            if not expert_signals:
                strategic_signal = Signal(
                    ...
                    action=Action.HOLD,
                    confidence=0.0,
                    explanation={"reason": "Nenhum sinal gerado pelos especialistas."},
                )
            else:
                ...
                voters = [
                    (s, weight, n, k)
                    for s, weight, n, k in expert_signals
                    if s.action != Action.HOLD
                ]
                ...
                if voters:
                    ...
                    for sig, w, name, key in expert_signals:
                        ...
                        sl_val = (
                            sig.stop_loss
                            if (sig.stop_loss is not None and sig.stop_loss > 0)
                            else self.config_trading.DEFAULT_STOP_LOSS_PCT
                        )
                        tp_val = (
                            sig.take_profit
                            if (sig.take_profit is not None and sig.take_profit > 0)
                            else self.config_trading.DEFAULT_TAKE_PROFIT_PCT
                        )
                ...
                onchain_signal = self.onchain_engine.get_onchain_sentiment_signal(
                    asset="BTC"
                )
                tape_pulse = self.tape_engine.get_market_pulse(
                    self.config_trading.PRIMARY_PAIR
                )
                ...
                _base_threshold = getattr(
                    self.config_trading, "MIN_CONFIDENCE_FOR_TRADE", 0.60
                )
                ...
                if _reg_conf < 0.75 and _pulse.startswith(("STRONG", "EXTREME")):
                    ...
                    if _min_conf < _base_threshold:
                        logger.info(
                            f" [ADAPTIVE] Threshold reduzido: {_base_threshold:.2f} -> {_min_conf:.2f} (Regime Conf: {_reg_conf:.0%}, Tape: {_pulse})"
                        )
                ...
                _dir_signals = [
                    (s, w) for s, w, _, _ in expert_signals if s.action != Action.HOLD
                ]
                if _dir_signals:
                    _dir_w_total = sum(w for _, w in _dir_signals)
                    _dir_score = (
                        sum(
                            (s.confidence if s.action == Action.BUY else -s.confidence)
                            * w
            ...
            if strategic_signal.action == Action.HOLD and not getattr(
                ...
                try:
                    if self.explainer:
                        ...
                        top_expl = self.explainer.explain_decision(
                            strategic_signal, market_data_dict, top_n=8
                        )
                        self._is_explaining = False

                        strategic_signal.explanation["main_contributors"] = (
                            top_expl.get("main_contributors")
                        )
                        strategic_signal.explanation["xai_mode"] = top_expl.get(
                            "mode", "full"
                        )
                        if "narrative" in top_expl:
                            strategic_signal.explanation["narrative"] = top_expl[
                                "narrative"
                            ]
                except Exception as e:
            ...
            # --- Etapa 7: Meta-Labeling Filter (Lopez de Prado 2018, Ch.3) + Risk ---
            # Use the ProfitabilityPredictor to filter low-probability trades.
            profit_prob = 0.5
            if self.profitability_predictor and self.profitability_predictor.is_trained:
                try:
                    entry_feat = (
                        latest_row[
                            [
                                c
                                for c in self.profitability_predictor.feature_names
                                if c in latest_row.index
                            ]
                        ].values.astype(np.float64)
                        if self.profitability_predictor.feature_names
                        else None
                    )
                    if entry_feat is not None and len(entry_feat) > 0:
                        profit_prob = self.profitability_predictor.predict(entry_feat)
                        threshold = self.profitability_predictor.profit_threshold
                        if profit_prob < threshold:
                            logger.info(
                                f"[MetaLabel] Trade REJEITADO: P(profit)={profit_prob:.3f} < {threshold}"
                            )
                            return TradingSignal(
                                action=Action.HOLD,
                                confidence=strategic_signal.confidence,
                                explanation={
                                    "reason": f"MetaLabel: P(profit)={profit_prob:.3f} < {threshold}"
                                },
                            )
                except Exception as e:
                    logger.debug(f"[MetaLabel] Filter skipped: {e}")
            else:
                # Fallback: use specialist confidence as proxy
            ...
            strategic_signal = dc_replace(
                strategic_signal, profit_probability=profit_prob
            )
            ...
            is_approved, reason = self.risk_manager.check_trade_approval(
                modulated_signal
            )
            ...
            if not is_approved:
                ...
                modulated_signal.explanation["original_action"] = (
                    strategic_signal.action
                )  # Salva a inteno original
                modulated_signal.explanation["original_confidence"] = (
                    strategic_signal.confidence
                )  # Salva a confiana original

                final_rejection = Signal(
                    symbol=self.config_trading.PRIMARY_PAIR,
                    action=Action.HOLD,
                    confidence=0.0,
                ...
                try:
                    if self.explainer:
                        top_expl = self.explainer.explain_decision(
                            final_rejection, latest_row
                        )
                        final_rejection.explanation["main_contributors"] = top_expl.get(
                            "main_contributors"
                        )
                        if "narrative" in top_expl:
                            final_rejection.explanation["narrative"] = top_expl[
                                "narrative"
                            ]
                except Exception as e:
            ...
            try:
                ...
                if self.explainer and modulated_signal:
                    top_expl = self.explainer.explain_decision(
                        modulated_signal, latest_row
                    )
                    modulated_signal.explanation["main_contributors"] = top_expl.get(
                        "main_contributors"
                    )
                    modulated_signal.explanation["xai_mode"] = top_expl.get(
                        "mode", "full"
                    )

                    # Se tiver narrativa, anexa tambm
                    if "narrative" in top_expl:
                        modulated_signal.explanation["narrative"] = top_expl[
                            "narrative"
                        ]
            except Exception as e:
            ...
            logger.info(
                f" [AI] Deciso final da IA: {modulated_signal.action.value} para {modulated_signal.symbol}"
            )
            return modulated_signal

        except Exception as e:
            logger.critical(
                f" [AI] Falha catastrfica ao gerar deciso: {e}", exc_info=True
            )
            return Signal(
                symbol=self.config_trading.PRIMARY_PAIR,
                action=Action.HOLD,
    ...
    def _apply_contextual_modulation(
        ...
        # --- 1. Modulação de Regime (Contra-Tendência) ---
        is_counter_trend = (action == Action.BUY and current_regime == "BEAR") or (
        ...
        if is_counter_trend:
            ...
            # Fluxo Institucional Massivo (Tape) — única condição para permitir contra-tendência
            _tape_dir_match = (action == Action.BUY and "BULLISH" in pulse) or (
                action == Action.SELL and "BEARISH" in pulse
            )
            if _tape_dir_match and tape_score > 0.80 and rvol > 1.5:
                allow_counter_trend = True
                final_signal.confidence *= 0.95
                logger.info(
                    f" [TREND MOD] Contra-tendência permitida por Fluxo Institucional (Tape={tape_score:.2f})"
                )

            if not allow_counter_trend:
                final_signal.confidence *= (
                    0.6  # penalidade fixa de 40% para contra-tendência
                )
                logger.info(
                    f" [TREND MOD] Contra-tendência em regime persistente. Penalidade de 40%"
                )
                if final_signal.confidence < 0.65:
                    veto_reason = "Contra-tendencia bloqueada"
        ...
        if veto_reason:
            ...
            return replace(
                final_signal, action=Action.HOLD, confidence=0.0, position_size_pct=0.0
            )

        # --- 4. Amplificação Contextual ---
        # Se tudo está alinhado (OnChain + Tape + Confiança), aumentamos a aposta.
        onchain_sig = (
            onchain_signal.get("signal", "NEUTRAL")
        ...
        if is_buy_amplified or is_sell_amplified:
            logger.info(
                f" [QUANTUM MOD] AMPLIFICANDO sinal de {action.value} (Convergência Total)"
            )
    ...
    async def adapt(
        ...
        if recent_data.empty or len(recent_data) < 50:
            logger.warning(
                " [ADAPT] Dados insuficientes para adaptao (mnimo 50 pontos)."
            )
            return False

        try:
            logger.info(
                f" [ADAPT] Iniciando adaptao com {len(recent_data)} pontos de dados (fora: {adaptation_strength})"
            )
            ...
            if self.profitability_predictor and hasattr(
                self.profitability_predictor, "adapt"
            ):
                ...
                if profit_adapt_success:
                    ...
                    logger.warning(
                        " [ADAPT] Falha na adaptao do ProfitabilityPredictor."
                    )

            # Adapta os especialistas
            specialist_adaptations = 0
            for specialist_name, specialist in self.specialists.items():
                if hasattr(specialist, "adapt") and getattr(
                    specialist, "is_trained", False
                ):
                    try:
                        ...
                        logger.warning(
                            f" [ADAPT] Erro ao adaptar {specialist_name}: {e}"
                        )

            logger.info(
                f" [ADAPT] {specialist_adaptations}/{len(self.specialists)} especialistas adaptados."
            )

            # Adapta o HRL Master se disponvel
            if (
                self.hrl_master
                and hasattr(self.hrl_master, "adapt")
                and self.hrl_master.is_trained
            ):
                try:
