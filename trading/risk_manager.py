...
class RiskManager:
    ...
    def __init__(self, config: TradingConfig):
        if not isinstance(config, TradingConfig):
            raise TypeError(
                "🚨 [ERRO RISCO] 'config' deve ser uma instância da classe TradingConfig."
            )
        self.config = config
        ...
        logger.info(
            f"🛡️ [RISCO] RiskManager inicializado. Limites de Risco: Max Drawdown {self.config.MAX_DRAWDOWN_PERCENT:.2%}, Max VaR {self.config.MAX_VAR_1D_PERCENT:.2%}."
        )
    ...
    def update_portfolio_state(
        ...
        if not isinstance(portfolio_value, (int, float)) or portfolio_value < 0:
            logger.error(
                f"❌ [ERRO RISCO] 'portfolio_value' inválido: {portfolio_value}. Deve ser um número não negativo."
            )
            return
        if not isinstance(cash, (int, float)) or cash < 0:
            logger.error(
                f"❌ [ERRO RISCO] 'cash' inválido: {cash}. Deve ser um número não negativo."
            )
            return
        if not isinstance(positions, dict):
            logger.error(
                f"❌ [ERRO RISCO] 'positions' inválido. Deve ser um dicionário. Recebido: {type(positions)}."
            )
            return
        ...
        if self.portfolio_value > 0:
            self.current_leverage_ratio = (
                self.total_notional_value / self.portfolio_value
            )
        else:
        ...
        logger.debug(
            f"🛡️ [RISCO] Estado do portfólio atualizado. Equity: ${self.portfolio_value:,.2f}, Cash: ${self.cash:,.2f}, "
    ...
    def _update_risk_envelopes(self):
        ...
        if (
            self.current_leverage_ratio > self.config.MAX_LEVERAGE_PER_TRADE * 0.7
        ):  # 70% do limite máximo
            self.leverage_reduction_factor = max(
                0.3,
                1.0
                - (self.current_leverage_ratio / self.config.MAX_LEVERAGE_PER_TRADE),
            )
            logger.warning(
                f"⚠️ [RISCO] Alavancagem alta ({self.current_leverage_ratio:.2f}x). Reduzindo posições por fator {self.leverage_reduction_factor:.2f}"
            )
        else:
        ...
        if (
            self.current_drawdown > self.config.MAX_DRAWDOWN_PERCENT * 0.6
        ):  # 60% do limite máximo
            self.drawdown_reduction_factor = max(
                0.2, 1.0 - (self.current_drawdown / self.config.MAX_DRAWDOWN_PERCENT)
            )
            logger.warning(
                f"⚠️ [RISCO] Drawdown alto ({self.current_drawdown:.2%}). Reduzindo posições por fator {self.drawdown_reduction_factor:.2f}"
            )
        else:
            self.drawdown_reduction_factor = 1.0

        # Fator de redução combinado (usa o mais restritivo)
        self.combined_reduction_factor = min(
            self.leverage_reduction_factor, self.drawdown_reduction_factor
        )
    ...
    def _get_current_price_from_positions(self, symbol: str) -> float:
        ...
        if symbol in self.positions:
            ...
            )  # Usar entry_price como fallback se não tiver acesso ao preço atual
        return 0.0
    ...
    def add_historical_data(self, data: Dict[str, pd.DataFrame]):
        ...
        if not isinstance(data, dict):
            logger.error(
                f"❌ [ERRO RISCO] 'data' deve ser um dicionário. Recebido: {type(data)}."
            )
            return
        ...
        for symbol, df in data.items():
            if (
                ...
                logger.warning(
                    f"⚠️ [ALERTE RISCO] Dados históricos inválidos ou vazios para '{symbol}'. Ignorando."
                )
                continue
            ...
            returns = np.log(
                df["close"] / (df["close"].shift(1) + np.finfo(float).eps)
            ).dropna()

            if returns.empty:
                logger.warning(
                    f"⚠️ [ALERTE RISCO] Retornos vazios calculados para '{symbol}'. Dados insuficientes ou estáticos."
                )
                continue

            self.historical_returns[symbol] = returns
            logger.debug(
                f"📊 [RISCO] Retornos históricos para '{symbol}' atualizados. {len(returns)} pontos."
            )
    ...
    def check_trade_approval(self, signal: Signal) -> Tuple[bool, str]:
        ...
        if (
            signal.action == Action.HOLD or signal.action == "HOLD"
        ):  # [BUG FIX] comparar enum corretamente
            return (
                True,
                "✅ Sinal HOLD: Nenhuma alteração de risco, aprovação automática.",
            )
        ...
        # [FIX C2] Validação de custo da operação — rejeita trades onde custo
        # estimado (fee + slippage) excede fração razoável do retorno esperado.
        _est_notional = abs(
            getattr(signal, "position_size_notional", 0)
            or getattr(signal, "notional", 0)
            or 0
        )
        if _est_notional > 0:
            trade_cost = self._calculate_trade_cost_impact(signal, _est_notional)
            _expected_return = (
                _est_notional * 0.002
            )  # proxy: 0.2% de retorno esperado mínimo
            if trade_cost > _expected_return * 0.5:
                _reason = (
                    f"❌ [CUSTO] Custo estimado (${trade_cost:.2f}) > 50% do retorno "
                    f"esperado (${_expected_return:.2f}). Trade rejeitado."
                )
        ...
        if current_pos:
            ...
            if (
                (action_val == "BUY" and current_pos.quantity < 0)
                or (action_val == "SELL" and current_pos.quantity > 0)
                or (action_val == "CLOSE")
            ):
                is_closing_trade = True
                logger.info(
                    f"🛡️ [RISCO] Ordem de FECHAMENTO detectada para {signal.symbol}. Ignorando limites de margem para redução de risco."
                )

        # 2. Checagem de Margem Disponível (o sinal usa signal.position_size_pct como % de margem)
        initial_margin_needed_for_trade = (
            self.portfolio_value * signal.position_size_pct
        )
        ...
        if not is_closing_trade and available_cash < initial_margin_needed_for_trade:
            reason = (
                f"❌ [REJEITADO] Caixa disponível (${available_cash:,.2f}) insuficiente para "
        ...
        if self.current_drawdown > self.config.MAX_DRAWDOWN_PERCENT:
            reason = (
                f"❌ [REJEITADO] Drawdown atual ({self.current_drawdown:.2%}) "
            ...
            logger.warning(
                f"🚨 {reason} Equity atual: ${self.portfolio_value:,.2f}, Pico: ${self.peak_portfolio_value:,.2f}."
            )
            return False, reason
        ...
        trade_notional_value = (
            self.portfolio_value * signal.position_size_pct * signal.leverage
        )
        ...
        projected_leverage_ratio = projected_total_notional_value / (
            self.portfolio_value + np.finfo(float).eps
        )
        ...
        if (
            ...
            and projected_leverage_ratio > self.config.MAX_TOTAL_EXPOSURE_PERCENT
        ):
            reason = (
                f"❌ [REJEITADO] Alavancagem projetada do portfólio ({projected_leverage_ratio:.2f}x) "
        ...
        margin_ratio = projected_margin_used / (
            self.portfolio_value + np.finfo(float).eps
        )
        logger.debug(
            f"📊 [RISCO] Check Margem: {projected_margin_used:.2f} / {self.portfolio_value:.2f} = {margin_ratio:.2%} (Limite: {self.config.MAX_POSITION_SIZE_PERCENT:.2%}). Closing: {is_closing_trade}"
        )

        if (
            not is_closing_trade
            and margin_ratio > self.config.MAX_POSITION_SIZE_PERCENT
        ):
            reason = (
                f"❌ [REJEITADO] Margem total utilizada projetada "
        ...
        if not (
            self.config.MIN_LEVERAGE_PER_TRADE
            <= signal.leverage
            <= self.config.MAX_LEVERAGE_PER_TRADE
        ):
            reason = (
                f"❌ [REJEITADO] Alavancagem solicitada ({signal.leverage:.2f}x) fora dos limites permitidos "
        ...
        if projected_var_value > self.config.MAX_VAR_1D_PERCENT:
            reason = (
                f"❌ [REJEITADO] VaR projetado do portfólio ({projected_var_value:.2%}) "
        ...
        if (
            hasattr(self, "combined_reduction_factor")
            and self.combined_reduction_factor < 1.0
        ):
            ...
            logger.info(
                f"🛡️ [RISCO] Aplicando envelope dinâmico: posição reduzida de {original_position_size:.2%} para {signal.position_size_pct:.2%} "
        ...
        logger.info(
            f"✅ [APROVADO] Sinal para {signal.symbol} ({signal.action.value}, Confiança: {signal.confidence:.2%}, P: {signal.profit_probability:.2%}, Alavancagem: {signal.leverage:.2f}x) aprovado pela gestão de risco."
        )
    ...
    def validate_signal_quality(self, signal: Signal) -> Tuple[bool, str]:
        ...
        if signal.confidence < self.config.MIN_CONFIDENCE_FOR_TRADE:
            return False, (
                f"❌ [QUALIDADE] Confiança do modelo ({signal.confidence:.2%}) "
        ...
        if signal.profit_probability < self.config.MIN_PROFIT_PROBABILITY:
            return False, (
                f"❌ [QUALIDADE] Probabilidade de lucro estimada ({signal.profit_probability:.2%}) "
    ...
    def check_liquidation_risk(self) -> List[str]:
        ...
        try:
            for symbol, pos in self.positions.items():
                ...
                distance_pct = (
                    abs(pos.mark_price - pos.liquidation_price) / pos.mark_price
                )
                ...
                if distance_pct < 0.05:
                    alert_msg = (
                        f"🚨 PERIGO DE LIQUIDAÇÃO: {symbol} está a APENAS {distance_pct:.2%} "
    ...
    def calculate_position_size(
        ...
        if not isinstance(symbol, str) or not symbol:
            logger.error(
                "❌ [ERRO RISCO] 'symbol' inválido para calculate_position_size."
            )
            return 0.0, 1.0
        if not isinstance(confidence, (int, float)) or not (0.0 <= confidence <= 1.0):
            logger.error(
                f"❌ [ERRO RISCO] 'confidence' inválida: {confidence}. Deve ser entre 0 e 1."
            )
            return 0.0, 1.0
        if not isinstance(volatility, (int, float)) or volatility <= 0:
            logger.warning(
                f"⚠️ [ALERTE RISCO] Volatilidade para {symbol} é zero/negativa ({volatility}). "
                "Usando volatilidade padrão mínima para evitar divisão por zero."
            )
            volatility = 0.005  # Valor mínimo razoável, 0.5%
        if not isinstance(profit_probability, (int, float)) or not (
            0.0 <= profit_probability <= 1.0
        ):
            logger.error(
                f"❌ [ERRO RISCO] 'profit_probability' inválida: {profit_probability}. Deve ser entre 0 e 1."
            )
            return 0.0, 1.0
        if not isinstance(desired_leverage, (int, float)) or not (
            self.config.MIN_LEVERAGE_PER_TRADE
            <= desired_leverage
            <= self.config.MAX_LEVERAGE
        ):
            logger.warning(
                f"⚠️ [ALERTE RISCO] Alavancagem desejada inválida ({desired_leverage}). Limitando ao range [{self.config.MIN_LEVERAGE_PER_TRADE}, {self.config.MAX_LEVERAGE}]."
            )
            desired_leverage = np.clip(
                desired_leverage,
                self.config.MIN_LEVERAGE_PER_TRADE,
                self.config.MAX_LEVERAGE,
            )
        ...
        risk_adjustment_factor = (
            1.0 + (confidence - 0.5) * 0.5 + (profit_probability - 0.5) * 0.75
        )
        risk_adjustment_factor = np.clip(
            risk_adjustment_factor, 0.5, 1.5
        )  # Limita o fator de ajuste
        ...
        final_leverage_to_apply = np.clip(
            desired_leverage,
            self.config.MIN_LEVERAGE_PER_TRADE,
            self.config.MAX_LEVERAGE_PER_TRADE,
        )
        ...
        margin_allocation_pct = position_size_notional_base_pct / (
            final_leverage_to_apply + np.finfo(float).eps
        )

        # 5. Limites Finais: Margem
        # Garante que a margem alocada não exceda a MAX_POSITION_SIZE_PERCENT (margem máxima por trade).
        margin_allocation_pct = min(
            margin_allocation_pct, self.config.MAX_POSITION_SIZE_PERCENT
        )
        ...
        if resulting_notional_pct > self.config.MAX_TOTAL_EXPOSURE_PERCENT:
            ...
            final_leverage_to_apply = self.config.MAX_TOTAL_EXPOSURE_PERCENT / (
                margin_allocation_pct + np.finfo(float).eps
            )
            final_leverage_to_apply = np.clip(
                final_leverage_to_apply,
                self.config.MIN_LEVERAGE_PER_TRADE,
                self.config.MAX_LEVERAGE_PER_TRADE,
            )

            logger.debug(
                f"📐 [RISCO] Alavancagem ajustada devido ao limite de exposição total. Nova Alavancagem: {final_leverage_to_apply:.2f}x."
            )

        logger.debug(
            f"📐 [RISCO] Cálculo de Posição Final para {symbol}: Confiança={confidence:.2f}, P_Lucro={profit_probability:.2f}, Volatilidade={volatility:.2%}, "
    ...
    def calculate_portfolio_var(
        self, add_trade_signal: Optional[Signal] = None
    ) -> float:
        """
        ...
        if (
            self.portfolio_value <= 0 and not self.positions
        ) and add_trade_signal is None:
            return 0.0
        ...
        if self.portfolio_value > 0:
            for symbol, pos in self.positions.items():
                ...
                if len(self.positions) == 1:
                    current_notional_weights[symbol] = self.current_leverage_ratio * (
        ...
        if add_trade_signal:
            if not isinstance(add_trade_signal, Signal):
                logger.error(
                    f"❌ [ERRO RISCO] Sinal de trade inválido para cálculo de VaR. Tipo: {type(add_trade_signal)}. Ignorando."
                )
            else:
                # O valor nocional do trade hipotético (em relação ao equity total)
                # (Capital Total * Margem Alocada % ) * Alavancagem
                signal_notional_value_pct_of_equity = (
                    add_trade_signal.position_size_pct * add_trade_signal.leverage
                )
                ...
                if add_trade_signal.action == OrderSide.SELL:
                    trade_weight *= (
                        -1
                    )  # Se for uma ordem de venda (abrir short), o peso é negativo
                ...
                projected_notional_weights[add_trade_signal.symbol] = (
                    projected_notional_weights.get(add_trade_signal.symbol, 0.0)
                    + trade_weight
                )

                logger.debug(
                    f"📊 [RISCO] Calculando VaR projetado com sinal: {add_trade_signal.symbol} {add_trade_signal.action.value} {add_trade_signal.leverage:.2f}x alavancagem."
                )

        if not projected_notional_weights:
            # Em startup ou se apenas caixa, o VaR é 0 - não é um alerta de risco mas um estado normal
            logger.debug(
                "🛡️ [RISCO] Nenhuns pesos de portfólio (100% Caixa). VaR = 0.0."
            )
            return 0.0
        ...
        for sym, weight in projected_notional_weights.items():
            if (
                sym in self.historical_returns
                and not self.historical_returns[sym].empty
            ):
                relevant_returns_series[sym] = self.historical_returns[sym]
            elif abs(weight) > 1e-9:
                logger.warning(
                    f"⚠️ [ALERTE RISCO] Dados históricos de retorno ausentes para '{sym}' para cálculo de VaR. Isso pode afetar a precisão."
                )

        if not relevant_returns_series:
            logger.warning(
                "⚠️ [ALERTE RISCO] Não há dados históricos de retorno para os ativos relevantes no portfólio. Impossível calcular VaR. Retornando VaR máximo configurado."
            )
        ...
        if len(returns_df) < min_var_samples:
            logger.warning(
                f"⚠️ [ALERTE RISCO] Dados históricos insuficientes ({len(returns_df)} pontos, mínimo {min_var_samples}) para calcular VaR de forma robusta. Retornando VaR máximo configurado."
            )
            return self.config.MAX_VAR_1D_PERCENT
        ...
        if portfolio_hist_returns.empty:
            logger.error(
                "❌ [ERRO RISCO] Retornos históricos do portfólio estão vazios após cálculo ponderado. Retornando VaR máximo."
            )
            return self.config.MAX_VAR_1D_PERCENT
        ...
        var_95 = -np.percentile(
            portfolio_hist_returns, 5
        )  # 5º percentil para 95% de confiança

        # Impacto das Funding Fees no VaR
        # Custo diário = Valor Nocional Total * Custo Diário da Alavancagem %
        estimated_daily_funding_cost_pct = (
            self.total_notional_value
            * self.config.LEVERAGE_COST_PER_DAY_PCT
            / (self.portfolio_value + np.finfo(float).eps)
        )
        ...
        logger.debug(
            f"📊 [RISCO] VaR do Portfólio (95%, 1D) calculado: {var_95:.4f} (base), "
    ...
    def get_current_risk_profile(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "is_risk_off_mode": (
                self.current_drawdown
                > self.config.MAX_DRAWDOWN_PERCENT  # Drawdown crítico
                or self.current_leverage_ratio
                > self.config.MAX_TOTAL_EXPOSURE_PERCENT  # Exposição total excedida
    ...
    def _calculate_trade_cost_impact(
        self, signal: Signal, trade_value_notional: float, is_maker_order: bool = False
    ) -> float:
        """
