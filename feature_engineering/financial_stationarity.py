...
try:
    ...
    try:
        ...
        import_logger.warning(
            f"⚠️ [STATIONARITY] statsmodels not found or failed to import: {e}"
        )
    except Exception:
        print(
            f"Warning: statsmodels not found. Stationarity tests will fail. Error: {e}"
        )
try:
    import torch
except ImportError:
...
class StationarityValidator:
    ...
    def test_adf(
        ...
        try:
            ...
            current_alpha = alpha if alpha is not None else self.adf_alpha
            is_stationary = pvalue < current_alpha
    ...
    def test_kpss(
        self, series: pd.Series, regression: str = "c", alpha: Optional[float] = None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            current_alpha = alpha if alpha is not None else self.kpss_alpha
            is_stationary = pvalue > current_alpha
            ...
            return {
                ...
                "interpretation": "Stationary"
                if is_stationary
                else "Non-stationary (trend/drift)",
            }
    ...
    def validate_series(
        self, series: pd.Series, name: str = "", alpha: Optional[float] = None
    ) -> Dict[str, Any]:
        """
        ...
        """
        adf_result = self.test_adf(series, alpha=alpha)
        kpss_result = self.test_kpss(series, alpha=alpha)
    ...
    def validate_dataframe(
        ...
        for col in cols_to_test:
            ...
            result = self.validate_series(df[col], name=col, alpha=significance_level)
    ...
    def fractional_diff(
        self, series: pd.Series, d: float = 0.5, threshold: float = 1e-5
    ) -> pd.Series:
        """
        ...
        if torch is not None and torch.cuda.is_available():
            try:
                ...
                series_tensor = torch.tensor(
                    series.values, dtype=torch.float32, device=device
                ).view(1, 1, -1)
                ...
                logger.warning(
                    f"⚠️ GPU FracDiff falhou, usando fallback CPU: {torch_err}"
                )

        # Fallback Vectorized Numpy (Convolve) - Muito mais rápido que loop Python
        # np.convolve implementa convolução pura (inverte o kernel internamente).
        # Passar weights[::-1] resultaria em dupla inversão → pesos trocados no tempo.
        # Passamos weights sem inversão para obter y[t] = sum w_k * x[t-k] correto.
        padding = len(weights) - 1
        output_values = np.convolve(series.values, weights, mode="valid")
    ...
    def make_stationary(
        ...
        if method == "fractional_diff":
            ...
            weights = self._get_frac_diff_weights(d_optimal)
            warmup = len(weights) - 1
            logger.info(
                f"🔗 [Stationarity] Causal overlap: usando {warmup} observações "
                f"do tail do treino como warmup para fracdiff na validação."
            )
            ...
            for col in train_df.columns:
                ...
                diff_train = self.fractional_diff(train_df[col], d=d_optimal)
                train_transformed[col] = diff_train

                # Causal overlap: val_df usa tail do treino como warmup convolucional
                train_tail = train_df[col].iloc[-warmup:]
                overlap_series = pd.concat([train_tail, val_df[col]])
                diff_val_overlap = self.fractional_diff(overlap_series, d=d_optimal)
                diff_val = diff_val_overlap.iloc[warmup:]
                val_transformed[col] = diff_val.values

            # Truncamento global deterministico pelo warmup (todas as features
            # compartilham o mesmo d_optimal → mesmo numero de pesos → mesmo warmup).
            # dropna() e perigoso: features com NaNs upstream podem truncar datasets
            # de forma inconsistente, desalinhando regime labels e targets.
            train_transformed = train_transformed.iloc[warmup:]
            val_transformed = val_transformed.iloc[warmup:]

        elif method == "diff":
            # Primeira diferença simples
            # [R3 FIX] Causal overlap: usa a última linha do treino para calcular
            # a primeira diferença da validação, evitando NaN na borda train/val.
            # Antes: val_df.diff() produzia NaN[0] porque não tinha X_{t-1} do treino.
            train_transformed = train_df.diff().dropna()
            val_extended = pd.concat([train_df.iloc[[-1]], val_df])
            val_transformed = val_extended.diff().dropna()

        elif method == "returns":
            # Log returns
            # [M3 FIX] Causal overlap igual ao método 'diff' (R3 FIX).
            # Antes: val_df.shift(1) tinha NaN no primeiro elemento → val_df perdia o
            # primeiro bar artificialmente, criando descontinuidade entre train e val.
            # Agora: usa a última linha do treino como X_{t-1} do primeiro bar de val.
            train_transformed = np.log(train_df / train_df.shift(1)).dropna()
            val_extended = pd.concat([train_df.iloc[[-1]], val_df])
            val_transformed = np.log(val_extended / val_extended.shift(1)).dropna()
    ...
    def find_optimal_d(
        ...
        step: float = 0.05,  # [SCIENTIFIC FIX v2] precisão fina (era 0.1)
        p_threshold: float = 0.01,  # [SCIENTIFIC FIX v2] p<0.01 evita fragilidade OOS
        ...
        Args:
            train_ratio: Fração da série a usar para encontrar d (1.0 = toda série).
                Se < 1.0, apenas os primeiros train_ratio% são usados para evitar
                look-ahead bias (o parâmetro d deve ser fitado apenas no treino).
        """
        if train_ratio < 1.0:
            original_len = len(series)
            split_idx = int(original_len * train_ratio)
            series = series.iloc[:split_idx]
            logger.info(
                f"🔒 [Stationarity] Anti-leakage: usando apenas {train_ratio:.0%} "
                f"da série ({split_idx}/{original_len} observações) para fitar d ótimo."
            )
        ...
        logger.warning(
            f"⚠️ [Stationarity] Nenhum d no range atingiu p < {p_threshold}. Retornando d=1.0 (Standard Diff)"
        )
        return 1.0
