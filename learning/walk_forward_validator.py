...
class WalkForwardValidator:
    ...
    def __init__(
        ...
        gap_periods: int = 96,  # Embargo period between train and test (24h para 15m candles)
    ...
    def split(
        ...
        test_size_per_fold = max(
            self.min_test_samples, total_test_size // self.n_splits
        )
...
class WalkForwardOptimizer:
    ...
    def __init__(
        ...
        self.sampler = TPESampler(n_startup_trials=10, multivariate=True, group=True)

        # Pruner for early stopping of bad trials
        self.pruner = HyperbandPruner(
            min_resource=1, max_resource=self.validator.n_splits, reduction_factor=3
        )
    ...
    def optimize(
        ...
        def objective(trial: optuna.Trial) -> float:
            ...
            try:
                ...
                trial.report(
                    getattr(metrics, self.objective_metric), step=metrics.n_folds
                )
        ...
        logger.info(
            f"🔬 Starting optimization: {self.n_trials} trials, {self.timeout}s timeout"
        )
...
def quick_walk_forward_split(
    df: pd.DataFrame, n_splits: int = 5, gap: int = 50
