...
from risk.risk_layer import RiskLayer, RiskConfig
...
system_state: Dict[str, Any] = {
    ...
    "binance_connection": "desconectado",
    "last_explanation": {},
    "live_trading_enabled": False,
    "ai_status": "inicializando",
    "system_health": "unknown",
    "drift_status": "desconhecido",
...
def save_trade_to_log(trade_data: Dict):
    ...
    try:
        ...
        logger.info(
            f"💾 [AUDITORIA] Trade salvo: {trade_data.get('symbol')} {trade_data.get('action')}"
        )
    except Exception as e:
        logger.error(
            f"❌ [AUDITORIA] Falha ao salvar trade no log CSV: {e}", exc_info=True
        )
...
def log_portfolio_status(portfolio_name: str, portfolio_data: Dict):
    ...
    lev_char = (
        "🟢"
        if portfolio_data.get("total_leverage_ratio", 0) < 2
        else "🟡"
        if portfolio_data.get("total_leverage_ratio", 0) < 5
        else "🔴"
    )
...
def log_system_status():
    ...
    mode = (
        "LIVE TRADING (REAL)"
        if system_state["live_trading_enabled"]
        else "PAPER TRADING (SIMULADO)"
    )
    ...
    if latest_feat is not None and hasattr(latest_feat, "get"):
        ...
        regime_str = (
            f"{regime_map.get(regime_code, 'DESCONHECIDO')} ({regime_conf:.0%})"
        )
    else:
...
def log_trade_decision(signal, explainer, ai_monitor=None):
    ...
    if explainer is not None and latest_features is not None:
        ...
        reason = (
            signal.explanation.get("reason", "N/A") if signal.explanation else "N/A"
        )
        explanation = {
            "decision": action_str,
            "confidence": f"{signal.confidence:.2%}",
    ...
    status_icon = (
        "⚖️" if signal.action == Action.HOLD else ("🚀" if action_str == "BUY" else "🔻")
    )
    ...
    if ai_monitor is not None:
        ...
        tape = system_state.get("tape_pulse", {})
        regime = (
            int(latest_features.get("regime", 2)) if latest_features is not None else -1
        )
        ...
        context = {
            ...
            "price": float(latest_features.get("close", 0))
            if latest_features is not None
            else 0,
            "specialist": signal.explanation.get("specialist", "N/A")
...
async def check_profitability_condition(components: Dict[str, Any]) -> bool:
    ...
    """
    logger.info(
        "🔎 [PRÉ-VOO] Verificando lucratividade e risco da estratégia via backtest..."
    )
    ...
    try:
        ...
        logger.info(
            "⚙️ [PRÉ-VOO] Tentando carregar DataFrame de features pré-processado para acelerar o backtest..."
        )
        ...
        if os.path.exists(base_featured_path):
            try:
                ...
                logger.info(
                    f"✅ [PRÉ-VOO] DataFrame pré-processado carregado com sucesso! Shape: {featured_df.shape}"
                )
                ...
                if len(backtest_df) >= 100:  # Mínimo de 100 linhas para backtest válido
                    logger.info(
                        f"📊 [PRÉ-VOO] Slice de backtest do cache: {len(backtest_df)} linhas de {start_date.strftime('%Y-%m-%d')} a {end_date.strftime('%Y-%m-%d')} (últimos 180 dias)"
                    )
                else:
                    logger.warning(
                        "⚠️ [PRÉ-VOO] Dados pré-processados insuficientes. Buscando dados históricos..."
                    )
                    featured_df = None

            except Exception as e:
                logger.warning(
                    f"⚠️ [PRÉ-VOO] Erro ao carregar DataFrame pré-processado: {e}. Buscando dados históricos..."
                )
                featured_df = None
        ...
        if featured_df is None:
            logger.info(
                "⚙️ [PRÉ-VOO] Coletando dados históricos multi-timeframe para o backtest de validação..."
            )
            from datetime import datetime, timedelta, timezone
            ...
            if not historical_data_multi_tf:
                logger.error(
                    "❌ [PRÉ-VOO] Falha ao coletar dados históricos multi-timeframe para o backtest. Condição REPROVADA."
                )
                return False

            logger.info(
                "⚙️ [PRÉ-VOO] Recalculando features para o dataset de backtest para garantir consistência..."
            )
        ...
        if featured_df is None or featured_df.empty:
            logger.error(
                "❌ [PRÉ-VOO] Falha ao gerar features para o backtest. Condição REPROVADA."
            )
            return False
        ...
        if "backtest_df" not in locals():
            ...
            backtest_df = featured_df.loc[start_date:end_date]
            logger.info(
                f"📊 [PRÉ-VOO] Slice de backtest calculado: {len(backtest_df)} linhas de {start_date.strftime('%Y-%m-%d')} a {end_date.strftime('%Y-%m-%d')} (últimos 180 dias)"
            )
        ...
        run_kwargs = {
            "historical_data_multi_tf": {
                TradingConfig.PRIMARY_TIMEFRAME_TRADING: backtest_df
            },
        ...
        if (
            result_obj
            and result_obj.total_return_pct is not None
            and result_obj.max_drawdown_pct is not None
        ):
            ...
            logger.info(
                f"📊 [PRÉ-VOO] Resultado: Retorno={return_pct:.2%}, Drawdown={drawdown_pct:.2%}"
            )
            logger.info(
                f"📊 [PRÉ-VOO] Métricas: Profit Factor={profit_factor:.2f}, Sharpe={sharpe_ratio:.2f}, Avg Leverage={avg_leverage:.2f}"
            )

            if (
                meets_return_criteria
                and meets_drawdown_criteria
                and meets_profit_factor_criteria
                and meets_sharpe_criteria
                and meets_leverage_criteria
            ):
                logger.info(
                    "✅ [PRÉ-VOO] CONDIÇÃO DE LUCRATIVIDADE E RISCO ATENDIDA (critérios rigorosos)."
                )
                return True, result_obj
            else:
                failed_criteria = []
                if not meets_return_criteria:
                    failed_criteria.append("Retorno insuficiente")
                if not meets_drawdown_criteria:
                    failed_criteria.append("Drawdown excessivo")
                if not meets_profit_factor_criteria:
                    failed_criteria.append("Profit Factor baixo")
                if not meets_sharpe_criteria:
                    failed_criteria.append("Sharpe Ratio baixo")
                if not meets_leverage_criteria:
                    failed_criteria.append("Alavancagem alta")

                logger.warning(
                    f"⚠️ [PRÉ-VOO] Critérios não atendidos: {', '.join(failed_criteria)}"
                )
                return False, result_obj

    except Exception as e:
        logger.error(
            f"❌ [PRÉ-VOO] Erro na verificação de lucratividade: {e}", exc_info=True
        )
...
async def reconcile_positions(
    ...
    try:
        ...
        balance_discrepancy_pct = (
            (balance_discrepancy / real_balance * 100) if real_balance > 0 else 0
        )

        if balance_discrepancy_pct > 5.0:  # Discrepância maior que 5%
            logger.info(
                f"🔄 [RECONCILIATION] Sincronizando saldo: Simulado=${simulated_balance:,.2f} → Real=${real_balance:,.2f}"
            )

            # Corrige o saldo simulado
            portfolio.cash = real_balance - sum(
                pos.quantity * pos.entry_price for pos in simulated_positions.values()
            )
            # Força atualização do histórico para refletir o novo saldo imediatamente
            portfolio.update_portfolio_value({})
            logger.info(
                f"✅ [RECONCILIATION] Saldo simulado corrigido para ${portfolio.cash:,.2f}"
            )
        ...
        if real_position and simulated_position:
            ...
            if quantity_discrepancy > 0.001 or price_discrepancy > 0.01:  # Tolerâncias
                ...
                logger.warning(
                    f"   Quantidade: Simulado={simulated_position.quantity:.6f}, Real={real_quantity:.6f}"
                )
                logger.warning(
                    f"   Preço: Simulado=${simulated_position.entry_price:.2f}, Real=${real_entry_price:.2f}"
                )
                ...
                simulated_position.liquidation_price = float(
                    real_position.get("liquidationPrice", 0.0) or 0.0
                )
                simulated_position.mark_price = float(
                    real_position.get("markPrice", 0.0) or 0.0
                )
        ...
        for symbol, real_pos in real_positions.items():
            if symbol not in simulated_positions and real_pos.get("quantity", 0) != 0:
                logger.warning(
                    f"⚠️ [RECONCILIATION] Posição real {symbol} não encontrada no portfólio simulado. Adicionando..."
                )
                ...
                new_position = Position(
                    ...
                    liquidation_price=float(
                        real_pos.get("liquidationPrice", 0.0) or 0.0
                    ),
        ...
        for symbol, sim_pos in list(simulated_positions.items()):
            if (
                symbol not in real_positions and abs(sim_pos.quantity) > 0
            ):  # Fix: check quantity > 0
                logger.warning(
                    f"⚠️ [RECONCILIATION] Posição simulada {symbol} não existe na realidade. Removendo..."
                )
        ...
        logger.info(
            "✅ [RECONCILIATION] Reconciliação de posições concluída com sucesso!"
        )

    except Exception as e:
        logger.error(
            f"❌ [RECONCILIATION] Erro durante reconciliação: {e}", exc_info=True
        )


async def ensure_trailing_stops_for_existing_positions(
    connector, positions: Dict[str, Any], config
) -> None:
    """
    ...
    try:
        ...
        logger.info(
            f"🔍 [TRAILING STOP] Verificando trailing stops para {len(positions)} posição(ões)..."
        )
        ...
        for symbol, pos_data in positions.items():
            ...
            if has_trailing:
                logger.info(
                    f"✅ [TRAILING STOP] {symbol} já possui trailing stop ativo."
                )
                continue

            # Precisa colocar trailing stop
            logger.warning(
                f"⚠️ [TRAILING STOP] {symbol} NÃO possui trailing stop! Colocando agora..."
            )
            ...
            if result:
                logger.info(
                    f"✅ [TRAILING STOP] Trailing stop colocado para {symbol} ({trailing_side} @ {callback_rate}%)"
                )
            else:
                logger.error(
                    f"❌ [TRAILING STOP] Falha ao colocar trailing stop para {symbol}"
                )

    except Exception as e:
        logger.error(
            f"❌ [TRAILING STOP] Erro ao verificar trailing stops: {e}", exc_info=True
        )


async def check_funds_condition(
    components: Dict[str, Any], min_balance_usd: float = 100.0
) -> bool:
    """Verifica se há fundos suficientes na conta de produção."""
    logger.info(
        f"🔎 [PRÉ-VOO] Verificando fundos na conta (mínimo: ${min_balance_usd})..."
    )
    try:
        ...
        logger.error(
            f"❌ [PRÉ-VOO] Erro ao verificar fundos da conta: {e}", exc_info=True
        )
...
async def initialize_all_components():
    ...
    try:
        ...
        logger.info(
            "💹 [SETUP] Trading Connector inicializado (modo definido pelo .env)."
        )
        ...
        system_components["data_provider"] = DataProvider(
            data_connector, system_components["feature_pipeline"]
        )
        system_components["portfolio"] = PortfolioOptimizer(trading_config)
        system_components["risk_manager"] = RiskManager(trading_config)
        # [VALIDATOR FIX] RiskLayer: flash crash detection + intraday kill-switch.
        # Previously dead code — now integrated into production loop.
        system_components["risk_layer"] = RiskLayer(
            initial_capital=trading_config.INITIAL_CAPITAL,
            config=RiskConfig(
                max_drawdown_intraday=getattr(
                    trading_config, "MAX_DRAWDOWN_INTRADAY", 0.05
                ),
                max_drawdown_total=getattr(
                    trading_config, "MAX_DRAWDOWN_PERCENT", 0.15
                ),
                kelly_fraction=getattr(trading_config, "KELLY_FRACTION", 0.25),
                max_position_size=getattr(
                    trading_config, "MAX_POSITION_SIZE_PERCENT", 0.15
                ),
            ),
        )
        system_components["ai_controller"] = AIController(
            ai_config, trading_config, system_state
        )
        system_components["onchain_engine"] = OnChainEngine(active_config)
        system_components[
            "onchain_engine"
        ].start()  # Inicia imediatamente para carregar o valor real (29)

        # TapeEngine usa data_connector para dados reais de mercado
        system_components["tape_engine"] = TapeEngine(
            data_connector, [trading_config.PRIMARY_PAIR]
        )
        system_components["ai_monitor"] = AIMonitor(log_dir="logs/ai_events")
        system_components["drift_detector"] = None
        ...
        if background_df is None and os.path.exists(base_df_path):
            logger.info(
                f"ℹ️ [SHAP] Usando '{base_df_path}' como fallback para background."
            )
            try:
        ...
        if background_df is not None:
            ...
            if not base_cols:
                ...
                metadata_path = os.path.join(
                    ai_config.MODEL_DIR, "training_metadata.json"
                )
                if os.path.exists(metadata_path):
                    try:
                        ...
                        if base_cols:
                            logger.info(
                                f"📋 [SHAP] base_feature_columns carregado de metadata ({len(base_cols)} features)"
                            )
                    except Exception as e:
            ...
            if base_cols:
                ...
                if (
                    hasattr(ai_controller, "feature_pipeline")
                    and ai_controller.feature_pipeline.temporal_autoencoder_pipeline
                ):
                    try:
                        logger.info(
                            "🧪 [SHAP] Enriquecendo background com Hidden Features do Autoencoder..."
                        )
                        ae = (
                            ai_controller.feature_pipeline.temporal_autoencoder_pipeline
                        )
                        if ae.is_trained:
                            # Garante que temos as colunas base para o AE
                            ae_input_cols = (
                                ae.feature_cols
                                if hasattr(ae, "feature_cols")
                                else base_cols
                            )
                            available_ae_cols = [
                                c for c in ae_input_cols if c in background_df.columns
                            ]

                            if len(available_ae_cols) >= len(ae_input_cols) * 0.8:
                                # Aplica AE para gerar as 25-32 colunas latentes
                                # [HYBRID v3 FIX] HybridAutoencoderPipeline usa apply_hidden_features_temporal
                                # (apply_hidden era metodo do TemporalAutoencoder legado, ja removido)
                                hidden_df = ae.apply_hidden_features_temporal(
                                    background_df
                                )
                                # Adiciona à lista de colunas que o SHAP deve monitorar
                                latent_cols = [
                                    c
                                    for c in hidden_df.columns
                                    if "hidden_feature" in c
                                ]
                                ...
                                ai_controller.base_feature_columns = list(
                                    background_df.columns
                                )
                                logger.info(
                                    f"✅ [SHAP] Background enriquecido: {len(latent_cols)} Hidden Features adicionadas."
                                )
                    except Exception as ae_err:
                        logger.warning(
                            f"⚠️ [SHAP] Falha ao enriquecer background com AE: {ae_err}"
                        )
                ...
                if len(available_cols) > 0:
                    ...
                    logger.info(
                        f"✅ [SHAP] Background filtrado para {len(available_cols)} features"
                    )
                else:
                    _n_shap = getattr(ai_controller, "final_feature_count", 62)
                    logger.info(
                        f"ℹ️ [SHAP] Colunas base não encontradas. Usando primeiras {_n_shap} colunas."
                    )
                ...
                _n_shap = getattr(ai_controller, "final_feature_count", 62)
                logger.info(
                    f"ℹ️ [SHAP] base_feature_columns não encontrado. Usando primeiras {_n_shap} colunas."
                )
                background_df = background_df.iloc[:, :_n_shap]
            ...
            explainer_instance = Explainer(
                ai_controller, background_df, production_mode=production_mode
            )
            ...
            logger.warning(
                "⚠️ [SETUP] Nenhum background válido encontrado (shap_background.pkl ou base_featured_df.pkl). SHAP desativado."
            )
            system_components["explainer"] = (
                None  # Define como None se não puder ser criado
            )
        ...
        system_components["execution_engine"].set_risk_manager(
            system_components["risk_manager"]
        )
        ...
        logger.critical(
            f"🚨 [CRÍTICO SETUP] Falha crítica na inicialização: {e}", exc_info=True
        )
        shutdown_event.set()


def manage_validation_stamp(
    action: str, ai_controller: "AIController", backtest_result: Optional[Any] = None
) -> bool:
    """
    ...
    stamp_path = os.path.join(
        ai_controller.config_ai.MODEL_DIR, "validation_stamp.json"
    )
    metadata_path = os.path.join(
        ai_controller.config_ai.MODEL_DIR, "training_metadata.json"
    )
    ...
    if action == "check":
        ...
        if ai_controller.last_trained_date:
            ...
            try:
                ...
                if training_date_str:
                    logger.info(
                        f"📋 [PRÉ-VOO] Data de treino obtida de metadata: {training_date_str[:19]}"
                    )
            except Exception as e:
        ...
        if not os.path.exists(stamp_path):
            logger.info(
                "ℹ️ [PRÉ-VOO] Selo de validação não encontrado. Backtest de verificação é necessário."
            )
            return False
        ...
        try:
            ...
            if training_date_str and last_validated_training_date == training_date_str:
                logger.info(
                    "✅ [PRÉ-VOO] Selo de validação encontrado e corresponde aos modelos atuais. Backtest de verificação pulado."
                )
                return True
            elif validation_timestamp:
                try:
                    stamp_time = datetime.fromisoformat(
                        validation_timestamp.replace("Z", "+00:00")
                    )
                    age_days = (
                        ...
                        - stamp_time.replace(
                            ...
                            else stamp_time.tzinfo
                        )
                    ).days
                    if age_days <= 7:
                        logger.info(
                            f"🎫 [PRÉ-VOO] Selo de validação recente ({age_days} dias). FAST RESTART ativado - pulando backtest."
                        )
                        return True
                    else:
                        logger.warning(
                            f"⚠️ [PRÉ-VOO] Selo de validação expirado ({age_days} dias > 7). Novo backtest necessário."
                        )
                        return False
                except Exception as parse_err:
                    logger.warning(
                        f"⚠️ [PRÉ-VOO] Erro ao parsear data do selo: {parse_err}"
                    )

            logger.warning(
                "⚠️ [PRÉ-VOO] Modelos foram retreinados desde a última validação. Novo backtest de verificação é necessário."
            )
            return False
        except Exception as e:
            logger.error(
                f"❌ [PRÉ-VOO] Erro ao ler o selo de validação: {e}. Forçando novo backtest."
            )
            return False
        ...
        if training_date_str and backtest_result:
            ...
            try:
                ...
                logger.info(
                    f"✅ [PRÉ-VOO] Selo de validação criado com sucesso para o treino de {training_date_str[:19]}."
                )
                return True
            except Exception as e:
...
async def main_trading_loop():
    ...
    # [MELHORIA-04] Circuit breaker: desativa novas entradas quando LearningMonitor detecta degradação
    ...
    if not tape_engine.is_running:
        logger.warning(
            "[LOOP] TapeEngine não estava rodando — iniciando como fallback."
        )
        await tape_engine.start()
    onchain_engine = components.get("onchain_engine")
    if onchain_engine and not onchain_engine.is_running:
        onchain_engine.start()
    ...
    while not shutdown_event.is_set():
        ...
        try:
            ...
            onchain_pulse = (
                onchain_engine.get_onchain_sentiment_signal()
                if onchain_engine
                else None
            )
            system_state["onchain_pulse"] = onchain_pulse or {
                "signal": "NEUTRAL",
                "confidence": 0.1,
                "score": 50,
            }
            ...
            featured_df = await data_provider.get_latest_features(
                symbol,
                tape_metrics=system_state.get("tape_pulse"),
                sentiment_metrics=onchain_pulse,
            )
            if featured_df is None or featured_df.empty:
                logger.warning(
                    "⚠️ [LOOP] Falha ao gerar features para os dados recentes."
                )
            ...
            risk_manager.update_portfolio_state(
                ...
                portfolio.cash,
                portfolio.positions,
                portfolio.total_notional_value,
                portfolio.margin_used,
            )

            # [VALIDATOR FIX] RiskLayer: kill-switch + flash crash protection
            _risk_layer = system_components.get("risk_layer")
            if _risk_layer is not None:
                # Check if kill-switch is active (cooldown period)
                if _risk_layer.check_kill_switch():
                    logger.warning(
                        "🛑 [RISK_LAYER] Kill-switch ativo. Pulando iteração."
                    )
                    await asyncio.sleep(15)
                    continue
                # Update equity and check intraday drawdown
                _rl_result = _risk_layer.update_equity(portfolio.get_total_value())
                if _rl_result.get("kill_switch_active"):
                    logger.critical(
                        "🛑 [RISK_LAYER] KILL SWITCH ATIVADO — DD intraday=%.2f%%. "
                        "Pausa de 24h.",
                        _rl_result["drawdown"] * 100,
                    )
                    if telegram:
                        asyncio.create_task(
                            telegram.send_alert(
                                f"🛑 KILL SWITCH: DD intraday {_rl_result['drawdown']:.1%}"
                            )
                        )
                    await asyncio.sleep(15)
                    continue
                # Flash crash detection on recent returns
                if "close" in featured_df.columns and len(featured_df) >= 100:
                    _returns_window = (
                        featured_df["close"].pct_change().dropna().tail(100)
                    )
                    if _risk_layer.check_flash_crash(_returns_window):
                        logger.warning(
                            "⚠️ [RISK_LAYER] Flash crash detectado. Bloqueando entradas."
                        )
                        system_state["flash_crash_block"] = True
                    else:
                        system_state["flash_crash_block"] = False
                ...
                try:
                    ...
                    if r_prof.get("is_risk_off_mode"):
                        ...
                        if telegram:
                            ...
                            _lev = portfolio.get_detailed_status().get(
                                "total_leverage_ratio", 0
                            )
                            asyncio.create_task(telegram.alert_risk_off(_dd, _lev))
                    elif (
                        r_prof.get("current_drawdown_pct", 0)
                        > r_prof.get("max_drawdown_limit_pct", 0.1) * 0.7
                    ):
            ...
            # [MELHORIA-04] Circuit breaker: verifica saúde do aprendizado a cada 50 iterações
            _cb_check_counter += 1
            if _cb_check_counter % 50 == 0:
                _lm = getattr(ai_controller, "learning_monitor", None)
                if _lm is not None and _lm.should_abort_trial():
                    _circuit_breaker_active = True
                    _reason = _lm.get_abort_reason()
                    logger.critical(
                        "🚨 [CIRCUIT BREAKER] LearningMonitor ativou parada de novas entradas: %s",
                        _reason,
                    )
                    if telegram:
                        asyncio.create_task(
                            telegram.alert_error(
                                f"Circuit Breaker: {_reason}", level="CRITICAL"
                            )
                        )
                else:
                    _circuit_breaker_active = False
            if ai_controller.is_trained:
                ...
                log_trade_decision(
                    signal, explainer, ai_monitor
                )  # [XAI] explica + persiste
                ...
                if telegram and signal:
                    ...
                    telegram.log_decision(
                        ...
                        confidence=signal.confidence,
                        regime=_regime_map3.get(
                            int(latest_features.get("regime", 2)), "UNKNOWN"
                        ),
                    )

                if (
                    signal
                    and signal.action != Action.HOLD
                    and not _circuit_breaker_active
                    and not system_state.get("flash_crash_block", False)
                ):
                ...
                if narrator and signal:
                    try:
                        ...
                        narrator_ctx = {
                            ...
                            "top_features": signal.explanation.get(
                                "main_contributors", []
                            ),
                        }
                        ...
                        if portfolio.positions:
                            ...
                            if current_price and pos.entry_price > 0:
                                pnl = (
                                    float(current_price) - pos.entry_price
                                ) * pos.quantity
                            else:
            ...
            time_since_last_adaptation = (
                current_time - last_adaptation_time
            ).total_seconds() / 3600
            ...
            should_adapt = (
                ...
                and len(adaptation_data_buffer) >= 100  # Mínimo de dados para adaptação
                and ai_controller.is_trained
            )

            if should_adapt:
                try:
                    logger.info(
                        f"🧠 [META-LEARNING] Iniciando ciclo de adaptação com {len(adaptation_data_buffer)} pontos de dados..."
                    )
                    ...
                    if adaptation_success:
                        ...
                        logger.info(
                            "✅ [META-LEARNING] Adaptação concluída com sucesso!"
                        )

                        # Atualiza o estado do sistema
                        system_state["last_adaptation"] = current_time.isoformat()
                        system_state["adaptation_count"] = (
                            system_state.get("adaptation_count", 0) + 1
                        )
                    else:
                        logger.warning(
                            "⚠️ [META-LEARNING] Adaptação falhou. Tentará novamente no próximo ciclo."
                        )

                except Exception as e:
                    logger.error(
                        f"❌ [META-LEARNING] Erro durante adaptação: {e}", exc_info=True
                    )
                    # Não limpa o buffer em caso de erro para tentar novamente

        except Exception as e:
            logger.critical(
                f"🚨 [CRÍTICO LOOP] Erro fatal no loop de trading: {e}", exc_info=True
            )
            system_state["system_health"] = "critical"
            # [TELEGRAM] Alerta imediato para erro crítico no loop
            if telegram:
                asyncio.create_task(
                    telegram.alert_error(str(e)[:200], level="CRITICAL")
                )
        ...
        try:
            ...
            if portfolio := system_components.get("portfolio"):
                log_portfolio_status(
                    "Portfólio de Papel", portfolio.get_detailed_status()
                )

            if (
                system_state["live_trading_enabled"]
                and system_state["binance_connection"] == "conectado"
            ):
                if connector := system_components.get("binance_connector"):
                    ...
                    if portfolio:
                        ...
                        for s, p_data in real_data.get("positions", {}).items():
                            if s in portfolio.positions:
                                portfolio.positions[s].liquidation_price = float(
                                    p_data.get("liquidation_price", 0.0) or 0.0
                                )
                                portfolio.positions[s].mark_price = float(
                                    p_data.get("mark_price", 0.0) or 0.0
                                )
                    ...
                    current_time = datetime.utcnow()
                    time_since_last_reconciliation = (
                        current_time - last_reconciliation_time
                    ).total_seconds() / 3600
            ...
            if _telegram:
                ...
                if _tape.get("pulse") == "WARMUP...":
                    if _tape_warmup_since is None:
                        ...
                        _elapsed = (
                            datetime.utcnow() - _tape_warmup_since
                        ).total_seconds()
            ...
            logger.error(
                f"❌ [MONITOR] Erro no loop de monitoramento: {e}", exc_info=True
            )
...
async def main():
    ...
    try:
        ...
        if trading_connector:
            ...
            logger.info(
                f"✅ [CONEXÃO] Trading Connector ({mode}) conectado com sucesso."
            )

        system_state["binance_connection"] = "conectado"
    except Exception as e:
        logger.critical(
            f"🚨 [CRÍTICO] Falha ao conectar à Binance: {e}. Encerrando.", exc_info=True
        )
        return
    ...
    try:
        ...
        if (
            trading_connector
            and portfolio
            and system_state.get("binance_connection") == "conectado"
        ):
            logger.info(
                "🔄 [INIT] Sincronizando portfólio com o saldo real da Binance..."
            )
            ...
            if real_balance > 0:
                ...
                if real_positions:
                    logger.info(
                        f"🔄 [INIT] Importando {len(real_positions)} posições abertas da exchange..."
                    )
                ...
                logger.info(
                    f"✅ [INIT] Portfólio sincronizado! Saldo Real: ${real_balance:,.2f} | Disponível: ${real_cash:,.2f}"
                )
            else:
                logger.warning(
                    "⚠️ [INIT] Saldo retornado pela Binance é Zero ou inválido. Usando valor padrão/paper trading."
                )
        ...
        if "Unauthorized" in str(e) or "401" in str(e):
            logger.warning(
                "⚠️ [INIT] Falha de autenticação na Binance (401). Verifique suas chaves API. O bot continuará, mas o trading real pode falhar."
            )
        else:
            logger.error(
                f"❌ [INIT] Falha ao sincronizar portfólio com Binance: {e}",
                exc_info=False,
            )
    # -------------------------------------------------------------------------
    ...
    if ai_controller.is_retraining_due(AIConfig.MODEL_RETRAIN_DAYS):
        ...
        if valid_models_count == 0:
            logger.warning(
                f"🗓️ RETREINAMENTO COMPLETO: Nenhum modelo válido encontrado. Iniciando reconstrução total."
            )
            ...
            if drift_detector is not None:
                drift_detector.set_reference_data(
                    final_training_df, ai_controller.base_feature_columns
                )
        else:
            logger.info(
                f"ℹ️ Modelos antigos detectados, mas há {valid_models_count}/{total_models} modelos válidos. Treinamento granular recomendado."
            )
            needs_training_this_session = True
            ...
            if not success:
                logger.critical(
                    "🚨 Falha no treinamento granular dos componentes ausentes. Encerrando."
                )
                return
    ...
    # Cenário 2: Se NÃO for um retreinamento por idade, verifica se algo está faltando
        ...
        logger.warning(
            f"⚠️ ESTADO INCONSISTENTE: {valid_models_count}/{total_models} modelos válidos. Executando treinamento granular para corrigir."
        )
        needs_training_this_session = True
        ...
        logger.info(
            "🔄 [RELOAD] Treinamento granular concluído. Recarregando modelos para operação..."
        )
        try:
            ai_controller.load_all_models()
        except Exception as e:
            logger.critical(
                f"🚨 Falha crítica ao recarregar modelos após treinamento: {e}",
                exc_info=True,
            )
            return
    ...
    # Cenário 3: Se NENHUM dos cenários acima for verdadeiro, significa que tudo está pronto
        ...
        logger.info(
            "✅ [CARREGAMENTO] Modelos estão atualizados e consistentes. Carregando pesos..."
        )
        try:
            ai_controller.load_all_models()
        except Exception as e:
    ...
    if not ai_controller.is_trained:
        ...
        if TradingConfig.ALLOW_UNTRAINED_EXECUTION:
            logger.warning(
                "⚠️ [ALERTA DE SEGURANÇA] Bot operando em modo SEM TREINAMENTO (BYPASS ATIVO). Resultados podem ser aleatórios!"
            )
        else:
            logger.critical(
                "🚨 Carregamento ou treinamento final dos modelos falhou. O bot não pode operar. Encerrando."
            )
            return

    logger.info(
        "✅ [IA PRONTA] Todas as fases de treinamento/carregamento foram concluídas."
    )
    ...
    system_state["ai_status"] = (
        "pronto" if ai_controller.is_trained else "bypass (não treinado)"
    )
    ...
    if "--force-backtest" in sys.argv or "--backtest" in sys.argv:
        ...
        backtest_passed, backtest_result_obj = await check_profitability_condition(
            system_components
        )
        if backtest_passed:
    ...
    mode_label = (
        "LIVE (REAL)" if not TradingConfig.BINANCE_TESTNET else "TESTNET (REAL API)"
    )
    ...
    if _telegram:
        ...
        _bal = (
            system_components["portfolio"].get_total_value()
            if system_components.get("portfolio")
            else 0.0
        )
    ...
    try:
        ...
        system_components["state_restore"].save_checkpoint(
            state_to_save, "startup_state"
        )
    except Exception as e:
...
async def shutdown_gracefully():
    ...
    if not shutdown_event.is_set():
        ...
        try:
            if system_components.get("state_restore") and system_components.get(
                "portfolio"
            ):
                ...
                # Converter histórico
                hist_dict = {
                    k.isoformat(): v for k, v in p.portfolio_value_history.items()
                }
        ...
        try:
            if ee := system_components.get("execution_engine"):
                await ee.stop()
            if te := system_components.get("tape_engine"):
                await te.stop()
            ...
            logger.critical(
                f"🚨 [CRÍTICO] Erro na parada de componentes: {e}", exc_info=True
            )
    ...
    for sig in (signal.SIGINT, signal.SIGTERM):
        try:
            loop.add_signal_handler(
                sig, lambda s=sig: asyncio.ensure_future(shutdown_gracefully())
            )
