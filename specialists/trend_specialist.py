...
TENSORBOARD_LOG_DIR_TREND = os.path.join(_TREND_PROJECT_ROOT, "logs", "tensorboard", "TrendSpecialist")
...
def _setup_gpu_optimizations():
    ...
    try:
        ...
        tf32_status = "ON (Ampere+)" if tf32_supported else f"OFF (CC {props.major}.{props.minor}, requer CC>=8.0)"
        logger.info(
...
class ClippedSAC(BaseSAC):
    ...
    def train(self, gradient_steps: int, batch_size: int = 64) -> None:
        ...
        for gradient_step in range(gradient_steps):
            ...
            if use_amp:
                ...
                if self.max_grad_norm is not None:
                    ...
                    clip_grad_norm_(self.critic.parameters(), max_norm=min(self.max_grad_norm, 0.5))
            ...
            if hasattr(self.actor, 'log_std') and self.actor.log_std is not None:
                if isinstance(self.actor.log_std, torch.nn.Parameter):
                    with torch.no_grad():
                        ...
                        self.actor.log_std.data = torch.max(self.actor.log_std.data, min_log_std)
        ...
        if len(ent_coef_losses) > 0:
            self.logger.record("train/ent_coef_loss", torch.stack(ent_coef_losses).mean().item())
        ...
        if self.learning_monitor is not None:
            try:
                ...
                if hasattr(self.actor, 'log_std') and self.actor.log_std is not None:
                    if isinstance(self.actor.log_std, torch.nn.Parameter):
                        with torch.no_grad():
                            mean_std = float(torch.exp(self.actor.log_std.data).mean().item())
...
class TrendFollowingEnv(gym.Env):
    ...
    def __init__(
        ...
        if not isinstance(df, pd.DataFrame) or df.empty:
            raise ValueError("Ã°Å¸Å¡Â¨ [ERRO ENV] DataFrame de entrada nÃƒÂ£o pode ser vazio.")
        if not isinstance(df.index, pd.DatetimeIndex):
            raise TypeError("Ã°Å¸Å¡Â¨ [ERRO ENV] O ÃƒÂ­ndice do DataFrame deve ser do tipo DatetimeIndex.")
        self.df = df.copy()
        ...
        if ema_candidate in self.df.columns:
            ...
            fallback = next((col for col in self.df.columns if isinstance(col, str) and col.startswith('ema_trend_')), None)
        ...
        if mode == 'optimization':
            ...
            self._leverage_cap_hpo = min(self.trading_config.MAX_LEVERAGE_PER_TRADE, 2.0)
        ...
        self.action_space = spaces.Box(
            low=np.array([-1.0, self.trading_config.SL_DIST_MULTIPLIER_MIN, self.trading_config.MIN_LEVERAGE_PER_TRADE]),
            high=np.array([1.0, self.trading_config.SL_DIST_MULTIPLIER_MAX, self._leverage_cap_hpo]),
            dtype=np.float32
        )
        ...
        for window in [20, 50]:
            roll_max = self.df['high'].rolling(window).max()
            roll_min = self.df['low'].rolling(window).min()
            # Evita divisão por zero e preenche NaNs
            self.df[f'dist_to_max_{window}'] = ((self.df['close'] - roll_max) / (roll_max + 1e-9)).fillna(0.0)
            self.df[f'dist_to_min_{window}'] = ((self.df['close'] - roll_min) / (roll_min + 1e-9)).fillna(0.0)
        ...
        extras_not_in_numeric = [k for k in self.extra_feature_keys if k not in self.numeric_df.columns]
        ...
        if feature_columns is None:
            ...
            self.feature_columns: List[str] = self._filter_to_core_features(all_filtered)
        else:
        ...
        try:
            ...
            self._rnp_values = _num_df.to_numpy(dtype=np.float32, copy=False, na_value=np.nan)
        ...
        self.post_trade_cooldown_base = 0  # ZERO para permitir reversões imediatas (flip)
        ...
        self.scalp_penalty_min_duration = 20  # Aumentar de 6 para 20 candles (anti-scalping)
        ...
        self._sl_volatile_min = float(getattr(self.trading_config, 'SL_DIST_MULTIPLIER_MIN_VOLATILE', self._sl_min_base))
        self._sl_volatile_max = float(getattr(self.trading_config, 'SL_DIST_MULTIPLIER_MAX_VOLATILE', self._sl_max_base))
        self.unrealized_reward_scale = float(getattr(self.config, "TREND_UNREALIZED_REWARD_SCALE", 6.0))
        self.realized_reward_scale = float(getattr(self.config, "TREND_REALIZED_REWARD_SCALE", 40.0))
        self.drawdown_penalty_weight = float(getattr(self.config, "TREND_DRAWDOWN_PENALTY_WEIGHT", 0.35))
        self.prior_flip_penalty_weight = float(getattr(self.config, "TREND_PRIOR_FLIP_PENALTY_WEIGHT", 0.1))  # [FIX E] Reduzido 0.6→0.1: penalty de 1.32/step dominava rewards de trades lucrativos
        self.prior_flip_penalty_exponent = float(getattr(self.config, "TREND_PRIOR_FLIP_PENALTY_EXP", 1.0))
        self.prior_flip_threshold = float(getattr(self.config, "TREND_PRIOR_FLIP_THRESHOLD", 0.3))
        self.duration_pressure_weight = float(getattr(self.config, "TREND_DURATION_PRESSURE_WEIGHT", 0.35))
        self.duration_pressure_start = int(getattr(self.config, "TREND_DURATION_PRESSURE_START", 90))
        self.duration_pressure_exponent = float(getattr(self.config, "TREND_DURATION_PRESSURE_EXP", 1.0))
        ...
        self._reward_norm_clip = 20.0  # [FIX] Alinhado com scientific_corrections.py clip final [-20,20].
        # Antes: 10.0 descartava metade do sinal negativo desenhado pela scientific reward.
        # O SAC nunca via penalidades > -10 mesmo quando a scientific gerava -15 a -20.
        ...
        if feature_scaler is not None:
            self._feature_scaler = feature_scaler
            logger.debug("[SCALER] Usando scaler externo (já fit no treino) para evitar data leakage.")
    ...
    def set_anti_scalping_config(self, config_params: Dict[str, Any]):
        ...
        try:
            self.scalp_penalty_min_duration = config_params.get('min_trade_duration', self.scalp_penalty_min_duration)
            self.long_trade_bonus_threshold = config_params.get('long_trade_bonus_threshold', self.long_trade_bonus_threshold)
            self.long_trade_bonus_max = config_params.get('long_trade_bonus_max', self.long_trade_bonus_max)
            ...
            if 'trend_alignment_threshold' in config_params:
                self.trend_alignment_threshold = float(config_params['trend_alignment_threshold'])
            ...
            if 'min_action_threshold_opt' in config_params:
                self.opt_min_action_threshold = float(config_params['min_action_threshold_opt'])
            if 'prior_open_long_threshold_opt' in config_params:
                self.opt_prior_open_long_threshold = float(config_params['prior_open_long_threshold_opt'])
            if 'prior_open_short_threshold_opt' in config_params:
                self.opt_prior_open_short_threshold = float(config_params['prior_open_short_threshold_opt'])
            if 'force_threshold_factor_opt' in config_params:
                self.opt_force_threshold_factor = float(config_params['force_threshold_factor_opt'])
            if 'penalty_cap_per_step_opt' in config_params:
                self.opt_penalty_cap_per_step = float(config_params['penalty_cap_per_step_opt'])
            ...
            if 'post_trade_cooldown' in config_params:
                try:
                    self.post_trade_cooldown_base = int(config_params['post_trade_cooldown'])
                except Exception:
                    pass
            if 'max_trades_per_episode' in config_params:
                try:
                    self.max_trades_per_episode = max(1, int(config_params['max_trades_per_episode']))
                    self.trades_remaining = self.max_trades_per_episode
                except Exception:
                    pass
            if 'max_trade_duration' in config_params:
                try:
                    self.max_trade_duration_cap = max(self.scalp_penalty_min_duration, int(config_params['max_trade_duration']))
                except Exception:
                    pass
            # Cooldown removido
            self._grace_median = max(int(self.scalp_penalty_min_duration), self._grace_median)
            ...
            logger.debug(f"[FASE 1] ConfiguracaÃƒÂ§ÃƒÂ£o aplicada (sem cooldown): min_duration={self.scalp_penalty_min_duration}")
        except Exception as e:
            logger.error("[FASE 1] Erro ao aplicar configuraÃƒÂ§ÃƒÂ£o: %s" % e, exc_info=True)
    ...
    def set_phase2_runtime_tweaks(self):
        ...
        try:
            ...
            self.disable_new_entries = False
            try:
                self.phase2_bias_threshold = max(0.0, float(os.getenv("TREND_PHASE2_BIAS_THR", "0.12")))
            except Exception:
            ...
            if isinstance(regime_source, pd.DataFrame) and not regime_source.empty:
                up_mean = float(regime_source.get("tp_regime_up", pd.Series(0.0)).mean())
                down_mean = float(regime_source.get("tp_regime_down", pd.Series(0.0)).mean())
                ema_series = None
                if hasattr(self, "ema_trend_col") and self.ema_trend_col in regime_source.columns:
                    ema_series = regime_source[self.ema_trend_col]
                elif "ema_trend" in regime_source.columns:
                    ema_series = regime_source["ema_trend"]
                ema_mean = float(getattr(ema_series, "mean", lambda: 0.0)()) if ema_series is not None else 0.0
            ...
            self.phase2_disable_longs = False
            try:
                self.phase2_grace_cap_steps = max(5, int(os.getenv("TREND_PHASE2_GRACE_CAP", "24")))
            except Exception:
            ...
            try:
                relax_factor = float(os.getenv('TREND_PHASE2_MIN_DURATION_RELAX', '0.33'))
            except Exception:
                relax_factor = 0.33
            try:
                self.scalp_penalty_min_duration = max(5, int(self.scalp_penalty_min_duration * relax_factor))
            except Exception:
                pass
        except Exception:
            pass
    ...
    def set_reward_shaping_params(self, params: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if "prior_flip_penalty_weight" in params:
                self.prior_flip_penalty_weight = float(params["prior_flip_penalty_weight"])
            if "prior_flip_penalty_exponent" in params:
                self.prior_flip_penalty_exponent = float(params["prior_flip_penalty_exponent"])
            ...
            if "duration_pressure_weight" in params:
                self.duration_pressure_weight = float(params["duration_pressure_weight"])
            ...
            if "duration_pressure_exponent" in params:
                self.duration_pressure_exponent = float(params["duration_pressure_exponent"])
        except Exception as exc:
            logger.debug("[TREND ENV] Falha ao aplicar parametros de recompensa dinamicamente: %s", exc)
    ...
    def reset(self, seed: Optional[int] = None, options: Optional[Dict] = None) -> Tuple[np.ndarray, Dict[str, Any]]:
        ...
        if self.max_steps < len(self.df) - 1:
            ...
            logger.debug(f"[ENV RESET] Episódio curto: max_steps={self.max_steps}, start_idx={self.start_idx}, end_idx={self.start_idx + self.max_steps}")
            ...
            logger.debug(f"[ENV RESET] Episódio otimização (random start): start_idx={self.start_idx}, max_steps={self.max_steps}, safe_offset={safe_max_offset}")
        else:
            self.start_idx = 0
        ...
        try:
            ...
            avg_dir = float(self.df['tp_prior_dir'].mean()) if 'tp_prior_dir' in self.df.columns else 0.0
            std_dir = float(self.df['tp_prior_dir'].std()) if 'tp_prior_dir' in self.df.columns else 0.0
            
            logger.debug(f"[CHECK DADOS] Media Confianca no DF: {avg_conf:.4f} | Max Confianca: {max_conf:.4f} | Std: {std_conf:.4f}")
            ...
            if std_dir < 0.01:
                ...
                if not getattr(self, '_tp_prior_dir_warned', False):
                    ...
                    _is_ranger = 'ranger' in str(getattr(self, 'specialist_name', '')).lower()
                    if _is_ranger:
                        logger.debug(
                            "[Ranger] tp_prior_dir constante (std=%.4f) — esperado "
                            "em mercado lateral (sem viés direcional).", std_dir
                        )
                    else:
                        logger.warning(
                            "⚠️ [ALERTA] tp_prior_dir quase constante (std=%.4f). "
                            "Agente pode ter dificuldade em decidir direção.", std_dir
                        )
        except Exception as e:
    ...
    def set_trend_debug_logging(self, enabled: bool = True, level: str = "debug") -> None:
    ...
    def _get_observation(self) -> np.ndarray:
        ...
        agent_state = np.array([np.sign(self.position), self.pnl_since_entry, self.steps_in_position / 100.0], dtype=np.float32)
        ...
        # Física pré-calculada por candle (mesma função usada ao vivo)
        market_entropy = float(self._physics_entropy[_safe_idx])
        market_hurst = float(self._physics_hurst[_safe_idx])
    ...
    def _apply_realized_pnl(self, pnl_realized: float) -> None:
        ...
        # Taxa de saída cobrada em TODO fechamento (agente, stops, fim de episódio/segmento)
        self.net_worth -= float(self.initial_notional_value or 0.0) * float(self.trading_config.TAKER_FEE)
    ...
    def _filter_to_core_features(self, all_features: List[str]) -> List[str]:
        ...
        Fix v3.1:
          ...
          - ELIMINADOS: stoch, williams_r, cci, roc, candlestick patterns (ruído para RL).
        ...
        logger.debug(
            ...
            "— 1m/5m/4h excluidos",
            len(all_features), len(core),
            n_latents, n_15m, n_1h,
    ...
    def _dynamic_sl_bounds(self, atr_pct: float) -> Tuple[float, float]:
        ...
        if ratio >= 1.0:
            ...
            floor = self._sl_min_base + interp * (self._sl_volatile_min - self._sl_min_base)
            cap = self._sl_max_base + interp * (self._sl_volatile_max - self._sl_max_base)
        else:
            interp = np.clip((ratio - self._vol_ratio_low) / (1.0 - self._vol_ratio_low), 0.0, 1.0)
            floor = (self._sl_min_base * 0.6) + interp * (self._sl_min_base - (self._sl_min_base * 0.6))
            cap = (self._sl_max_base * 0.7) + interp * (self._sl_max_base - (self._sl_max_base * 0.7))
        floor = float(np.clip(floor, 0.8, self._sl_volatile_min))
        cap = float(np.clip(cap, floor + 0.5, self._sl_volatile_max))
        return floor, cap

    def _compute_catastrophe_multiplier(self, atr_pct: float, prior_dir_strength: float, position_side: int) -> float:
        ...
        return float(np.clip(base * volatility_scale * conviction_bonus * directional_bias, base * 0.7, base * 2.5))

    def _simulate_slippage(self, price: float, atr: float, action_side: OrderSide) -> float:
        """Simula slippage realista baseado em ATR e direção da ordem."""
        slippage_amount = atr * self.trading_config.SLIPPAGE_TOLERANCE_PCT
    ...
    def _compute_trade_reward(
        ...
        dur_amortization = float(np.clip(np.sqrt(duration_steps / 5.0), 0.3, 2.0))
        clamped_return = float(np.clip(trade_return_pct, -0.25, 0.25))
        reward = clamped_return * 30.0 * dur_amortization
    ...
    def _build_financial_snapshot(self, returns: List[float], durations: List[int]) -> Dict[str, float]:
        ...
        max_dd = float(self.episode_max_drawdown) if hasattr(self, 'episode_max_drawdown') else 0.0
        ...
        if isinstance(getattr(self, "regime_counters", None), dict):
            ...
            if total_regime_trades > 0.0:
                for key in ("up", "down", "side"):
                    trades_val = float(self.regime_counters.get(key, {}).get("trades", 0.0))
        ...
        trend_regime_sum = getattr(self, "_trend_regime_sum", {"up": 0.0, "down": 0.0, "side": 0.0}) or {}
        ema_trend_avg = 0.0
        dominant_regime = "none"
        trend_regime_avg = {"up": 0.0, "down": 0.0, "side": 0.0}
        if trend_sample_count > 0:
            ema_trend_avg = float(
                (getattr(self, "_ema_trend_accumulator", 0.0) or 0.0) / trend_sample_count
            )
            for key in ("up", "down", "side"):
                trend_regime_avg[key] = float(trend_regime_sum.get(key, 0.0) / trend_sample_count)
            dominant_regime = max(
                trend_regime_avg.items(), key=lambda item: item[1]
            )[0] if trend_regime_avg else "none"
        snapshot = {
            "profit_factor": profit_factor if np.isfinite(profit_factor) else 0.0,
            "profit_factor_raw": float(profit_factor_raw if np.isfinite(profit_factor_raw) else 0.0),
            ...
            "total_return_pct": float((self.net_worth - self.initial_balance) / (self.initial_balance + 1e-9)),
            "episode_length": float(self.current_step),
            "pf_reliable": float(1.0 if pf_reliable else 0.0),
            "max_trades_cap": float(getattr(self, "max_trades_per_episode", 0) or 0),  # None -> 0
        ...
        try:
            ...
            snapshot["long_trade_pct"] = float((long_ct / total_sides) * 100.0) if total_sides > 0 else 0.0
    ...
    def get_exit_reason_counts(self) -> Dict[str, int]:
        ...
        return dict(getattr(self, 'exit_reason_counts', {"agent": 0, "stop_loss": 0, "catastrophe": 0, "time_stop": 0, "other": 0}))
    ...
    def _log_episode_matrix(self) -> None:
        ...
        logger.debug(SEP)
        logger.debug("[EPISODE] Capital: $%.2f -> $%.2f  |  P&L: %s$%.2f (%s%.2f%%)",
                    initial, final, sign_u, pnl_usd, sign_p, pnl_pct)
        logger.debug("[EPISODE] Trades: %d  |  Wins: %d  |  Losses: %d  |  Win Rate: %.1f%%",
                    len(trades), wins, losses, wins / (len(trades) + 1e-9) * 100.0)
        logger.debug(SEP2)
        logger.debug("  %3s  %-5s  %5s  %12s  %12s  %10s  %7s  %s",
                    "#", "Dir", "Dur", "Entry", "Exit", "P&L USD", "P&L %", "Motivo")
        logger.debug(SEP2)
        # linhas de trades
        for t in trades:
            ...
            logger.debug("  %3d  %-5s  %5d  %12.2f  %12.2f  %s$%8.2f  %s%5.2f%%  %s %s",
                        t['#'], t['side'], t['duration'],
                        t['entry'], t['exit'],
                        su, t['pnl_usd'], sp, t['pnl_pct'],
                        marker, t['exit_reason'])
        # rodape: breakdown de motivos de saida
        exit_counts: dict = {}
        for t in trades:
            exit_counts[t['exit_reason']] = exit_counts.get(t['exit_reason'], 0) + 1
        reasons_str = "  ".join("%s:%d" % (k, v) for k, v in sorted(exit_counts.items()))
        logger.debug(SEP2)
    ...
    def _log_trial_matrix(self, trial_number: int = -1, score: float = 0.0) -> None:
        ...
        trades = getattr(self, '_episode_trades_log', [])
        if not trades:
            logger.debug("[TRIAL %d] Sem trades registrados no ultimo episodio de avaliacao.", trial_number)
            return
        initial = float(self.initial_balance)
        final   = float(self.net_worth)
        pnl_usd = final - initial
        ...
        SEP2 = "-" * 80
        logger.debug(SEP)
        logger.debug("[TRIAL %d] Score=%.4f | Capital: $%.2f -> $%.2f | P&L: %s$%.2f (%s%.2f%%)",
                    trial_number, score, initial, final, sign_u, pnl_usd, sign_p, pnl_pct)
        logger.debug("[TRIAL %d] Trades: %d | Wins: %d | Loss: %d | WinRate: %.1f%% | AvgDur: %.1f candles",
                    trial_number, len(trades), wins, losses,
                    wins / (len(trades) + 1e-9) * 100.0, avg_dur)
        logger.debug(SEP2)
        logger.debug("  %3s  %-5s  %5s  %12s  %12s  %10s  %7s  %s",
                    "#", "Dir", "Dur", "Entry", "Exit", "P&L USD", "P&L %", "Motivo")
        logger.debug(SEP2)
    ...
    def _step_training(self, action: np.ndarray) -> Tuple[np.ndarray, float, bool, bool, Dict[str, Any]]:
        if getattr(self, 'disable_new_entries', False):
            logger.warning(f"[DISABLE ENTRIES] step={self.current_step}, trades_remaining={getattr(self, 'trades_remaining', None)}")
        ...
        _prev_step = min(max(0, _raw_step - 1), len(self.df) - 1) if self.current_step > 0 else _safe_step
        prev_price = float(self._rnp_values[_prev_step][self._col_idx_map.get('close', 0)])
        ...
        if exploring:
            ...
            trend_alignment_threshold *= max(0.3, min(1.0, self.exploration_threshold_scale))
        ...
        warmup_factor = warmup_base + warmup_scale * float(min(1.0, max(0.0, self.current_step / float(max(1, steps_warmup)))))
        ...
        if self.position != 0:
            ...
            unrealized_return = ((current_price - self.entry_price) / (self.entry_price + 1e-9)) * np.sign(self.position)
        ...
        prev_timestamp = self.df.index[self.current_step - 1] if self.current_step > 0 else self.df.index[0]
        ...
        if self.position != 0:
            ...
            _ts_prev = min(self.start_idx + max(0, self.current_step - 1), len(self.df) - 1)
        ...
        if trend_strength > 0.8:
            ...
            required_conf = 0.005  # Tendência Muito Forte (>60%): Aceita confiança mínima
            ...
            required_conf = 0.30 * eval_relax  # Tendência Fraca: Exige confiança moderada
        ...
        if low_confidence_gate:
            ...
            if prior_dir_val > 0.2:
                ...
                neutral_conf_thresh = 0.2 if self.mode in ('evaluation', 'holdout') else 0.4
        ...
        if exploring:
            ...
            min_action_threshold = min(min_action_threshold * scale, warmup_vote_threshold)
            force_threshold_factor = min(force_threshold_factor, self.exploration_force_threshold_factor)

        # Ajuste do limiar com base na confiança do Prior e ATR
        if exploring:
            action_threshold = min(
                warmup_vote_threshold,
                base_threshold * (1.0 - (prior_conf_dyn * 0.2))
            )
            ...
            if self.relaxed_gates or self.mode in ('evaluation', 'holdout'):
                ...
                logger.debug("[ENV EVAL] Action threshold relaxado em 50%% (FINAL): %.3f", action_threshold)
        ...
        if warmup_override:
            ...
            should_open_long = (self.position <= 0) and (agent_vote_strength > warmup_vote_threshold)
            should_open_short = (self.position >= 0) and (agent_vote_strength < -warmup_vote_threshold)
            ...
            if getattr(self, '_specialist_short_only', False):
                should_open_long = False
            if self.mode == 'optimization' and (self.current_step % max(1, getattr(self, "exploration_warmup_log_span", 250)) == 0):
                logger.debug(
        ...
        if cooldown_active:
            ...
            agent_votes_strong_reversal = (
                (self.last_action_type < 0 and agent_vote_strength > strong_reversal_threshold) or
                (self.last_action_type > 0 and agent_vote_strength < -strong_reversal_threshold)
            )
            prior_confirms_reversal = (
                (self.last_action_type < 0 and prior_dir_val > self.opt_prior_open_long_threshold) or
                (self.last_action_type > 0 and prior_dir_val < self.opt_prior_open_short_threshold)
            )
            market_structure_allows_reversal = (
                (self.last_action_type < 0 and ema_trend_val >= -0.01) or
                (self.last_action_type > 0 and ema_trend_val <= 0.01)
            )
            high_conviction_reversal = (
                agent_votes_strong_reversal and prior_confirms_reversal and market_structure_allows_reversal
            )
            if high_conviction_reversal:
                logger.info(f"[COOLDOWN BYPASS] Sinal de reversao de alta conviccao detectado no step {self.current_step}. Cooldown ignorado.")
                cooldown_active = False
        if cooldown_active:
            should_open_long = False
            should_open_short = False
        ...
        if (self.current_step % 5000 == 0):
            try:
                ...
                vote_misaligned = (
                    (prior_dir_val > 0.5 and agent_vote_strength < -0.1) or
                    (prior_dir_val < -0.5 and agent_vote_strength > 0.1)
                ) and not reversal_opportunity

                log_level = "WARNING" if (is_golden_window or vote_misaligned) else "INFO"
        ...
        if self.mode == 'optimization':
            ...
            if should_log_gate:
                ...
                if not warmup_override and self.position == 0 and not is_opening_or_reversing:
                    entry_notes: List[str] = []
                    if agent_vote_strength <= action_threshold:
                        entry_notes.append("vote_below_threshold")
                    if agent_vote_strength > action_threshold and not trend_allows_long and agent_vote_strength > 0:
                        entry_notes.append("trend_block_long")
                    if agent_vote_strength < -action_threshold and not trend_allows_short and agent_vote_strength < 0:
                    ...
                    if entry_notes:
                        ...
                        if isinstance(getattr(self, "_gate_stats", None), Counter):
                            self._gate_stats.update(f"entry_block:{note}" for note in entry_notes)
        if self.position == 0 and (self.current_step % 200 == 0):
            status_report: List[str] = []
            if getattr(self, 'post_trade_cooldown', 0) > 0:
                status_report.append(f"Em cooldown por mais {self.post_trade_cooldown} steps.")
            luz_verde_acesa = (prior_dir_val > prior_open_long_threshold or prior_dir_val < prior_open_short_threshold)
            if not luz_verde_acesa:
                status_report.append("Aguardando 'Luz Verde' (prior do TrendPredictor fraco).")
            elif not trend_allows_long and not trend_allows_short:
                status_report.append("Aguardando alinhamento de tendencia (EMA/Regime).")
            elif abs(agent_vote_strength) < action_threshold:
                status_report.append(f"Aguardando conviccao do agente (voto: {agent_vote_strength:.2f} < thr: {action_threshold:.2f}).")
            if status_report:
                logger.debug(f"[PATIENCE REPORT] Step {self.current_step}: Sem posicao. Motivo: {' | '.join(status_report)}")
        ...
        if is_opening_or_reversing and last_position_side != 0:
            ...
            if is_reversal:
                ...
                if reversal_opportunity:
                    ...
                    predictor_confirms_flip = ((action_side == 1 and prior_dir_val > 0.2) or 
        ...
        if self.position > 0:
            ...
            if prior_flipped_long:
                prior_mismatch_ratio = float(np.clip((prior_threshold - prior_dir_val) / max(prior_threshold, 1e-6), 0.0, 5.0))
        elif self.position < 0:
            prior_flipped_short = prior_dir_val > -prior_threshold
            if prior_flipped_short:
                prior_mismatch_ratio = float(np.clip((prior_dir_val + prior_threshold) / max(prior_threshold, 1e-6), 0.0, 5.0))
        if self.position != 0:
            base_grace = int(self._active_grace_period or self.scalp_penalty_min_duration)
            if self.mode == 'optimization':
        ...
        trend_allows_current = (self.position > 0 and trend_allows_long) or (self.position < 0 and trend_allows_short)
        prior_penalty = 0.0
        ...
        if (prior_flipped_long or prior_flipped_short) and not is_in_grace_period and prior_pressure_active:
            prior_conf_scale = float(np.clip(prior_conf_dyn, 0.0, 1.0))
            exponent = float(getattr(self, "prior_flip_penalty_exponent", 1.0))
            prior_penalty = self.prior_flip_penalty_weight * (prior_conf_scale * prior_mismatch_ratio) ** exponent
            if prior_penalty > 0:
                reward -= prior_penalty
                penalty_accum += abs(prior_penalty)
                penalties_debug["prior_flip"] = float(prior_penalty)
                info["prior_mismatch_ratio"] = float(prior_conf_scale * prior_mismatch_ratio)
        max_duration_cap = getattr(self, 'max_trade_duration_cap', None)
        duration_penalty = 0.0
        if self.position != 0 and max_duration_cap:
            duration_start = int(getattr(self, "duration_pressure_start", max_duration_cap))
            ...
            if steps_over_start > 0:
                ...
                pressure_ratio = float(np.clip(steps_over_start / effective_span, 0.0, 5.0))
                duration_penalty = self.duration_pressure_weight * (pressure_ratio ** exponent)
        ...
        _EMERGENCY_MIN_STEPS = max(3, int(getattr(self, '_grace_floor', 10) * 0.3))  # mínimo absoluto
        _pnl_pct_unrealized = float(getattr(self, 'pnl_since_entry', 0.0))
        if is_in_grace_period and _pnl_pct_unrealized < -0.02 and self.steps_in_position >= _EMERGENCY_MIN_STEPS:
            is_in_grace_period = False
            logger.debug(
                "[EMERGENCY EXIT] Grace period anulado: PnL=%.4f abaixo de -2%% no step=%d (steps_in=%d >= min=%d)",
                _pnl_pct_unrealized, self.current_step, self.steps_in_position, _EMERGENCY_MIN_STEPS
            )
        ...
        if self.position != 0 and self.steps_in_position < _HARD_MIN_HOLD:
            is_in_grace_period = True  # força grace ativa — nenhuma saída voluntária permitida
            is_opening_or_reversing = False  # impede que reversão force fechamento prematuro
        ...
        if self.mode == 'optimization' and self.position != 0 and not is_closing_trade:
            ...
            if exit_notes and (exploring or (self.current_step % max(1, getattr(self, "exploration_warmup_log_span", 250)) == 0)):
                logger.debug(
        ...
        if self.position == 0:
            ...
            self.steps_without_action = 0
        if getattr(self, 'post_trade_cooldown', 0) > 0 or getattr(self, 'disable_new_entries', False):
            should_open_long = False
            should_open_short = False
        ...
        if self.position != 0:
            ...
            if self.position > 0:
                ...
                if self._peak_price_since_entry is None:
                    ...
                    self._peak_price_since_entry = max(self._peak_price_since_entry, current_price)
                profit_in_atr = (self._peak_price_since_entry - self.entry_price) / (atr + 1e-9)
                # CSL estático (proteção contra crash imediato pós-entrada)
                catastrophe_sl_static = self.entry_price - (atr * catastrophe_multiplier)
                ...
                if profit_in_atr >= 3.0:
                    trail_level = self._peak_price_since_entry - (atr * catastrophe_multiplier * 1.0)
                    new_floor = max(be_price, trail_level)
                    self._catastrophe_sl_floor = max(self._catastrophe_sl_floor or be_price, new_floor)
                elif profit_in_atr >= 2.0:
                    # Breakeven apenas — trail ainda não arrasta
                    self._catastrophe_sl_floor = max(self._catastrophe_sl_floor or be_price, be_price)
                ...
                if self._peak_price_since_entry is None:
                    ...
                    self._peak_price_since_entry = min(self._peak_price_since_entry, current_price)
                profit_in_atr = (self.entry_price - self._peak_price_since_entry) / (atr + 1e-9)
                # CSL estático
                catastrophe_sl_static = self.entry_price + (atr * catastrophe_multiplier)
                fee = getattr(self.trading_config, 'TAKER_FEE', 0.001)
                be_price = self.entry_price * (1.0 - 2.1 * fee)
                ...
                if profit_in_atr >= 3.0:
                    ...
                    self._catastrophe_sl_floor = min(self._catastrophe_sl_floor or be_price, new_floor)
                elif profit_in_atr >= 2.0:
                    # Breakeven apenas — trail ainda não arrasta
                    self._catastrophe_sl_floor = min(self._catastrophe_sl_floor or be_price, be_price)
        ...
        if not trade_was_closed and self.position != 0 and (self.sl_level is not None) and not is_in_grace_period and not getattr(self, 'disable_normal_sl', False):
            if self.position > 0:
                if self.sl_level is not None and current_price <= self.sl_level:
                    trade_was_closed = True
                    close_price = self._simulate_slippage(current_price, atr, OrderSide.SELL)
                    pnl_realized = self.initial_notional_value * ((close_price - self.entry_price) / (self.entry_price + 1e-9))
                    self._apply_realized_pnl(pnl_realized)
                    self._prev_unrealized_return = 0.0
                    closed_trade_duration = self.steps_in_position
                ...
                if self.sl_level is not None and current_price >= self.sl_level:  # [FIX #1] condição restaurada
                    trade_was_closed = True
                    close_price = self._simulate_slippage(current_price, atr, OrderSide.BUY)
                    pnl_realized = self.initial_notional_value * ((self.entry_price - close_price) / (self.entry_price + 1e-9))
                    self._apply_realized_pnl(pnl_realized)  # chamado apenas 1x (era duplicado)
                    self._prev_unrealized_return = 0.0
                    closed_trade_duration = self.steps_in_position
                    closed_trade_side = 'short'
                    closed_entry_price = float(self.entry_price)
        ...
        if not trade_was_closed and self.position != 0:
            if hasattr(self, '_should_exit_fast') and self._should_exit_fast(self.pnl_since_entry):
                trade_was_closed = True
        ...
        if self.position != 0 and not trade_was_closed:
            lookback = 22
            atr_mult = 3.0
            _abs_end = min(self.start_idx + self.current_step, len(self.df) - 1)
            start = max(0, _abs_end - lookback + 1)
            window_high = float(np.max(self.df['high'].iloc[start:_abs_end + 1])) if 'high' in self.df.columns else current_price
            window_low = float(np.min(self.df['low'].iloc[start:_abs_end + 1])) if 'low' in self.df.columns else current_price
            if self.position > 0:
        ...
        if self.position != 0 and not trade_was_closed:
            if self._psar is None:
                ...
                if self._psar_trend == 1:
                    new_ep = max(self._psar_ep, float(current_row.get('high', current_price)))
                    ...
                    new_ep = min(self._psar_ep, float(current_row.get('low', current_price)))
                    if new_ep < self._psar_ep:
                        self._psar_ep = new_ep
                        self._psar_af = min(self._psar_af + 0.02, 0.2)
                    self._psar = self._psar + self._psar_af * (self._psar_ep - self._psar)
        ...
        if self.position != 0 and not trade_was_closed and (not getattr(self, 'disable_time_stop', False)) and self.time_stop_step is not None and self.current_step >= self.time_stop_step and not is_in_grace_period:
            ...
            ema_trend_present = (self.ema_trend_col is not None) and (not pd.isna(ema_trend_val))
            ...
            if self.steps_in_position < 10:
                ...
                neutral_or_against = (regime_side_now >= max(regime_up_now, regime_down_now)) or (self.position > 0 and ema_trend_val <= 0) or (self.position < 0 and ema_trend_val >= 0)
        ...
        if not trade_was_closed and (is_closing_trade or is_opening_or_reversing):
            ...
            if is_opening_or_reversing and not trade_was_closed:
                ...
                # Tamanho da posição: com prior de confiança nos dados, escala pela confiança; sem ele (caso atual),
                # usa a alocação cheia e a alavancagem escolhida pelo agente controla o risco. Antes o notional
                # ficava <= 3% do patrimônio (10% x 0.1) e o PnL por passo era ruído perto da entropia do SAC.
                if 'tp_prior_conf' in self._col_idx_map:
                ...
                base_allocation = self.net_worth * float(getattr(self.trading_config, 'ENV_BASE_ALLOCATION', 1.0))
                ...
                alignment_threshold = 0.1 if ('bull' in _spec_name or 'bear' in _spec_name) else 0.2
                
                if abs(prior_dir_val) > alignment_threshold:
                    aligned = (self.position > 0 and prior_dir_val > 0) or (self.position < 0 and prior_dir_val < 0)
                    self._current_trade_aligned = aligned  # salva para contar no fechamento
                    if not aligned:
                ...
                duration_hint_raw = current_row.get('tp_duration_median', self.scalp_penalty_min_duration)
                try:
                    duration_hint = float(duration_hint_raw)
                except Exception:
                    duration_hint = float(self.scalp_penalty_min_duration)
                trend_bias = 1.0 if ((action_side > 0 and trend_allows_long) or (action_side < 0 and trend_allows_short)) else 0.0
                prior_strength = float(np.clip(abs(prior_dir_val_safe), 0.0, 1.0))
                self._active_grace_period = self._compute_trade_grace_period(atr_pct, duration_hint, prior_strength, trend_bias)
                self.steps_in_position = 0
                ...
                if getattr(self, 'disable_time_stop', False):
                    ...
                    cap = int(max(self.scalp_penalty_min_duration, self.max_trade_duration_cap))
                ...
                try:
                    self._last_entry_prior_conf = float(self.df.iloc[self.start_idx + self.current_step].get('tp_prior_conf', np.nan))
                except Exception:
                ...
                self.position, self.entry_price, self.initial_notional_value, self.steps_in_position = 0.0, 0.0, 0.0, 0
                self._active_grace_period = 0
                self.sl_level = None
                self.time_stop_step = None
                self._psar = None
        ...
        if trade_was_closed:
            ...
            leverage_used = float(abs(self.current_leverage)) if abs(self.current_leverage) > 1e-9 else 1.0
            ...
            logger.debug("[FASE 1 DEBUG] Trade closed: duration=%s reason=%s trades_remaining=%s", duration_steps, info.get('exit_reason'), getattr(self, 'trades_remaining', None))
            ...
            if within_quota:
                ...
                reward_do_trade = self._compute_trade_reward(pnl_realized, trade_return_pct, duration_steps, exit_reason, atr_pct)
            ...
            try:
                ...
                pass
            if isinstance(getattr(self, "_gate_stats", None), Counter):
                close_key = f"closed_{trade_side}" if trade_side in {"long", "short"} else "closed_unknown"
            ...
            if self._last_entry_prior_conf is not None and not np.isnan(self._last_entry_prior_conf):
                self._ece_records.append((float(np.clip(self._last_entry_prior_conf, 0.0, 1.0)), 1 if pnl_realized > 0 else 0))
            self._last_entry_prior_conf = None
            self.position = 0.0
            self.entry_price = 0.0
            self.initial_notional_value = 0.0
        ...
        if self.position != 0 and ((self.position > 0 and ema_trend_val < 0) or (self.position < 0 and ema_trend_val > 0)):
        ...
        if (done or at_segment_end) and self.position != 0:
            ...
            reward += self._compute_trade_reward(pnl_realized, trade_return_pct, self.steps_in_position, 'Episode End', atr_pct)
        ...
        if done:
            ...
            try:
                ...
                logger.debug("[%s] [ALINHAMENTO] Ep#%d | Total Trades: %s | A Favor do Predictor: %s | Contra o Predictor: %s", self.__class__.__name__, _ep_num, self.episode_trades, self._trades_aligned_with_predictor, self._trades_against_predictor)
        ...
        #     penalty_accum = 0.0
        if penalties_debug:
            penalties_payload = {key: float(val) for key, val in penalties_debug.items()}
        ...
        if 'bull' in _spec_name:
            if self.position < 0: # Short em bull
                is_mismatch = True
            elif should_open_short and agent_vote_strength < -0.1: # Tentando abrir short em bull
                is_mismatch = True
        ...
        physics_metrics = {
            'shannon_entropy': float(self._physics_entropy[_safe_step]),
            'hurst_exponent': float(self._physics_hurst[_safe_step]),
        }
        ...
        if trade_was_closed:
            ...
            _trade_reward_part = getattr(self, '_last_trade_reward_component', float('nan'))
            _sci_reward_part = getattr(self, '_last_scientific_reward_component', float('nan'))
            logger.debug(
        ...
        if capital_ratio < ruin_buffer_threshold:
            ...
            logger.debug("[RUIN BUFFER] capital_ratio=%.2f | penalty=%.2f", capital_ratio, ruin_penalty)
...
class OnlineFeatureCalculator:
    ...
    def __init__(self, full_df: pd.DataFrame):
        ...
        if not all(c in full_df.columns for c in base_cols):
            raise ValueError(f"Ã°Å¸Å¡Â¨ Faltam colunas base em full_df: {set(base_cols) - set(full_df.columns)}")
        self.df_raw = full_df[base_cols].copy()
        self._cache: Dict[int, np.ndarray] = {}
        self.min_history = 200  # HistÃƒÂ³rico mÃƒÂ­nimo para indicadores longos (ex: EMA 200)
        ...
        self.feature_names: List[str] = [
            ...
            'atr', 'atr_percentage', 'obv', 'vwap',
            'adx', 'plus_di', 'minus_di',
            'cmf', 'pvt', 'adl'
        ]
        self.num_features = len(self.feature_names)
        logger.info(f"[FEATURE CALC] OnlineFeatureCalculator inicializado com {self.num_features} features.")
    ...
    def calculate_features_at(self, step: int) -> np.ndarray:
        ...
        vals = [
            ...
            (NativeIndicators.bollinger_bands(close, 20, 2.0)[0].iloc[-1] - NativeIndicators.bollinger_bands(close, 20, 2.0)[2].iloc[-1]) / (NativeIndicators.bollinger_bands(close, 20, 2.0)[1].iloc[-1] + 1e-9),
            NativeIndicators.atr(high, low, close, 14).iloc[-1],
            (NativeIndicators.atr(high, low, close, 14).iloc[-1] / (close.iloc[-1] + 1e-9)) * 100.0,
            ...
            NativeIndicators.calculate_money_flow_volume_pressure(high, low, close, volume)['cmf'].iloc[-1],
            NativeIndicators.calculate_money_flow_volume_pressure(high, low, close, volume)['pvt'].iloc[-1],
            NativeIndicators.calculate_money_flow_volume_pressure(high, low, close, volume)['adl'].iloc[-1],
        ]
        vec = np.array([0.0 if pd.isna(x) else float(x) for x in vals], dtype=np.float32)
        self._cache[step] = vec
        return vec
    @staticmethod
    ...
    def _compute_ece(records: List[Tuple[float, int]], n_bins: int = 10) -> float:
        ...
        for i in range(n_bins):
            ...
            mask = (confs >= left) & (confs < right) if i < n_bins - 1 else (confs >= left) & (confs <= right)
            if not np.any(mask):
                continue
...
class TrendSpecialist:
    ...
    def __init__(self, config: AIConfig, input_dim: int, profitability_predictor: Optional[ProfitabilityPredictor] = None, specialist_name: str = "trend_specialist"):
        ...
        self.model_path = os.path.join(self.config.MODEL_DIR, f"{specialist_name}_sac.zip")
        ...
        self.hyperparams_path = os.path.join(self.log_dir, f"{specialist_name}_sac_hyperparams.json")
        self.optuna_db_path = os.path.join(self.log_dir, f"{specialist_name}_sac_optuna.db")
        self.reward_params_path = os.path.join(self.log_dir, f"{specialist_name}_reward_params.json")
        ...
        self.action_space = spaces.Box(
            low=np.array([-1.0, self.trading_config.SL_DIST_MULTIPLIER_MIN, self.trading_config.MIN_LEVERAGE_PER_TRADE]),
            high=np.array([1.0, self.trading_config.SL_DIST_MULTIPLIER_MAX, self.trading_config.MAX_LEVERAGE_PER_TRADE]),
            dtype=np.float32
        )
    ...
    def is_optimized(self) -> bool:
        ...
        """
        return self._hyperparams_loaded_from_file and os.path.exists(self.hyperparams_path)
    ...
    def _make_trend_env(
        ...
        *,
        ...
        kwargs: Dict[str, Any] = {
            ...
            "specialist_name": specialist_name or getattr(self, "specialist_name", "TrendSpecialist")
        }
        ...
        try:
            ...
            if "mode" in str(exc):
                ...
                logger.warning(f"[TREND SAC] {env_class.__name__} sem suporte a 'mode'. Recriando sem o parâmetro para compatibilidade.")
                env = env_class(**kwargs)
                return self._post_process_env(env)
            raise
    ...
    def _get_device(self) -> str:
        ...
        import torch
        self.logger.warning(f"[DEBUG GPU] torch version: {getattr(torch, '__version__', 'unknown')}")
        self.logger.warning(f"[DEBUG GPU] torch.cuda.is_available(): {torch.cuda.is_available()}")
        import os
        self.logger.warning(f"[DEBUG GPU] CUDA_VISIBLE_DEVICES: {os.environ.get('CUDA_VISIBLE_DEVICES', 'NOT_SET')}")
        if torch.cuda.is_available():
            try:
                gpu_name = torch.cuda.get_device_name(0)
                self.logger.warning(f"🚀 [GPU] Usando GPU: {gpu_name} (cuda:0) para treinamento SAC.")
            except Exception as e:
        ...
        try:
            self.logger.warning(f"[DEBUG GPU] device count: {torch.cuda.device_count()}")
        except Exception as e:
    ...
    def _load_hyperparams(self):
        if os.path.exists(self.hyperparams_path):
            try:
                ...
                logger.info(f"[TREND SAC] Hiperparametros carregados de '{self.hyperparams_path}'.")
            except Exception:
                self.hyperparams = {}
                self._hyperparams_loaded_from_file = False
        else:
        ...
        if self.force_optuna_retrain:
            if os.path.exists(self.hyperparams_path):
                ...
                try:
                    ...
                    logger.info(f"[TREND SAC] Backup dos hiperparametros antigos criado em '{backup_path}'.")
                except Exception as exc:
                    logger.warning(f"[TREND SAC] Nao foi possivel criar backup dos hiperparametros antigos: {exc}")
            self.hyperparams = {}
            self._hyperparams_loaded_from_file = False
            logger.info("[TREND SAC] Otimizacao via Optuna sera forcada nesta execucao.")
    ...
    def _save_hyperparams(self):
        try:
            ...
            logger.error("[ERRO TREND SAC] Falha ao salvar hiperparametros: %s" % e, exc_info=True)

    def _save_anti_scalping_config(self):
        """Salva a configuraÃƒÂ§ÃƒÂ£o anti-scalping otimizada"""
        try:
            config_path = os.path.join(self.log_dir, "trend_specialist_anti_scalping_config.json")
            ...
            logger.error("[ERRO FASE 1] Falha ao salvar configuraÃƒÂ§ÃƒÂ£o anti-scalping: %s" % e, exc_info=True)
    ...
    def _load_anti_scalping_config(self):
        ...
        try:
            ...
            if os.path.exists(config_path):
                ...
                if version is None:
                    missing_keys = [key for key in ANTI_SCALPING_REQUIRED_KEYS if key not in loaded_config]
                    if not missing_keys:
                        logger.info("[FASE 1] Configuracao anti-scalping encontrada sem versao. Migrando para versao %s.", ANTI_SCALPING_CONFIG_VERSION)
                        self.anti_scalping_config = {key: loaded_config[key] for key in ANTI_SCALPING_REQUIRED_KEYS}
            ...
            logger.warning("[FASE 1] Falha ao carregar configuracao anti-scalping: %s" % e)
        return False
    ...
    def _extract_ema_trend_value(self, row: pd.Series) -> float:
        ...
        candidates.extend(col for col in row.index if isinstance(col, str) and col.startswith("ema_trend_"))
    ...
    def _calculate_average_metrics(self, summaries: List[Dict[str, float]]) -> Dict[str, float]:
        """
        ...
        for summary in summaries:
            ...
            trend_samples_val = _safe_float(summary.get("trend_regime_samples", 0.0)) or 0.0
        ...
        for key in keys:
            ...
            if key in {"win_rate_pct", "avg_trade_duration"} and total_trades > 0:
                ...
                aggregated[key] = float(weighted_sum / weight_total) if weight_total > 0 else float(np.mean(values))
            else:
                aggregated[key] = float(np.mean(values))

        if total_trades > 0:
            aggregated["num_trades"] = aggregated.get("num_trades", float(total_trades / len(summaries)))
    ...
    def _calculate_trend_surfing_score(self, metrics: Dict[str, float]) -> float:
        ...
        try:
            ...
            early_exit_penalty = max(0.0, duration_target_min - avg_trade_duration) * 0.06
    ...
    def _calculate_cooldown_effectiveness(self, metrics: Dict[str, float]) -> float:
        ...
        try:
            ...
            desired_density = float(np.clip(desired_density, 20.0, 120.0)) if np.isfinite(desired_density) else 60.0
    ...
    def optimize_anti_scalping_config(self, train_df: pd.DataFrame, eval_df: pd.DataFrame, n_trials: int = 1000, n_timesteps: int = 10000, n_eval_episodes: int = 3):
        """
        ...
        logger.info("[TREND SAC FASE 1] Iniciando otimizaÃƒÂ§ÃƒÂ£o de configuraÃƒÂ§ÃƒÂ£o anti-scalping...")
        ...
        def objective(trial: optuna.Trial) -> float:
            ...
            trend_alignment = trial.suggest_float('trend_alignment_threshold', 0.01, 0.12)
            ...
            min_action_threshold_opt = trial.suggest_float('min_action_threshold_opt', 0.08, min(0.22, base_threshold_opt))
            prior_open_long_opt = trial.suggest_float('prior_open_long_threshold_opt', 0.25, 0.45)
            prior_open_short_opt = trial.suggest_float('prior_open_short_threshold_opt', -0.45, -0.25)
            force_threshold_factor_opt = trial.suggest_float('force_threshold_factor_opt', 0.5, 0.8)
            penalty_cap_per_step_opt = trial.suggest_float('penalty_cap_per_step_opt', 1.0, 3.0)
            warmup_base_opt = trial.suggest_float('warmup_base_opt', 0.1, 0.4)
            warmup_scale_high = max(0.4, 1.0 - warmup_base_opt)
            warmup_scale_opt = trial.suggest_float('warmup_scale_opt', 0.4, warmup_scale_high)
            config_params = {
            ...
            try:
                ...
                def _summarize(df):
                    ...
                    return {
                        'len': int(len(df)),
                        'reg_up_mean': float(np.nanmean(df.get('tp_regime_up', 0.0))) if 'tp_regime_up' in df.columns else 0.0,
                        'reg_down_mean': float(np.nanmean(df.get('tp_regime_down', 0.0))) if 'tp_regime_down' in df.columns else 0.0,
                        'reg_side_mean': float(np.nanmean(df.get('tp_regime_sideways', 0.0))) if 'tp_regime_sideways' in df.columns else 0.0,
                ...
                logger.debug(
                    ...
                    trial.number,
                    {k: (round(v, 4) if isinstance(v, float) else v) for k, v in config_params.items()},
                    train_sum['len'], train_sum['reg_up_mean'], train_sum['reg_down_mean'], train_sum['reg_side_mean'], train_sum['ema_mean'],
                    eval_sum['len'], eval_sum['reg_up_mean'], eval_sum['reg_down_mean'], eval_sum['reg_side_mean'], eval_sum['ema_mean'],
                )
            except Exception:
                pass
            try:
                ...
                train_env = VecNormalize(train_env, norm_obs=False, norm_reward=False, clip_reward=10.0)
                
                eval_env = DummyVecEnv([eval_env_fn])
                eval_env = VecNormalize(eval_env, norm_obs=False, norm_reward=False, clip_reward=10.0)
                
                default_env_params = self._convert_reward_params_to_env(self.reward_hyperparams)
                ...
                model = ClippedSAC(
                    ...
                    **sac_params,
                    ...
                    tensorboard_log=os.path.join(os.getcwd(), "logs", "tensorboard", self.specialist_name),
                ...
                try:
                    ...
                    if summaries_batches and len(summaries_batches) > 0:
                        ...
                        if all_summaries:
                            ...
                            trend_up_avg = float(avg_metrics.get('trend_regime_avg_up', 0.0))
                            trend_down_avg = float(avg_metrics.get('trend_regime_avg_down', 0.0))
                            trend_side_avg = float(avg_metrics.get('trend_regime_avg_side', 0.0))
                            trade_up_pct = float(avg_metrics.get('regime_trade_up_pct', 0.0))
                            trade_down_pct = float(avg_metrics.get('regime_trade_down_pct', 0.0))
                            trade_side_pct = float(avg_metrics.get('regime_trade_side_pct', 0.0))
                            ema_trend_avg = float(avg_metrics.get('ema_trend_avg', 0.0))
                            dominant_trend = str(avg_metrics.get('trend_dominant', 'none')).upper()
                            trend_samples = float(avg_metrics.get('trend_regime_samples', 0.0))
                            logger.debug(
                            ...
                            avg_trade_duration = float(avg_metrics.get('avg_trade_duration', 0.0))
                            win_rate = float(avg_metrics.get('win_rate_pct', 0.0))
                            profit_factor = float(avg_metrics.get('profit_factor', 0.0))
                            profit_factor_raw = float(avg_metrics.get('profit_factor_raw', profit_factor))
                            pf_reliability = float(np.clip(avg_metrics.get('pf_reliable', 1.0), 0.0, 1.0))
                            trade_sharpe = float(avg_metrics.get('trade_sharpe', 0.0))
                            max_trades_cap = float(avg_metrics.get('max_trades_cap', 0.0))
                            trades_per_1000 = num_trades / max(1.0, episode_steps / 1000.0)
                            avg_metrics['trades_per_1000'] = trades_per_1000
                            trade_target_max = max(1.0, max_trades_cap or 60.0)
                            if trades_per_1000 > 1000.0:
                                logger.info(f"[FASE 1 PODA] Trial {trial.number} podado: densidade de trades muito alta (trades/1000={trades_per_1000:.1f}, trades={num_trades:.1f}).")
                                raise optuna.exceptions.TrialPruned()
                            if num_trades < 1:
                                logger.info(f"[FASE 1] Trial {trial.number}: nenhum trade executado. Score penalizado, sem pruning.")
                                score = -6.0
                            else:
                            ...
                            cooldown_effectiveness = self._calculate_cooldown_effectiveness(avg_metrics)
                            trend_following_ratio = self._calculate_trend_following_ratio(avg_metrics)
                            trade_distribution = self._calculate_trade_distribution(avg_metrics)
                            logger.info(f"[FASE 1] Trial {trial.number}: Score={score:.4f}")
                            logger.info(
                            ...
                            if max_trades_cap:
                                utilization = (num_trades / max(1.0, max_trades_cap)) * 100.0
                                logger.debug(f"    Trades: {num_trades} / {max_trades_cap:.0f} (utilizacao: {utilization:.1f}%)")
                            else:
                                logger.debug(f"    Trades: {num_trades} (ideal: 1-60)")
                            min_dur_param = config_params.get('min_trade_duration')
                            target_min_label = f"{min_dur_param}+" if min_dur_param is not None else "N/A"
                            logger.debug(f"    Duration: {avg_trade_duration:.1f} candles (target_min: {target_min_label})")
                            logger.debug(f"    Win Rate: {win_rate:.1f}% (ideal: 50-70%)")
                            pf_tag = "ok" if pf_reliability >= 0.5 else "baixa confianca"
                            logger.debug(f"    Profit Factor: {profit_factor:.2f} (raw {profit_factor_raw:.2f}, {pf_tag}) (ideal: 1.2-2.0)")
                            logger.debug(f"    Sharpe: {trade_sharpe:.2f} (ideal: 0.5-2.0)")
                            logger.debug(f"    Max DD: {max_drawdown * 100.0:.1f}% (ideal: <10%)")
                            logger.debug(f"    Cooldown Eff: {cooldown_effectiveness:.1f}% ")
                            logger.debug(f"    Trend Follow: {trend_following_ratio:.1f}%")
                            logger.debug(f"    Distribution: {trade_distribution:.1f}% long ")
                            return score
...
# Otimizacao da configuracao com poda agressiva
        ...
        anti_scalp_db = os.path.join(os.getcwd(), "logs", "trend_specialist_anti_scalping_config.db")
        ...
        if len(study.trials) == 0:
            ...
            try:
                ...
                logger.info("[FASE 1] Seed trial score=%.4f with params=%s", seed_score, seed_params)
            except optuna.exceptions.TrialPruned:
                logger.info("[FASE 1] Seed trial was pruned due to extreme trade density.")
            except Exception as exc:
        ...
        if completed_trials >= n_trials:
            logger.info(f"[FASE 1] JÃƒÂ¡ existem {completed_trials} trials. Nenhum trial adicional necessÃƒÂ¡rio.")
            remaining_trials = 0
        else:
            remaining_trials = n_trials - completed_trials
            logger.info(f"[FASE 1] Retomando estudo: executando {remaining_trials} trials adicionais (de {n_trials}).")
        if remaining_trials > 0:
            study.optimize(objective, n_trials=remaining_trials, timeout=43200) # Aumentado para 12h (era 1h)
        ...
        try:
            if any(t.state == optuna.trial.TrialState.COMPLETE for t in study.trials):
                ...
                logger.info(f"[FASE 1] Melhor configuraÃƒÂ§ÃƒÂ£o encontrada: {self.anti_scalping_config}")
    ...
    def optimize_hyperparameters(self, train_df: pd.DataFrame, eval_df: pd.DataFrame, n_trials: int = 100, n_timesteps: int = 50000, n_eval_episodes: int = 8, env_class=None):
        """
        ...
        self.logger.info("[TREND SAC OPTUNA] Iniciando otimizacao com paralelismo para SAC...")
        ...
        regime_emoji = {'bull': '🐂', 'bear': '🐻', 'ranger': '🤠', 'trend': '📈'}.get(regime_type, '🤖')
        
        self.logger.info(f"")
        self.logger.info(f"{'='*70}")
        self.logger.info(f"{regime_emoji} OTIMIZANDO: {specialist_name.upper()} {regime_emoji}")
        self.logger.info(f"{'='*70}")
        ...
        # [ROBUST SCALING] Harmonização com ScientificDataProcessor
        # Se os dados já vieram normalizados (prepare_for_specialist), o scaler deve ser Passthrough
        try:
            from sklearn.preprocessing import RobustScaler as _RobustScaler
            from feature_engineering.scientific_data_processor import ScientificDataProcessor
            processor = ScientificDataProcessor()
            
            _auto_cols = list(train_df_short.select_dtypes(include=np.number).columns)
            if 'tp_prior_dir' in _auto_cols:
                _auto_cols.remove('tp_prior_dir')
            
            _feature_cols_for_scaler = processor.filter_autoencoder_features(_auto_cols)
            # Armazena colunas para evitar recálculo no ambiente
            self.cached_feature_columns = _feature_cols_for_scaler

            # Check if likely already normalized (ScientificDataProcessor uses RobustScaler)
            # If extremes are near [-5, 5], it's likely already scaled.
            sample_data = train_df_short[_feature_cols_for_scaler].fillna(0).values
            is_normalized = np.abs(np.percentile(sample_data, [1, 99])).max() < 20.0
            
            if is_normalized:
                self.logger.info("🟢 [OPTUNA] Dados detectados como PRÉ-NORMALIZADOS. Scaler será Passthrough.")
                self.feature_scaler = None
            elif _feature_cols_for_scaler:
                _scaler = _RobustScaler()
                _scaler.fit(sample_data)
                self.feature_scaler = _scaler
                self.logger.info(f"🔄 [OPTUNA] Scaler (RobustScaler) ajustado em {len(_feature_cols_for_scaler)} features.")
            else:
                self.feature_scaler = None
        except Exception as _scaler_err:
            self.feature_scaler = None
            self.logger.warning(f"[SCALER] Falha ao ajustar scaler: {_scaler_err}")
        # --- CRÃTICO: NÃ£o vetorize o ambiente aqui ---
        # Cada trial paralelo cuidarÃ¡ de seu prÃ³prio ambiente Ãºnico.
        train_env_fn = lambda: self._make_trend_env(train_df_short, mode='optimization', env_class=env_class)
        eval_env_fn  = lambda: self._make_trend_env(eval_df_short,  mode='optimization', env_class=env_class)

        # [FIX SCALER MISMATCH OPTUNA] O scaler foi fit em 183 features via
        # filter_autoencoder_features(), mas o env usa _filter_to_core_features() -> 150.
        # Solucao: env probe para ler as feature_columns reais e re-fit o scaler.
        try:
            ...
            try:
                ...
                pass
            if _env_cols:
                _probe_data = train_df_short[_env_cols].fillna(0).values
                _already_scaled = np.abs(np.percentile(_probe_data, [1, 99])).max() < 20.0
                if _already_scaled:
                    self.feature_scaler = None
                    self.logger.info("[FIX SCALER] Features pre-normalizadas — scaler = None.")
                else:
                    _fixed_scaler = _RS_fix()
                    _fixed_scaler.fit(_probe_data)
                    self.feature_scaler = _fixed_scaler
                    self.logger.info(
                        "[FIX SCALER] Scaler re-ajustado em %d features do env (era %d). Mismatch resolvido.",
                        len(_env_cols),
                        len(getattr(self, 'cached_feature_columns', _env_cols)),
                    )
                # [FIX SCALER SYNC] Atualiza cached_feature_columns para as colunas REAIS do env.
                # BUG: sem esta linha, _make_trend_env (linha 2925) passava cached_feature_columns=183
                # para todos os trials subsequentes, forçando o env a usar 183 features e o scaler
                # (re-ajustado em 63) a falhar com "X has 63 features, expecting 183".
                self.cached_feature_columns = _env_cols
        except Exception as _fix_err:
            self.logger.warning("[FIX SCALER] Falha ao sincronizar scaler com env: %s", _fix_err)
        ...
        def objective(trial: optuna.Trial) -> float:
            ...
            if _monitor_ref is not None:
                try:
                    ...
                    logger.debug("[MONITOR] Falha ao resetar no inicio do trial: %s", _mr_err)
            ...
            sac_params = {
                ...
                # Para action_dim=3, o valor ideal é -3.0. Range reduzido para evitar Policy Collapse.
                "target_entropy": trial.suggest_float('target_entropy', -1.5, -0.5),
            }
            ...
            try:
                ...
                try:
                    ...
                    pass
                logger.info(f"[TREND SAC OPTUNA] TRIAL {trial.number}: LEARN start (timesteps={n_timesteps})")
                ...
                try:
                    ...
                    from stable_baselines3.common.callbacks import CallbackList as _CallbackList
                ...
                for step_int, mean_reward_guard in eval_history:
                    ...
                    if consecutive_negative >= 4:
                        trial.set_user_attr("forced_prune_reason", "guard_mean_reward_neg")
                        ...
                        raise optuna.exceptions.TrialPruned("Trial pruned by TrendSpecialist guard (mean_reward <= floor)")
                if getattr(_real_eval_cb, "is_pruned", False):
                    prune_reason = trial.user_attrs.get("forced_prune_reason", "callback")
                    ...
                    raise optuna.exceptions.TrialPruned(f"Trial pruned by callback ({prune_reason})")
                logger.info(f"[TREND SAC OPTUNA] TRIAL {trial.number}: LEARN end")
                # <<< CORREÃƒâ€¡ÃƒÆ'O: Limpa summaries acumulados do EvalCallback >>>
                logger.debug("Limpando summaries de avaliaÃƒÂ§ÃƒÂ£o intermediÃƒÂ¡rios...")
                try:
                ...
                mean_reward, std_reward = evaluate_policy(
                    model, eval_env, n_eval_episodes=n_eval_episodes, deterministic=True  # CORREÃƒâ€¡ÃƒÆ'O: AvaliaÃƒÂ§ÃƒÂ£o determinÃƒÂ­stica
                )
                if mean_reward <= guard_floor:
                    ...
                    raise optuna.exceptions.TrialPruned("Trial pruned by final evaluation (mean_reward <= floor)")
                logger.info(f"[TREND SAC OPTUNA] TRIAL {trial.number}: EVALUATE end | mean={mean_reward:.4f} std={std_reward:.4f}")
                ...
                try:
                    ...
                    if hasattr(model, 'logger') and hasattr(model.logger, 'name_to_value'):
                        _trial_actor_loss = model.logger.name_to_value.get('train/actor_loss')
                    if hasattr(model, 'actor') and hasattr(model.actor, 'log_std') and model.actor.log_std is not None:
                        import torch as _torch
                        with _torch.no_grad():
                            _trial_std = float(_torch.exp(model.actor.log_std.data).mean().item())
                    ...
                    if _monitor is not None:
                        ...
                        if _monitor.should_abort_trial():
                            logger.critical(
                                ...
                                trial.number, _monitor.get_abort_reason()
                            )
                            raise optuna.exceptions.TrialPruned(
                ...
                try:
                    ...
                    summaries_batches = eval_env.env_method("consume_episode_summaries")
                    episode_summaries = summaries_batches[0] if isinstance(summaries_batches, list) and summaries_batches else []
                except Exception:
                    episode_summaries = []
                if episode_summaries:
                    pf_values = [s.get("profit_factor", 0.0) for s in episode_summaries]
                    pf_raw_values = [s.get("profit_factor_raw", v) for s, v in zip(episode_summaries, pf_values)]
                    pf_reliabilities = [float(np.clip(s.get("pf_reliable", 1.0), 0.0, 1.0)) for s in episode_summaries]
                    profit_factor = float(np.mean([min(v, PF_MAX_DISPLAY) for v in pf_values]))
                    profit_factor_raw = float(np.mean(pf_raw_values)) if pf_raw_values else profit_factor
                    pf_reliability_mean = float(np.mean(pf_reliabilities)) if pf_reliabilities else 1.0
                    if pf_reliability_mean < 0.5:
                        profit_factor = min(profit_factor, 1.2)
                    trade_sharpe = float(np.mean([s.get("trade_sharpe", 0.0) for s in episode_summaries]))
                    avg_return_pct = float(np.mean([s.get("avg_return_pct", 0.0) for s in episode_summaries]))
                    max_drawdown_pct = float(np.mean([s.get("max_drawdown_pct", 1.0) for s in episode_summaries]))
                    avg_trade_duration = float(np.mean([s.get("avg_trade_duration", 0.0) for s in episode_summaries]))
                    num_trades = float(np.sum([s.get("num_trades", 0.0) for s in episode_summaries]))
                else:
                    try:
                        eval_env.envs[0].env
                        fin_stats = eval_env.envs[0].env.get_attr('get_financial_stats')[0]()
                    except Exception:
                        fin_stats = {"profit_factor": 0.0, "trade_sharpe": 0.0, "avg_return_pct": 0.0, "max_drawdown_pct": 1.0, "num_trades": 0.0, "pf_reliable": 1.0}
                    profit_factor = float(fin_stats.get("profit_factor", 0.0))
                    profit_factor_raw = float(fin_stats.get("profit_factor_raw", profit_factor))
                    pf_reliability_mean = float(np.clip(fin_stats.get("pf_reliable", 1.0), 0.0, 1.0))
                    if pf_reliability_mean < 0.5:
                        profit_factor = min(profit_factor, 1.2)
                ...
                disable_financial_prune = os.getenv('TREND_DISABLE_FINANCIAL_PRUNE', '1').strip().lower() in {'1', 'true', 'yes', 'on'}  # Default=1 (desabilitado)
                if not disable_financial_prune and (profit_factor < 1.0 or trade_sharpe <= 0.0):
                    prune_steps = int(getattr(model, "num_timesteps", 0))
                    trial.set_user_attr("forced_prune_reason", "financial_metrics_prune")
                    trial.set_user_attr("pruned_timesteps", prune_steps)
                    logger.info(
                    ...
                    raise optuna.exceptions.TrialPruned("Trial pruned due to weak financial metrics")
                ...
                try:
                    ...
                    reward_component = 0.5 * float(np.clip(reward_per_step / 3.0, -1.0, 1.0))
                except Exception:
                ...
                try:
                    ...
                    eval_df_ctx = eval_env.envs[0].df if hasattr(eval_env.envs[0], 'df') else None
                    if eval_df_ctx is not None and isinstance(eval_df_ctx, pd.DataFrame):
                        def _find_macro_columns(df: pd.DataFrame) -> tuple[Optional[pd.Series], Optional[pd.Series], Optional[pd.Series]]:
                            ...
                            for tf_suffix in ['1d', 'd', 'D', '4h', 'h4', '1h', 'h1']:
                                ...
                                if adx_col in df.columns and ema_fast_col in df.columns and ema_slow_col in df.columns:
                                    try:
                                        logger.info(f"[MTF] Contexto macro usando timeframe '{tf_suffix}'.")
                                    except Exception:
                                        pass
                                    return df[adx_col], df[ema_fast_col], df[ema_slow_col]
                            return None, None, None
                        adx_macro, ema_fast_macro, ema_slow_macro = _find_macro_columns(eval_df_ctx)
                        if adx_macro is not None and ema_fast_macro is not None and ema_slow_macro is not None:
                            ...
                            direcao_macro = float(np.sign(ema_fast_macro.astype(float) - ema_slow_macro.astype(float)).mean())
                            # Penaliza inatividade em tendÃªncia forte e clara
                            if forca_tendencia_macro > 0.5 and abs(direcao_macro) > 0.5 and num_trades < min_trades_razoavel:
                            ...
                            if forca_tendencia_macro > 0.4 and abs(direcao_macro) > 0.4:
                                hold_factor = float(np.clip((avg_trade_duration or 0.0) / 30.0, 0.0, 2.0))
                                pf_factor = float(np.clip(max(profit_factor - 1.0, 0.0), 0.0, 2.0))
                                trend_surf_bonus = 0.5 * forca_tendencia_macro * (0.5 + 0.5 * hold_factor) + 0.3 * pf_factor
                                logger.info(f"[MTF] BÃ´nus surfar tendÃªncia aplicado: {trend_surf_bonus:.3f} (forÃ§a={forca_tendencia_macro:.2f}, hold={hold_factor:.2f}, pf_factor={pf_factor:.2f})")
                            # Impulso adicional em macro muito forte para favorecer surfagem lucrativa
                            if forca_tendencia_macro > 0.7 and abs(direcao_macro) > 0.6 and (avg_trade_duration or 0.0) >= 20.0:
                                strong_surf_boost = 0.2 + 0.3 * float(np.clip(max(profit_factor - 1.0, 0.0), 0.0, 2.0))
                                trend_surf_bonus += strong_surf_boost
                                try:
                                    logger.info(f"[MTF] Boost de surf forte aplicado: +{strong_surf_boost:.3f}")
                                except Exception:
                                    pass
                        ...
                        logger.warning("[MTF] Colunas/DF de Multi-Timeframe nÃ£o disponÃ­veis para ajuste de atividade.")
                except Exception as _e:
                    try:
                        logger.debug(f"[MTF] Ajuste de atividade desativado por erro: {_e}")
                    except Exception:
                        pass
                score += score_ajuste_atividade
                score += trend_surf_bonus
                logger.info(f"[TREND SAC OPTUNA] <<< TRIAL {trial.number} BASE | score_base={score:.4f} trades={num_trades} pf={profit_factor:.3f} sharpe={trade_sharpe:.3f}")
                ...
                if num_trades < min_total_trades:
                    inactivity_penalty = (min_total_trades - num_trades) * 0.4 / n_episodes
                ...
                if avg_trade_duration > 1.0:
                    ...
                    expected_trades_per_ep = ep_steps_eval / max(avg_trade_duration, 1.0) * 0.5
                else:
                    expected_trades_per_ep = 200.0
                scalp_limit = int(expected_trades_per_ep * 1.5) * n_eval_eps  # 50% de folga
                if num_trades > scalp_limit:
                    scalp_penalty_weight = float(os.getenv('TREND_SCALP_WEIGHT', '0.005'))  # 0.02→0.005
                ...
                try:
                    ...
                    logger.warning("[TRIAL MATRIX] Falha ao imprimir matriz: %s", _mat_err)
                ...
                try:
                    ...
                    try:
                        ...
                        direcao_macro = 0.0
                    if (forca_tendencia_macro > 0.4 and abs(direcao_macro) > 0.4 and
                        float(avg_trade_duration or 0.0) >= 20.0 and float(profit_factor or 0.0) >= 1.0):
                        is_trend_surf = True
                    trial.set_user_attr("trend_surf_bonus", float(trend_surf_bonus))
                    trial.set_user_attr("forca_tendencia_macro", float(forca_tendencia_macro))
                    trial.set_user_attr("avg_trade_duration", float(avg_trade_duration or 0.0))
        ...
        if self.reset_optuna_db and os.path.exists(self.optuna_db_path):
            try:
                ...
                logger.warning(f'[TREND SAC] Falha ao remover banco de optuna existente: {exc}')
        storage_url = f"sqlite:///{self.optuna_db_path}"
        # Pruner paciente para dar tempo ao aprendizado
        disable_pruner = os.getenv('TREND_OPTUNA_NOPRUNE', '0').strip().lower() in {'1', 'true', 'yes', 'on'}
        if disable_pruner:
            pruner = optuna.pruners.NopPruner()
            logger.info("[TREND SAC OPTUNA] Pruner desabilitado por variavel de ambiente.")
        else:
            ...
            logger.info("[TREND SAC OPTUNA] MedianPruner v3.1: startup=10, warmup=15000, interval=2000")
        try:
            pruner_name = os.getenv('TREND_OPTUNA_PRUNER', 'median').strip().lower()
            if pruner_name == 'sh':
                pruner = optuna.pruners.SuccessiveHalvingPruner(min_resource=6000, reduction_factor=3, min_early_stopping_rate=0)
        ...
        completed_trials = len(study.trials)
        logger.info(f"[TREND SAC OPTUNA] Trials jÃƒÂ¡ executados: {completed_trials}, Objetivo: {n_trials}")
        # Se jÃƒÂ¡ temos trials suficientes, nÃƒÂ£o executa mais
        if completed_trials >= n_trials:
            logger.info(f"[TREND SAC OPTUNA] JÃƒÂ¡ temos {completed_trials} trials completados. Usando resultados existentes.")
            remaining_trials = 0
        else:
            remaining_trials = n_trials - completed_trials
            logger.info(f"[TREND SAC OPTUNA] Executando {remaining_trials} trials adicionais...")
        ...
        if remaining_trials > 0:
            ...
            try:
                ...
                class _HpoProgressCallback:
                    ...
                    def __call__(self, study, trial):
                        ...
                        self._pbar.set_postfix({
                            ...
                            "last": f"{val:.3f}" if trial.value is not None else "pruned",
                            "st": state_icon,
                        }, refresh=True)
                        # Log health dashboard a cada 10 trials
                        if self._monitor is not None and (trial.number + 1) % 10 == 0:
                            try:
                                self._monitor.print_health_dashboard(trial_number=trial.number + 1)
                            except Exception:
                                pass
                ...
                _monitor_ref = getattr(self, 'learning_monitor', None)
                _hpo_cb = _HpoProgressCallback(remaining_trials, self.specialist_name, _monitor_ref)
        ...
        if best_trial and best_trial.value is not None:
            ...
            sac_params = {key: best_params[key] for key in self.SAC_PARAM_KEYS if key in best_params}
            if sac_params:
                self.hyperparams.update(sac_params)
                self._save_hyperparams()
            reward_params = {key: best_params[key] for key in self.REWARD_PARAM_KEYS if key in best_params}
            if reward_params:
    ...
    def _initialize_model(self):
        if os.path.exists(self.model_path):
            try:
                ...
                logger.info("[TREND SAC] TrendSpecialist (SAC) carregado de '%s'.", self.model_path)
    ...
    def train(self, featured_df: pd.DataFrame, timesteps_to_train: int, env_class=None, specialist_name=None) -> int:
        """
        ...
        if featured_df is not None and len(featured_df) > 0:
            ...
            if _regime_col is not None:
                ...
                if 'bull' in _sname and 'bear' not in _sname:
                    _mask = featured_df[_regime_col].str.lower().str.contains('bull', na=False)
                        ...
                        try:
                            if isinstance(featured_df.index, pd.DatetimeIndex):
                                ...
                                logger.info(
                                    ...
                                    _orig_len, len(featured_df), 100.0 * len(featured_df) / _orig_len,
                                    _recent_pct
                                )
                            else:
                    ...
                    _mask = featured_df[_regime_col].str.lower().str.contains('bear', na=False)
                    _filtered = featured_df[_mask]
                    if len(_filtered) >= 5000:
                        featured_df = _filtered.copy()
        ...
        if timesteps_to_train <= 0:
            logger.info("[TREND SAC] Nenhum timestep adicional necessario. Objetivo de treino ja alcancado. Pulando.")
            return self.model.num_timesteps if self.model else 0
        logger.info(f"[TREND SAC] Iniciando ou continuando treinamento por {timesteps_to_train} timesteps...")
        try:
            ...
            if not self._hyperparams_loaded_from_file and os.path.exists(self.hyperparams_path):
                self._load_hyperparams()
            ...
            if should_run_phase2:
                ...
                try:
                    ...
                    pass
                if self.force_optuna_retrain:
                    logger.info("[FASE 2] Reexecutando otimizacao do Optuna conforme configuracao.")
                ...
                self.logger.info(
                    ...
                    _holdout_ratio * 100, len(self.holdout_df), len(featured_df)
                )
                train_df_opt, eval_df_opt = train_test_split(_train_pool, test_size=0.2, shuffle=False)
                ...
                num_cols_all = [c for c in train_df_opt.select_dtypes(include=np.number).columns if c not in protected_cols]
                ...
                self.optimize_hyperparameters(train_df_opt, eval_df_opt, n_trials=100, n_timesteps=100000)
            ...
            try:
                ...
                if self.learning_monitor.circuit_breaker_active:
                    logger.warning("[MONITOR] Circuit breaker da fase HPO resetado para treino final.")
            ...
            feature_columns = [c for c in train_df.select_dtypes(include=np.number).columns if c != 'tp_prior_dir']
            self.feature_columns = feature_columns # Persiste na instância para inferência
            ...
            logger.info(f"[TRAIN] Contrato de features sincronizado com env: {len(feature_columns)} colunas core.")
            ...
            protected_cols_main = set(_all_ohlcv + ['tp_prior_conf']) # 'tp_prior_dir' já foi removido de feature_columns
            # num_cols_all_main deve ser um subset de feature_columns
            num_cols_all_main = [c for c in feature_columns if c not in protected_cols_main]
            ...
            joblib.dump(scaler, os.path.join(models_dir, "feature_scaler_robust.joblib"))
            ...
            train_env = DummyVecEnv([lambda: self._make_trend_env(train_df, mode='training', feature_columns=feature_columns, env_class=env_class, specialist_name=specialist_name) for _ in range(num_envs)])
            train_env = VecFrameStack(train_env, n_stack=N_STACK_FRAMES)   # ← sequência temporal real
            train_env = VecNormalize(train_env, norm_obs=False, norm_reward=False, clip_reward=10.0)

            eval_env = DummyVecEnv([lambda: self._make_trend_env(eval_df, mode='training', feature_columns=feature_columns, env_class=env_class, specialist_name=specialist_name)])
            eval_env = VecFrameStack(eval_env, n_stack=N_STACK_FRAMES)     # ← sequência temporal real
            eval_env = VecNormalize(eval_env, norm_obs=False, norm_reward=False, clip_reward=10.0)
            
            env_reward_params = self._convert_reward_params_to_env(self.reward_hyperparams)
            if env_reward_params:
            ...
            checkpoint_dir = os.path.join(self.config.CHECKPOINT_DIR, f"{self.specialist_name.lower()}_checkpoints")
            ...
            try:
                ...
                from trading.callbacks.clean_metrics_callback import CleanMetricsCallback
                ...
                try:
                    ...
                    if _holdout_df_ref is not None and len(_holdout_df_ref) > 200:
                        from trading.callbacks.holdout_validation_callback import HoldoutValidationCallback
                        ...
                        def _build_holdout_env():
                            ...
                            _h_env = VecNormalize(_h_env, norm_obs=False, norm_reward=False, clip_reward=10.0)
                            if _erp_local:
                                try:
                                    _h_env.env_method('set_reward_shaping_params', _erp_local)
                                except Exception:
                                    pass
                            try:
                        ...
                        def _train_reward_getter():
                            ...
                            try:
                                ...
                                pass
                            try:
                                v = self.model.logger.name_to_value.get('rollout/ep_rew_mean', None)
                                if v is not None:
                                    return float(v)
                            except Exception:
                                pass
                        ...
                        _holdout_eval_freq = int(os.environ.get('HOLDOUT_EVAL_FREQ', '25000'))
                        ...
                        logger.info(
                            ...
                            len(_holdout_df_ref), _holdout_eval_freq,
                        )
                    else:
                        logger.warning(
                    ...
                    logger.warning(f"🧪 [HOLDOUT] Falha ao configurar validação OOS: {_he}")
                    holdout_validation_callback = None
                ...
                logger.info("🔬 [SCIENTIFIC] Callbacks científicos integrados: Sharpe/Sortino/Calmar/Kelly + Entropy Regularization + Clean Metrics")
            ...
            if os.path.exists(best_model_path):
                ...
                import glob
                checkpoint_files = glob.glob(os.path.join(checkpoint_dir, "checkpoint_*_steps.zip"))
                if checkpoint_files:
                    ...
                    logger.info(f"💾 [CHECKPOINT] Encontrado checkpoint periódico: {os.path.basename(checkpoint_to_load)}")
            ...
            if checkpoint_to_load:
                try:
                    ...
                    if not _zip_ok:
                        ...
                        logger.info(f"[TREND SAC] Carregando checkpoint de '{checkpoint_to_load}' para continuar.")
                        self.model = ClippedSAC.load(
                            checkpoint_to_load,
                            env=train_env,
                            device=self._get_device(),
                    ...
                    if "Observation spaces do not match" in str(e):
                        logger.warning(f"[TREND SAC] Checkpoint incompatível (dimensões diferentes). Criando novo modelo.")
                        self.model = None
                    else:
                        logger.warning("[TREND SAC] Falha ao carregar checkpoint ('%s'): %s. Criando novo modelo.", checkpoint_to_load, e)
                        self.model = None
                except Exception as e:
                    logger.warning("[TREND SAC] Erro inesperado ao carregar checkpoint ('%s'): %s. Criando novo modelo.", checkpoint_to_load, e)
                    self.model = None
            ...
            if self.model is None:
                ...
                self.model = ClippedSAC(
                    ...
                    policy_kwargs=policy_kwargs,
                    learning_rate=lr_schedule,  # LR com decay linear
                    buffer_size=self.hyperparams.get("buffer_size", 300000),
                    batch_size=self.hyperparams.get("batch_size", 1024),
                    gamma=self.hyperparams.get("gamma", 0.982),
                    ...
                    max_grad_norm=float(self.hyperparams.get("max_grad_norm", self.max_grad_norm or 0.5)),
                ...
                try:
                    ...
                    self.model.set_logger(_sb3_configure(_tb_dir, format_strings=["stdout", "tensorboard"]))
                    logger.info("[TREND SAC] TensorBoard reconfigurado para checkpoint: %s", _tb_dir)
                except Exception as _tb_err:
                    logger.warning("[TREND SAC] Falha ao reconfigurar TensorBoard pós-checkpoint: %s", _tb_err)
            ...
            try:
                ...
                logger.warning("[TREND SAC] Treinamento interrompido (Ctrl+C). Salvando modelo parcial em '%s'...", self.model_path)
                try:
                    self.model.save(self.model_path)
                    self.is_trained = True
                    logger.info("[TREND SAC] Modelo parcial salvo (%d steps). Encerrando.", self.model.num_timesteps)
            ...
            self.is_trained = True
            # Modelo final = checkpoint com maior retorno econômico no conjunto de avaliação (EvalCallback),
            # não simplesmente o último passo do treino.
            _best_path = os.path.join(checkpoint_dir, "best_model.zip")
            if os.path.exists(_best_path):
                try:
                    self.model.set_parameters(_best_path, exact_match=True, device=self.model.device)
                    logger.info(f"[TREND SAC] Modelo final = melhor checkpoint de avaliação ({_best_path}).")
                except Exception as _best_err:
                    logger.warning(f"[TREND SAC] Não foi possível carregar o melhor checkpoint: {_best_err}")
            self.model.save(self.model_path)
            self._save_observation_contract()
            ...
            try:
                ...
                if hasattr(train_env, 'env_method'):
                    try:
                        ...
                        pass
                _sharpe = float(np.mean([s.get('sharpe_ratio', 0.0) for s in _sums])) if _sums else 0.0
                _pnl_pt = float(np.mean([s.get('avg_pnl_per_trade', 0.0) for s in _sums])) if _sums else 0.0
                _dur_avg = float(np.mean([s.get('avg_trade_duration', 1.0) for s in _sums])) if _sums else 1.0
                _n_trades = int(np.mean([s.get('total_trades', 0) for s in _sums])) if _sums else 0
                # Calcula taxa de scalping (duration=1 / total_trades)
                _dur1 = float(np.mean([s.get('duration_1_count', 0) for s in _sums])) if _sums else 0.0
    ...
    def _observation_contract_path(self) -> str:
        return os.path.join(os.getcwd(), "models_ai", f"{self.specialist_name}_obs_contract.json")

    def _save_observation_contract(self) -> None:
        """Salva o layout exato da observação do treino (colunas, extras, frames) para a inferência reproduzir."""
        try:
            env = getattr(self, 'specialist_env', None)
            if env is None or self.model is None:
                return
            extras = [k for k in env.extra_feature_keys if k not in env.numeric_df.columns]
            frame_dim = int(env.observation_space.shape[0])
            obs_dim = int(self.model.observation_space.shape[0])
            contract = {
                "specialist": self.specialist_name,
                "feature_columns": list(env.feature_columns),
                "extra_feature_keys": extras,
                "frame_dim": frame_dim,
                "n_stack": obs_dim // frame_dim if frame_dim else 1,
                "obs_dim": obs_dim,
                "created_at": datetime.utcnow().isoformat(),
            }
            with open(self._observation_contract_path(), 'w', encoding='utf-8') as fh:
                json.dump(contract, fh, indent=2)
            logger.info(
                f"[CONTRACT] {self.specialist_name}: {len(contract['feature_columns'])} features + "
                f"{len(extras)} extras + 13 de estado × {contract['n_stack']} frames = {obs_dim}"
            )
        except Exception as exc:
            logger.error(f"[CONTRACT] Falha ao salvar contrato de observação: {exc}")

    def _load_observation_contract(self) -> Optional[Dict[str, Any]]:
        path = self._observation_contract_path()
        if not os.path.exists(path):
            logger.warning(f"[CONTRACT] {self.specialist_name}: contrato de observação ausente — retreine para operar ao vivo.")
            return None
        try:
            with open(path, 'r', encoding='utf-8') as fh:
                return json.load(fh)
        except Exception as exc:
            logger.error(f"[CONTRACT] Falha ao ler {path}: {exc}")
            return None

    def build_live_observation(self, df: pd.DataFrame, agent_state_at) -> Optional[np.ndarray]:
        """
        Monta ao vivo a observação exatamente como TrendFollowingEnv + VecFrameStack no treino:
        [frame(t-n+1), ..., frame(t)], frame = features escaladas + extras + estado do agente + tempo + prior + física.
        `agent_state_at(offset)` devolve o estado do agente `offset` candles atrás.
        Retorna None (recusa) se o contrato estiver ausente ou não bater com o modelo.
        """
        contract = getattr(self, 'observation_contract', None)
        if not contract or self.model is None or self.feature_scaler is None:
            return None
        feature_columns = contract["feature_columns"]
        extras = contract["extra_feature_keys"]
        n_stack = int(contract["n_stack"])
        frame_dim = int(contract["frame_dim"])
        if int(self.model.observation_space.shape[0]) != int(contract["obs_dim"]) or len(df) < n_stack:
            return None
        missing = [c for c in feature_columns if c not in df.columns]
        if missing:
            logger.error(f"[CONTRACT] {self.specialist_name}: features ausentes ao vivo: {missing[:8]}")
            return None

        window = df.iloc[-max(n_stack + 60, 60):].copy()
        for w in (20, 50):  # mesmas features de memória calculadas pelo ambiente de treino
            roll_max = window['high'].rolling(w).max()
            roll_min = window['low'].rolling(w).min()
            window[f'dist_to_max_{w}'] = ((window['close'] - roll_max) / (roll_max + 1e-9)).fillna(0.0)
            window[f'dist_to_min_{w}'] = ((window['close'] - roll_min) / (roll_min + 1e-9)).fillna(0.0)
        entropy_arr, hurst_arr = rolling_chaos_metrics(window['close'].to_numpy(dtype=float))

        market = window[feature_columns].iloc[-n_stack:].astype(float)
        market = market.replace([np.inf, -np.inf], np.nan).fillna(0.0)
        scaled = np.asarray(self.feature_scaler.transform(market), dtype=np.float32)
        ...
        for k in range(n_stack):
            ...
            row = window.iloc[i]
            ts = window.index[i]
            extras_vals = np.nan_to_num(np.array([float(row.get(key, 0.0)) for key in extras], dtype=np.float32))
            time_features = np.array([
                np.sin(2 * np.pi * ts.hour / 24), np.cos(2 * np.pi * ts.hour / 24),
                np.sin(2 * np.pi * ts.dayofweek / 7), np.cos(2 * np.pi * ts.dayofweek / 7),
            ], dtype=np.float32)
            prior = np.nan_to_num(np.array([
                float(row.get('tp_prior_dir', 0.0)),
                float(np.clip(float(row.get('tp_prior_conf', 0.0)), 0.0, 1.0)),
            ], dtype=np.float32))
            physics = np.array([entropy_arr[i], hurst_arr[i]], dtype=np.float32)
            state = np.asarray(agent_state_at(n_stack - 1 - k), dtype=np.float32)
            frame = np.concatenate([scaled[k], extras_vals, state, time_features, prior, physics])
            if frame.shape[0] != frame_dim:
                logger.error(f"[CONTRACT] {self.specialist_name}: frame com {frame.shape[0]} valores, contrato exige {frame_dim}.")
                return None
            frames.append(frame)
    ...
    def decide_action(self, observation: np.ndarray, df_row: pd.Series, pre_normalized: bool = False) -> Optional[Signal]:
        ...
        try:
            ...
            if self.feature_scaler is not None and not pre_normalized:
                try:
            ...
            if uncertainty is not None:
                try:
                    confidence *= float(np.clip(1.0 - float(uncertainty) * 0.5, 0.5, 1.0))
                except Exception:
                    pass
            ...
            if gating_active:
                ...
                explanation["reason"] = f"Sinal de tendencia fraco (valor: {position_action_raw:.2f})"
            elif position_action_raw > action_threshold:
                final_action = Action.BUY
                explanation["reason"] = f"Sinal de tendencia de alta forte (valor: {position_action_raw:.2f})"
            else:
                final_action = Action.SELL
                explanation["reason"] = f"Sinal de tendencia de baixa forte (valor: {position_action_raw:.2f})"
            ...
            return Signal(
                ...
                take_profit=take_profit, explanation=explanation
            )
        except Exception as e:
            logger.exception(f"[ERRO TREND SAC] Erro ao decidir acao: {e}.")
            return None

    def refine_decision(self, master_signal: Signal, observation: np.ndarray, df_row: pd.Series) -> Signal:
        """
        ...
        if specialist_signal is None or specialist_signal.action == Action.HOLD:
            ...
            return Signal(
                symbol=master_signal.symbol, action=Action.HOLD, confidence=0.0,
                explanation={
                    "master_delegated_to": master_signal.explanation.get("master_delegated_to"),
        ...
        if specialist_signal.action == Action.BUY and prior_dir < -0.1:
            return Signal(
                symbol=master_signal.symbol, action=Action.HOLD, confidence=0.0,
                explanation={"reason": f"VETO FINAL: Tentativa de Compra contra tendência de Baixa do Predictor ({prior_dir})."}
            )
        if specialist_signal.action == Action.SELL and prior_dir > 0.1:
            return Signal(
                symbol=master_signal.symbol, action=Action.HOLD, confidence=0.0,
                explanation={"reason": f"VETO FINAL: Tentativa de Venda contra tendência de Alta do Predictor ({prior_dir})."}
            )
        # FusÃƒÂ£o de Sinais (lÃƒÂ³gica mantida)
        combined_confidence = (master_signal.confidence * 0.3) + (specialist_signal.confidence * 0.7)
        final_pos_size_pct = self.trading_config.MAX_POSITION_SIZE_PERCENT * combined_confidence
        final_explanation = {**master_signal.explanation, **specialist_signal.explanation, "sanity_check": "Alinhado com a tendÃƒÂªncia principal."}
        return replace(
            specialist_signal,
    ...
    def save_model(self):
        if self.model:
            ...
            self.model.save(self.model_path)
            logger.info("[TREND SAC] TrendSpecialist (SAC) salvo em '%s'.", self.model_path)
    ...
    def load_model(self):
        ...
        try:
            ...
            if os.path.exists(scaler_path):
                ...
                logger.debug(f"✅ [SCALER] Scaler carregado para {self.specialist_name}: {scaler_filename}")
            else:
        ...
        try:
            if os.path.exists(self.reward_params_path):
                ...
                if isinstance(data, dict):
                    return {key: data[key] for key in self.REWARD_PARAM_KEYS if key in data}
        except Exception as exc:
            logger.warning("[TREND SAC] Falha ao carregar parametros de recompensa: %s", exc)
        return {}
    ...
    def _save_reward_params(self, params: Dict[str, Any]) -> None:
        try:
            ...
            logger.info("[TREND SAC] Parametros de recompensa salvos em %s", self.reward_params_path)
        except Exception as exc:
            logger.error("[TREND SAC] Falha ao salvar parametros de recompensa: %s", exc, exc_info=True)
        ...
        for config_key in self.REWARD_PARAM_KEYS:
            if config_key in params:
                try:
                    ...
                    logger.debug("[TREND SAC] Falha ao aplicar %s no config: %s", config_key, exc)
    ...
    def _post_process_env(self, env: TrendFollowingEnv) -> TrendFollowingEnv:
        try:
            ...
            logger.debug("[TREND SAC] Falha ao ajustar parametros de recompensa do ambiente: %s", exc)
        return env
