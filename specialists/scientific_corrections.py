...
logger = logging.getLogger("ScientificReward")
...
def compute_scientific_reward(
    ...
    _mr_halflife = (
        int(getattr(env, "mean_reversion_halflife", 15)) if _is_ranger else 15
    )
    ...
    trade_closed_this_step = _pnl_finite and (
        pnl_realized != 0 or exit_reason is not None
    )
    ...
    if env.position != 0 and hasattr(env, "entry_price") and env.entry_price > 0:
        unrealized_return = (
            (current_price - env.entry_price) / env.entry_price * np.sign(env.position)
        )
        ...
        if not _is_ranger:
            ...
            # [P1 FIX] Reduzido 5x: shaping dominava PnL em 8x (ratio deve ser <1.5x).
            depth_reward = 0.04 * np.tanh(depth_in_atr / 3.0) * leverage
            reward += float(np.clip(depth_reward, -0.06, 0.06))
        else:
            ...
            try:
                ...
                if ema_val > 0:
                    ...
                    _EMA_WELL_MIN_DIST = (
                        0.8  # mínimo 0.8 ATR de distância para ativar o well
                    )
                    if dist_entry >= _EMA_WELL_MIN_DIST:
                        convergence = (dist_entry - dist_current) / max(dist_entry, 0.5)
                        # [P1 FIX] Reduzido 4.5x: alinhado com escala gravity well.
                        ema_well_reward = 0.04 * np.tanh(convergence * 2.5)
                        reward += float(np.clip(ema_well_reward, -0.06, 0.06))
                    ...
                    # [P1 FIX] Fallback também reduzido 5x.
                    depth_reward = 0.04 * np.tanh(depth_in_atr / 3.0)
                    reward += float(np.clip(depth_reward, -0.06, 0.06))
            except Exception as _e:
                logger.warning("EMA Convergence Well indisponivel: %s", _e)
        ...
        if steps_in_pos <= 1:
            ...
            prev_unrealized_sci = float(
                getattr(env, "_prev_sci_unrealized", curr_unrealized)
            )
            ...
            if not _is_ranger:
                ...
                # [P1 FIX] Reduzido 5x.
                momentum_reward = 0.016 * np.tanh(momentum_norm * 5.0)
            else:
                ...
                if curr_unrealized < -0.005 and delta_return < 0:
                    momentum_reward = 0.008 * np.tanh(momentum_norm * 5.0)
                else:
                    momentum_reward = 0.0  # neutro — EMA Well já sinalizou
            env._prev_sci_unrealized = curr_unrealized
        reward += float(np.clip(momentum_reward, -0.03, 0.03))
    ...
    if env.position != 0 and not trade_closed_this_step:
        if not _is_ranger:
            ...
            if steps_in_pos < 5:
                ...
                # [CALIBRATION FIX] Anti-scalp reduced from -3.0 to -1.0.
                # Real tx cost = 2×taker_fee×leverage ≈ 0.24%. At trade_reward
                # scale (×30) = -0.72. Using -1.0 gives ~40% margin.
                # Before: -3.0 made ALL trades <10 steps net-negative regardless
                # of PnL, teaching "never open" instead of "hold longer".
                # Ref: López de Prado (2018) CUSUM — threshold ∝ tx cost.
                scalp_penalty = -1.0 * np.exp(-0.5 * max(0, steps_in_pos - 1))
                reward += float(np.clip(scalp_penalty, -1.5, 0.0))
                ...
                # [P1 FIX] Hold bonus reduzido 5x: 1.5→0.30 (máx). Era a maior
                # fonte de shaping dominance — 15 pts/episódio vs PnL real.
                hold_bonus = 0.30 * (1.0 - np.exp(-0.07 * (steps_in_pos - 10)))
                profit_scale = float(np.clip(0.3 + depth_in_atr * 0.7, 0.0, 1.0))
                reward += float(np.clip(hold_bonus * profit_scale, 0.0, 0.30))
            ...
            #
            # 1. O Ranger tem deterrente de scalping via Component 11 (custo de entrada -0.8)
            #    e via OU efficiency (d < 3 → multiplicador 0.10 ≈ zero).
            #    [C4 FIX] A dedução explícita de taxa foi removida do pnl_base para
            #    evitar double-counting com portfolio.update_from_trade().
            #
    ...
    if hasattr(env, "steps_in_position") and env.steps_in_position == 1:
        ...
        try:
                    ...
                    for n in names:
                        try:
                            v = (
                                row.get(n)
                                if hasattr(row, "get")
                                else (row[n] if n in row.index else None)
                            )
            ...
            logger.warning("Entry Precision (Comp 2) indisponivel: %s", _e)
    ...
    if (
        pnl_realized != 0
        and np.isfinite(pnl_realized)
        and not info.get("trade_reward_already_computed", False)
    ):
        trade_sharpe = trade_return_pct / max(atr_pct, 1e-6)
        sharpe_reward = 8.0 * np.tanh(trade_sharpe)
        duration_mult = float(np.clip(np.sqrt(duration_steps / 10.0), 0.316, 2.5))
        if sharpe_reward > 0:
    ...
    if info.get("_regime_mismatch", False) and not _is_ranger:
        steps_in_pos = int(info.get("steps_in_position", -1))
        if steps_in_pos == 0:
            ...
            # [P1 FIX] Cap reduzido -3.0→-1.5: era 2.64x o sinal PnL médio.
            mismatch_penalty = -1.5 * np.tanh(2.5 * signal_strength)
            reward += float(np.clip(mismatch_penalty, -1.5, 0.0))

    # ─────────────────────────────────────────────────────────────────────────
    # COMPONENTE 5B: REGIME ALIGNMENT BONUS (Rec 2)
    # ─────────────────────────────────────────────────────────────────────────
    # Recompensa o agente por operar com posicao ALINHADA ao regime quando a
    # confianca eh ALTA. Antes do fix, so havia PENALIDADE por mismatch — o
    # agente nao tinha incentivo positivo para "esperar a confirmacao" antes
    # de entrar. Resultado: agente entrava cedo (baixa conf) e era pego em ruido.
    #
    # Formula:
    #   aligned = (pos>0 ∧ dir>0.5 ∧ conf>0.70) OR (pos<0 ∧ dir<-0.5 ∧ conf>0.70)
    #   bonus = 0.30 * tanh(3 * (conf - 0.70))  ∈ [0, 0.30]
    #
    # Tabela de exemplos:
    #   conf=0.70 → bonus ≈ 0.00 (apenas threshold)
    #   conf=0.80 → bonus ≈ 0.087 (alinhamento bom)
    #   conf=0.90 → bonus ≈ 0.174 (alinhamento forte)
    #   conf=1.00 → bonus ≈ 0.247 (max)
    #
    # [RANGER GUARD] Ranger opera BIDIRECIONAL (mean reversion), nao alinhado
    # ao regime — esse componente nao se aplica.
    if env.position != 0 and not trade_closed_this_step and not _is_ranger:
        prior_conf_align = float(info.get("prior_conf_val", 0.0))
        prior_dir_align = float(info.get("prior_dir_val", 0.0))
        aligned_high_conf = (
            env.position > 0 and prior_dir_align > 0.5 and prior_conf_align > 0.70
        ) or (env.position < 0 and prior_dir_align < -0.5 and prior_conf_align > 0.70)
        if aligned_high_conf:
            # [CRITICAL FIX 2026-05-13 v2] Bonus reduzido de 0.20 -> 0.15.
            # Ratio shaping/PnL era 6.8x (meta: 0.5-1.5x). Com 0.20/step em 50 steps,
            # alignment acumulava ~8.05 vs trade_reward ~4.80 (ratio ~1.68x — ainda acima).
            # Com 0.15, ratio cai para ~1.26x dentro do target [0.5-1.5x].
            # Ref: audit finding reward-auditor CRITICAL line 46.
            base_bonus = 0.15 * np.tanh(3.0 * (prior_conf_align - 0.70))
            # Modulador por profundidade: mais lucro -> mais bonus (max 1.5x)
            depth_multiplier = float(
                np.clip(1.0 + 0.5 * np.tanh(depth_in_atr), 0.5, 1.5)
            )
            alignment_bonus = base_bonus * depth_multiplier
            reward += float(np.clip(alignment_bonus, 0.0, 1.2))
    ...
    if (
        ...
        # [SCALE FIX] Emergency reduced from -3.0 to -1.5/step.
        # Before: trade at -2.5% for 6 steps = -18 penalty, dominating eventual
        # +3.0 trade_reward if price reverses. Destroys trend-following where
        # temporary drawdowns of 2-3% before +10% moves are normal.
        emergency_penalty = -1.5 * np.tanh(excess_loss / 0.02)
        reward += float(np.clip(emergency_penalty, -1.5, 0.0))
    ...
    if (
        trade_closed_this_step
        and np.isfinite(trade_return_pct)
        and not _trade_already_computed
    ):
        if trade_return_pct >= 0:
            sortino_up = 0.5 * np.tanh(trade_return_pct / max(atr_pct, 1e-6))
            reward += float(np.clip(sortino_up, 0.0, 1.0))
        else:
            # [FIX BUG-05] Sortino canônico: penalidade proporcional a |retorno| / downside
            # (Sortino & van der Meer, 1991). Fórmula anterior usava r²/σ² (quadrática),
            # que saturada tanh para qualquer perda > 3% independente da magnitude real.
            downside_ratio = abs(trade_return_pct) / max(atr_pct, 1e-6)
            sortino_down = -1.5 * np.tanh(downside_ratio)
            reward += float(np.clip(sortino_down, -2.0, 0.0))

    # ─────────────────────────────────────────────────────────────────────────
    # COMPONENTE 8: RECOVERY RATIO BONUS (inspirado em Calmar — Young, 1991)
    # ─────────────────────────────────────────────────────────────────────────
    # Mede "retorno / drawdown" como proxy de qualidade de recuperação do trade.
    # Não é o Calmar Ratio canônico (que é CAGR/MaxDD sobre o sistema inteiro),
    # mas sim um bônus local que premia trades que recuperam drawdown acumulado.
    #
    # Aplicado no fechamento de trades positivos com drawdown nao-nulo:
    #   recovery_ratio = trade_return_pct / max(episode_max_dd, 0.01)
    #   bonus = 0.8 * tanh(recovery_ratio / 5.0)   [satura em ≈ +0.8]
    #
    # Nao aplicado em trades negativos (o Sortino e o Delta DD ja cobrem isso).
    # [RANGER GUARD] Idem ao Sortino — Ranger usa OU efficiency como proxy.
    if (
        trade_closed_this_step
        and trade_return_pct > 0
        and current_dd > 0.005
        and not _trade_already_computed
    ):
        recovery_ratio = trade_return_pct / max(current_dd, 0.01)
        recovery_ratio = min(recovery_ratio, 20.0)  # cap superior: saturar em 20x
        recovery_bonus = 0.8 * np.tanh(recovery_ratio / 5.0)
        reward += float(np.clip(recovery_bonus, 0.0, 0.8))
    ...
    if trade_closed_this_step and not _trade_already_computed:
        if not _is_ranger:
            ...
            if hold_completion_bonus > 0:
                quality = float(
                    np.clip(trade_return_pct / max(atr_pct, 1e-6), 0.0, 1.0)
                )
    ...
    if env.position == 0 and not trade_closed_this_step:
        ...
        if not _is_ranger:
            ...
            try:
                ...
                logger.warning("Opportunity Cost Ranger (Comp 10) indisponivel: %s", _e)
    ...
    #
    # [C4 FIX] Componente 11 agora se aplica a TODOS os especialistas uniformemente.
    # Antes: Ranger desabilitado porque pnl_base já deduzia 2×TAKER_FEE×leverage.
    # Depois do fix C4: pnl_base do Ranger usa retorno bruto (sem dedução de taxa).
    # Sem este componente, o Ranger aprenderia em ambiente free-of-cost → overtrading.
    # O -0.8 representa o custo de oportunidade + fricção de round-trip, não o valor
    # exato da taxa — é um sinal de qualidade de trade, não contabilidade exata.
    if getattr(env, "steps_in_position", -1) == 0 and env.position != 0:
        reward -= 0.8
    ...
    # ─────────────────────────────────────────────────────────────────────────
    # FINALIZACAO: CQL Clipping conservador
    # ─────────────────────────────────────────────────────────────────────────
    # Ref: Kumar et al. (2020) — Conservative Q-Learning.
    # [CD3 FIX v2] Clip interno assimetrico [-8, +20].
    # Coordenacao com clip externo em trend_specialist.py (15). Simetrico [-8,+8]
    # truncava trades excelentes ao mesmo nivel que ruins — mata convexidade.
    # trade +5%/20steps = 4.8 (trade_reward) + 20*tanh(5/1)=20 (exit_quality)
    # = ~25 total. Com +8 de cap, perde metade do sinal. Com +20, preserva.
    reward = float(np.clip(reward, -8.0, 20.0))
    ...
    if abs(reward) < 1e-6 and not exit_reason:
        reward = -0.10
...
# [FIX PRODUÇÃO] Gates de meta-labeling reativados (Lopez de Prado 2018 Ch.4).
# Estavam hardcoded True desde debugging de HPO — sem eles, o agente não filtra
# sinais de baixa qualidade histórica, anulando o cascateamento científico.
DISABLE_PRIOR_VETO = False
DISABLE_REVERSAL_BLOCK = False
