...
class Config:
    """Classe de configuraÃ§Ã£o base para todo o sistema."""
    SECRET_KEY = os.environ.get('SESSION_SECRET', 'a_super_secret_key_for_quantum_trading_2024')
    if SECRET_KEY == 'a_super_secret_key_for_quantum_trading_2024':
        logger.warning("ðŸš¨ [ALERTE CONFIG] SECRET_KEY padrÃ£o em uso. Por favor, defina SESSION_SECRET no .env para seguranÃ§a em produÃ§Ã£o.")
    ...
    if not DATABASE_URL:
        logger.error("âŒ [ERRO CONFIG] VariÃ¡vel de ambiente 'DATABASE_URL' nÃ£o definida. Usando padrÃ£o localhost.")
        DATABASE_URL = 'postgresql://quantum_user:quantum_pass@localhost:5432/quantum_trading_db'
    ...
    PAPER_TRADING_PURE = os.environ.get('PAPER_TRADING_PURE', 'False').lower() in ('true', '1', 't')
    if PAPER_TRADING_PURE:
        logger.info("📝 [CONFIG] Modo PAPER_TRADING_PURE ativo. Ordens serão simuladas localmente (sem API de ordens).")
...
class TradingConfig(Config):
    ...
    if PRIMARY_PAIR not in TRADING_PAIRS:
        ...
        logger.warning(f"âš ï¸ [ALERTE CONFIG] PRIMARY_PAIR '{PRIMARY_PAIR}' nÃ£o estÃ¡ na lista TRADING_PAIRS. Pode haver inconsistÃªncias.")
    
    PRICE_COLS = ['open', 'high', 'low', 'close', 'volume']
   
    # --- ConfiguraÃ§Ãµes Multi-Timeframe ---
    PRIMARY_TIMEFRAME_TRADING = os.environ.get('PRIMARY_TIMEFRAME_TRADING', '15m') # Exemplo para 15 minutos
    SECONDARY_TIMEFRAMES = os.environ.get('SECONDARY_TIMEFRAMES', '1h,4h').split(',') # Timeframes acima
    MICRO_TIMEFRAMES = os.environ.get('MICRO_TIMEFRAMES', '1m,5m').split(',') # Timeframes abaixo
    ...
    ATR_TAKE_PROFIT_MULTIPLIER = float(os.environ.get('ATR_TAKE_PROFIT_MULTIPLIER', 7.5))
    SL_DIST_MULTIPLIER_MIN = float(os.environ.get('SL_DIST_MULTIPLIER_MIN', 1.0))  # Evita SLs muito apertados
    SL_DIST_MULTIPLIER_MAX = float(os.environ.get('SL_DIST_MULTIPLIER_MAX', 5.0))  # Limite superior
    TP_DIST_MULTIPLIER_MIN = float(os.environ.get('TP_DIST_MULTIPLIER_MIN', 0.5))  # Limite inferior para TP
    TP_DIST_MULTIPLIER_MAX = float(os.environ.get('TP_DIST_MULTIPLIER_MAX', 10.0))  # Limite superior para TP
    ...
    SL_DIST_MULTIPLIER_MIN_VOLATILE = float(os.environ.get('SL_DIST_MULTIPLIER_MIN_VOLATILE', 3.0))
    SL_DIST_MULTIPLIER_MAX_VOLATILE = float(os.environ.get('SL_DIST_MULTIPLIER_MAX_VOLATILE', 10.0))
    TP_DIST_MULTIPLIER_MIN_VOLATILE = float(os.environ.get('TP_DIST_MULTIPLIER_MIN_VOLATILE', 2.0))
    TP_DIST_MULTIPLIER_MAX_VOLATILE = float(os.environ.get('TP_DIST_MULTIPLIER_MAX_VOLATILE', 20.0))
    ...
    USDC_TAKER_FEE = float(os.environ.get('USDC_TAKER_FEE', 0.00036)) # 0.036% para USDC Taker
    ...
    SLIPPAGE_TOLERANCE_PCT = float(os.environ.get('SLIPPAGE_TOLERANCE_PCT', 0.002)) # 0.2%
...
class AIConfig(Config):
    ...
    ONLINE_LEARNING_RATE = float(os.environ.get('ONLINE_LEARNING_RATE', 1e-5)) # Baixo para estabilidade
    ONLINE_ADAPTATION_WINDOW = int(os.environ.get('ONLINE_ADAPTATION_WINDOW', 20)) # Histórico recente de trades
    ...
    SPECIALIST_UPDATE_FREQUENCY_STEPS = int(os.environ.get('SPECIALIST_UPDATE_FREQUENCY_STEPS', 4))
    ...
    MAML_NUM_INNER_UPDATES = int(os.environ.get('MAML_NUM_INNER_UPDATES', 5)) # k-shot learning
    MAML_ADAPTATION_STEPS = int(os.environ.get('MAML_ADAPTATION_STEPS', 10)) # Passos para adaptar na inferÃªncia
    MAML_NUM_OUTER_UPDATES = int(os.environ.get('MAML_NUM_OUTER_UPDATES', 500)) # NÃºmero de meta-atualizaÃ§Ãµes

    # --- ConfiguraÃ§Ãµes de Autoencoder para PadrÃµes Ocultos ---
    AUTOENCODER_LATENT_DIM = int(os.environ.get('AUTOENCODER_LATENT_DIM', 32)) # DimensÃ£o do espaÃ§o latente
    AUTOENCODER_TRAINING_EPOCHS = int(os.environ.get('AUTOENCODER_TRAINING_EPOCHS', 200 ))
    ...
    TREND_PREDICTOR_OPTUNA_TRIALS = int(os.environ.get('TREND_PREDICTOR_OPTUNA_TRIALS', 100))
    ...
    ALLOW_UNTRAINED_EXECUTION = os.environ.get('ALLOW_UNTRAINED_EXECUTION', 'False').lower() in ('true', '1', 't')
    ...
    ENFORCE_STATIONARITY = os.environ.get('ENFORCE_STATIONARITY', 'True').lower() in ('true', '1', 't')
    
    # Walk-Forward Cross-Validation (Purged K-Fold)
    USE_WALK_FORWARD_CV = os.environ.get('USE_WALK_FORWARD_CV', 'False').lower() in ('true', '1', 't')
    ...
    EMBARGO_BARS = int(os.environ.get('EMBARGO_BARS')) if os.environ.get('EMBARGO_BARS') else None
    ...
    LOOKBACK_WINDOW_MAIN = int(os.environ.get('LOOKBACK_WINDOW_MAIN', 800))  # 800 = 250 warmup + 500 buffer para garantir 32+ rows após merge
    ...
    REWARD_SHARPE_RATIO_WEIGHT = float(os.environ.get('REWARD_SHARPE_RATIO_WEIGHT', 0.25)) # Aumentado para considerar performance
    REWARD_RISK_ADJUSTED_RETURN_WEIGHT = float(os.environ.get('REWARD_RISK_ADJUSTED_RETURN_WEIGHT', 0.1)) # Aumentado
    REWARD_PER_STEP_ALIGNED_TREND = float(os.environ.get('REWARD_PER_STEP_ALIGNED_TREND', 0.02)) # Nova recompensa por estar alinhado Ã  tendÃªncia
    
    # --- Penalidades ---
    PENALTY_FEE_MULTIPLIER = float(os.environ.get('PENALTY_FEE_MULTIPLIER', 1.0)) # Multiplicador para penalidade de taxa
    ...
    PENALTY_DRAWDOWN = float(os.environ.get('PENALTY_DRAWDOWN', 7.5))  # Aumentado para maior impacto
    
    # --- Penalidade de Holding DinÃ¢mica ---
    PENALTY_HOLDING_BASE = float(os.environ.get('PENALTY_HOLDING_BASE', 0.01)) # Base da penalidade por passo
    PENALTY_HOLDING_FACTOR_PROB_LOW = float(os.environ.get('PENALTY_HOLDING_FACTOR_PROB_LOW', 2.0)) # Multiplicador se P for baixo
    PENALTY_HOLDING_FACTOR_PROB_HIGH = float(os.environ.get('PENALTY_HOLDING_FACTOR_PROB_HIGH', 0.5)) # Multiplicador se P for alto
    PENALTY_HOLDING_TOO_LONG = float(os.environ.get('PENALTY_HOLDING_TOO_LONG', 0.1)) # Penalidade para especialistas Sideways/Range
    ...
    GRACE_EMERGENCY_EXIT_THRESHOLD = float(os.environ.get('GRACE_EMERGENCY_EXIT_THRESHOLD', -0.02))
    ...
    MIN_HOLD_DURATION_BONUS_STEPS = int(os.environ.get('MIN_HOLD_DURATION_BONUS_STEPS', 3))
    ...
    TREND_PRIOR_FLIP_PENALTY_WEIGHT = float(os.environ.get('TREND_PRIOR_FLIP_PENALTY_WEIGHT', 0.891))
    TREND_PRIOR_FLIP_PENALTY_EXP = float(os.environ.get('TREND_PRIOR_FLIP_PENALTY_EXP', 1.502))
    TREND_PRIOR_FLIP_THRESHOLD = float(os.environ.get('TREND_PRIOR_FLIP_THRESHOLD', 0.331))
    TREND_DURATION_PRESSURE_WEIGHT = float(os.environ.get('TREND_DURATION_PRESSURE_WEIGHT', 0.492))
    TREND_DURATION_PRESSURE_START = int(os.environ.get('TREND_DURATION_PRESSURE_START', 157))
    TREND_DURATION_PRESSURE_EXP = float(os.environ.get('TREND_DURATION_PRESSURE_EXP', 1.102))
    TREND_DRAWDOWN_PENALTY_WEIGHT = float(os.environ.get('TREND_DRAWDOWN_PENALTY_WEIGHT', 1.059))

    # SAC Hyperparameters (Best Trial #5, sincronizados)
    SAC_LEARNING_RATE_DEFAULT = float(os.environ.get('SAC_LEARNING_RATE_DEFAULT', 1.818e-05))
    ...
    SAC_TARGET_ENTROPY_DEFAULT = float(os.environ.get('SAC_TARGET_ENTROPY_DEFAULT', -3.884))
    SAC_MAX_GRAD_NORM_DEFAULT = float(os.environ.get('SAC_MAX_GRAD_NORM_DEFAULT', 0.867))
    ...
    HPO_MIN_TIMESTEPS_PER_TRIAL = int(os.environ.get('HPO_MIN_TIMESTEPS_PER_TRIAL', 25000))


    # --- ConfiguraÃ§Ãµes do Modelo Preditivo de Probabilidade de Lucratividade (P) ---
    PROFIT_PREDICTOR_HIDDEN_DIM = int(os.environ.get('PROFIT_PREDICTOR_HIDDEN_DIM', 128))
    REWARD_PROFIT_PROB_WEIGHT = float(os.environ.get('REWARD_PROFIT_PROB_WEIGHT', 1.0)) # Recompensa por alta P
    REWARD_EXPECTED_PROFIT_WEIGHT = float(os.environ.get('REWARD_EXPECTED_PROFIT_WEIGHT', 5.0)) # Recompensa por alto lucro esperado (escalado)
...
class DataConfig(Config):
    ...
    REALTIME_DATA_RETENTION_DAYS = int(os.environ.get('REALTIME_DATA_RETENTION_DAYS', 14))
    AGGREGATED_DATA_RETENTION_DAYS = int(os.environ.get('AGGREGATED_DATA_RETENTION_DAYS', 365 * 5))
    FEATURE_STORE_UPDATE_INTERVAL_SECONDS = int(os.environ.get('FEATURE_STORE_UPDATE_INTERVAL_SECONDS', 60))
...
class BacktestConfig(Config):
    ...
    EXECUTION_LATENCY_MS_MIN = int(os.environ.get('BACKTEST_EXECUTION_LATENCY_MS_MIN', 5))
    EXECUTION_LATENCY_MS_MAX = int(os.environ.get('BACKTEST_EXECUTION_LATENCY_MS_MAX', 50))
    BACKTEST_WITH_EXPLANATIONS = os.environ.get('BACKTEST_WITH_EXPLANATIONS', 'False').lower() in ('true', '1', 't')
...
class MonitoringConfig(Config):
    ...
    AI_MONITOR_LOG_RETENTION_DAYS = int(os.environ.get('AI_MONITOR_LOG_RETENTION_DAYS', 180))
...
class StressTestConfig(Config):
    ...
    SCENARIOS = {
        'flash_crash': {
            ...
            'max_allowable_drawdown': float(os.environ.get('STRESS_FLASH_CRASH_MAX_DRAWDOWN', 0.25))
        },
        'volatility_shock': {
            'volatility_multiplier': float(os.environ.get('STRESS_VOL_SHOCK_MULTIPLIER', 5.0)),
            'duration_hours': int(os.environ.get('STRESS_VOL_SHOCK_DURATION', 6)),
            'max_allowable_drawdown': float(os.environ.get('STRESS_VOL_SHOCK_MAX_DRAWDOWN', 0.30))
        },
        'liquidity_crisis': {
            'spread_multiplier': float(os.environ.get('STRESS_LIQ_CRISIS_SPREAD', 10.0)),
            'volume_divider': float(os.environ.get('STRESS_LIQ_CRISIS_VOLUME_DIV', 10.0)),
            'duration_hours': int(os.environ.get('STRESS_LIQ_CRISIS_DURATION', 4)),
            'max_allowable_drawdown': float(os.environ.get('STRESS_LIQ_CRISIS_MAX_DRAWDOWN', 0.20))
        },
        'correlation_breakdown': {
            'target_correlation': float(os.environ.get('STRESS_CORR_BREAKDOWN_TARGET', -0.9)),
            'duration_days': int(os.environ.get('STRESS_CORR_BREAKDOWN_DURATION', 5)),
            'max_allowable_drawdown': float(os.environ.get('STRESS_CORR_BREAKDOWN_MAX_DRAWDOWN', 0.40))
        },
        'api_outage': {
            'outage_duration_minutes': int(os.environ.get('STRESS_API_OUTAGE_DURATION', 120)),
            'partial_outage_pct': float(os.environ.get('STRESS_API_OUTAGE_PARTIAL_PCT', 0.8)),
            'max_allowable_drawdown': float(os.environ.get('STRESS_API_OUTAGE_MAX_DRAWDOWN', 0.10))
        },
        'black_swan': {
            ...
            'volatility_multiplier': float(os.environ.get('STRESS_BLACK_SWAN_VOL_MULT', 20.0)),
            'liquidity_crisis': os.environ.get('STRESS_BLACK_SWAN_LIQ_CRISIS', 'True').lower() in ('true', '1', 't'),
            'max_allowable_drawdown': float(os.environ.get('STRESS_BLACK_SWAN_MAX_DRAWDOWN', 0.50))
        }
    }
...
if hasattr(active_config, 'PENALTY_FEE_MULTIPLIER'): # Alterado para PENALTY_FEE_MULTIPLIER
    logger.info(f"[CONFIG DEBUG] Valor de PENALTY_FEE_MULTIPLIER carregado: {active_config.PENALTY_FEE_MULTIPLIER}")
